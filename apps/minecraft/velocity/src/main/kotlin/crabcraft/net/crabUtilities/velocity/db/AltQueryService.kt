...
class AltQueryService(private val dataSource: HikariDataSource, private val logger: Logger) {
    ...
    /**
     * Check if a Minecraft UUID is registered as an alt account. Called once per proxy join — zero overhead when nobody
     * is joining.
     */
    fun isAlt(minecraftUuid: String): Boolean {
        val sql: String = "SELECT 1 FROM player_alts WHERE minecraft_uuid = ? LIMIT 1"
        try {
