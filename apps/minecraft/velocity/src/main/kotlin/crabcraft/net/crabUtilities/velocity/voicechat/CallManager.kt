...
class CallManager(
    ...
    private val worker =
        ThreadPoolExecutor(
            ...
            { runnable ->
                Thread(runnable, "CrabUtilities-Calls").apply { isDaemon = true }
            },
    ...
    fun start() {
        val poolConfig =
            JedisPoolConfig().apply {
                maxTotal = 2
                setMaxWait(Duration.ofMillis(1500L))
            }
    ...
    fun invite(caller: Player, target: Player) {
        if (caller.uniqueId == target.uniqueId) {
    ...
    fun accept(target: Player, token: String) = submit(target) { acceptInvite(target, token) }

    fun decline(target: Player, token: String) = submit(target) { declineInvite(target, token) }

    @Subscribe(order = PostOrder.LAST)
    fun onDisconnect(event: DisconnectEvent) {
        val disconnectedSession = event.player
        val playerId = disconnectedSession.uniqueId
        val removed =
            synchronized(acceptanceLifecycleLock) {
                ...
                invites.removeSession(playerId, disconnectedSession).also { removed ->
        ...
        for (invite in removed) {
            ...
            plugin.getServer().getPlayer(otherId).ifPresent { other ->
                error(other, "${event.player.username} went offline, so the call was cancelled.")
            }
        }
    }

    fun shutdown() {
        // Let a committed transaction finish local state/messages before a replacement manager starts.
        ...
        synchronized(acceptanceLifecycleLock) {
            ...
            for (tasks in ArrayList(tasksByToken.values)) {
                pendingByToken[tasks.invite.token()] = tasks.invite
                tasks.cancel()
            }
            tasksByToken.clear()
            for (retry in ArrayList(stopRetriesByToken.values)) {
                pendingByToken[retry.invite.token()] = retry.invite
                retry.cancel()
            }
        ...
        if (pool != null && !pool.isClosed && pendingByToken.isNotEmpty()) {
            try {
                pool.resource.use { jedis ->
                    for (invite in pendingByToken.values) publishRingtoneStops(jedis, invite)
                }
            } catch (e: Exception) {
    ...
    private fun createInvite(callerSession: Player, targetSession: Player) {
        ...
        if (currentCaller !== callerSession || currentTarget !== targetSession) {
            if (currentCaller === callerSession) error(currentCaller, "That player is no longer online.")
            return
        }
        if (
            locationTracker.currentRoute(callerSession) == null || locationTracker.currentRoute(targetSession) == null
        ) {
        ...
        try {
            pool.resource.use { jedis ->
                ...
                if (targetCall != null) {
                    error(
                        currentCaller,
                        if (targetCall.groupId() == activeCall(jedis, callerId)?.groupId())
                            "${targetSession.username} is already in your call."
                        else "${targetSession.username} is already in another call.",
                    )
                    return
                }
                ...
                val call =
                    active
                        ?: invites.provisionalFor(callerId, callerSession, now) {
                            CallInviteRegistry.CallCredentials(
                                UUID.randomUUID(),
                                randomSecret(24),
                ...
                val invite =
                    CallInviteRegistry.Invite(
                        ...
                        call,
                        active != null,
                        ringingStartedAt + INVITE_TIMEOUT_MILLIS,
                    )
                synchronized(acceptanceLifecycleLock) {
                    if (!running) return
                    ...
                    if (!invites.add(invite, ringingStartedAt)) {
                        error(liveCaller, "${targetSession.username} already has an incoming call.")
                        return
                    }
    ...
    private fun acceptInvite(acceptingSession: Player, token: String) {
        ...
        val invite = takeRingingInvite(acceptingSession, token) ?: return
        ...
        if (caller !== invite.callerSession() || target !== invite.targetSession() || target !== acceptingSession) {
            if (target === acceptingSession) error(target, "The caller is no longer online.")
            return
        }
        // Session-bound production invitations always carry both live players.
        ...
        try {
            pool.resource.use { jedis ->
                ...
                if (
                    (callerCall != null && callerCall.groupId() != invite.call().groupId()) ||
                ...
                if (targetCall != null && targetCall.groupId() != invite.call().groupId()) {
                ...
                synchronized(acceptanceLifecycleLock) {
                    if (!running) return
                    if (
                        plugin.getServer().getPlayer(invite.callerId()).orElse(null) !== caller ||
                    ...
                    val result =
                        jedis.eval(
                            ...
                            listOf(
                                ...
                                groupId,
                                groupDefinition,
                                PLAYER_GROUP_TTL_SECONDS.toString(),
                                invite.callerId().toString(),
                                targetId.toString(),
                                GROUP_MEMBERS_KEY_PREFIX,
                                CALL_TARGET_TTL_SECONDS.toString(),
                                callerTarget,
                                targetTarget,
                                CONTROL_CHANNEL,
                                OP_GROUP_CHANGED + SEPARATOR,
                                encodeCallJoin(invite.call().groupId(), invite.callerId(), callerGeneration),
                                encodeCallJoin(invite.call().groupId(), targetId, targetGeneration),
                                callerGroupValue ?: "",
                                targetGroupValue ?: "",
                            ),
                        )
                    ...
                    target.sendMessage(
                        Component.text("Call accepted. Connecting you to ", NamedTextColor.GREEN)
                            .append(Component.text(invite.callerName(), NamedTextColor.AQUA))
                            .append(Component.text("…", NamedTextColor.GREEN))
                    )
                    caller.sendMessage(
                        Component.text(invite.targetName(), NamedTextColor.AQUA)
                            .append(
                                Component.text(
                                    " accepted your call. Connecting… Once connected, anyone in the call can use /call <player> to invite more.",
    ...
    private fun declineInvite(decliningSession: Player, token: String) {
        val invite = takeRingingInvite(decliningSession, token) ?: return
        plugin.getServer().getPlayer(decliningSession.uniqueId).ifPresent { player ->
    ...
    /** Both responses consume the same session-bound invite and terminate its ringtone. */
    private fun takeRingingInvite(targetSession: Player, token: String): CallInviteRegistry.Invite? {
        val targetId = targetSession.uniqueId
        val (taken, ringing) =
            synchronized(acceptanceLifecycleLock) {
                if (!running) return null
                val taken = invites.take(token, targetId, targetSession, System.currentTimeMillis()).orElse(null)
                val ringing = taken ?: cancelInviteTasksForTarget(token, targetId, targetSession)
                if (ringing != null) registerRingtoneStopRetriesLocked(ringing)
                taken to ringing
            }
        if (ringing != null) publishRingtoneStops(ringing)
        if (taken == null)
    ...
    private fun beginRinging(invite: CallInviteRegistry.Invite, caller: Player, target: Player, jedis: Jedis) {
        ...
        val remainingMillis = maxOf(0L, invite.expiresAtMillis() - System.currentTimeMillis())
        ...
        try {
            timeout =
                plugin
                    .getServer()
                    .scheduler
                    .buildTask(plugin, Runnable { expireInvite(invite.token()) })
            ...
            retry =
                plugin
                    .getServer()
                    .scheduler
                    .buildTask(plugin, Runnable { retryRingtoneStart(invite.token()) })
        ...
        caller.sendMessage(
            Component.text("Calling ", NamedTextColor.YELLOW)
    ...
    private fun retryRingtoneStart(token: String) {
        synchronized(acceptanceLifecycleLock) {
            ...
            if (
                !running ||
                    tasks == null ||
                    !invites.isPending(token, tasks.invite.targetId(), System.currentTimeMillis())
            )
                return
    ...
    private fun expireInvite(token: String) {
        val invite =
            synchronized(acceptanceLifecycleLock) {
                val expired = invites.expire(token, Long.MAX_VALUE).orElse(null)
                if (expired != null) registerRingtoneStopRetriesLocked(expired) else cancelInviteTasks(token)
                expired
            } ?: return
        publishRingtoneStops(invite)
        plugin.getServer().getPlayer(invite.callerId()).ifPresent { player ->
        ...
        plugin.getServer().getPlayer(invite.targetId()).ifPresent { player ->
            player.sendMessage(
                Component.text("Missed call from ", NamedTextColor.YELLOW)
                    .append(Component.text(invite.callerName(), NamedTextColor.AQUA))
                    .append(Component.text(".", NamedTextColor.YELLOW))
            )
        }
    ...
    private fun activeCall(jedis: Jedis, playerId: UUID): CallInviteRegistry.CallCredentials? =
        activeCall(jedis, jedis.get(playerGroupKey(playerId)))
    ...
    private fun activeCall(jedis: Jedis, groupValue: String?): CallInviteRegistry.CallCredentials? {
        ...
        return try {
            ...
            if (!isCallGroupDefinition(definition)) null
            else CallInviteRegistry.CallCredentials(groupId, definition!!.password()!!, Long.MAX_VALUE)
    ...
    private fun cancelInviteTasks(token: String) {
        tasksByToken.remove(token)?.cancel()
    }

    private fun cancelInviteTasksForTarget(
        token: String,
        targetId: UUID,
        targetSession: Any?,
    ): CallInviteRegistry.Invite? {
        val tasks = tasksByToken[token] ?: return null
        if (
            tasks.invite.targetId() != targetId ||
                (tasks.invite.targetSession() != null && tasks.invite.targetSession() !== targetSession) ||
        ...
        try {
            val task =
                plugin
                    .getServer()
                    .scheduler
                    .buildTask(plugin, Runnable { retryRingtoneStop(invite.token()) })
                    .delay(Duration.ofMillis(retryDelayMillis))
                    .repeat(RING_STOP_RETRY_INTERVAL)
                    .schedule()
            retry.attach(task)
        } catch (e: RuntimeException) {
            // Retain unscheduled entries so shutdown can publish their final STOP.
    ...
    private fun retryRingtoneStop(token: String) {
        val retry =
            synchronized(acceptanceLifecycleLock) {
                val retry = stopRetriesByToken[token] ?: return
    ...
    private fun publishRingtoneStops(invite: CallInviteRegistry.Invite): Unit =
        publishRingtone(invite, "Could not stop voice call ringtones", ::publishRingtoneStops)

    private fun publishRingtoneStopRetry(invite: CallInviteRegistry.Invite): Unit =
        publishRingtone(invite, "Could not retry voice call ringtone stops", ::publishRingtoneStopRetry)

    private fun publishRingtoneStarts(invite: CallInviteRegistry.Invite): Unit =
        publishRingtone(invite, "Could not retry voice call ringtones", ::publishRingtoneStarts)

    private fun publishRingtone(
        invite: CallInviteRegistry.Invite,
        failureMessage: String,
        publish: (Jedis, CallInviteRegistry.Invite) -> Unit,
    ) {
        val pool = jedisPool ?: return
        if (pool.isClosed) return
        try {
            pool.resource.use { publish(it, invite) }
        } catch (e: Exception) {
            if (running) plugin.getLogger().warn(failureMessage, e)
        }
    }
    ...
    private data class InviteTasks(
        val invite: CallInviteRegistry.Invite,
        val retry: ScheduledTask,
        val timeout: ScheduledTask,
    ) {
        fun cancel() {
            retry.cancel()
            timeout.cancel()
    ...
    private class StopRetry(val invite: CallInviteRegistry.Invite) {
        private var task: ScheduledTask? = null
        private var cancelled = false

        @Synchronized
        fun attach(scheduledTask: ScheduledTask) {
            if (cancelled) scheduledTask.cancel() else task = scheduledTask
        }
    ...
    companion object {
        ...
        private val ACCEPT_CALL_SCRIPT =
            """
        ...
        private val RETRY_CALL_RING_STOP_SCRIPT =
            """
        ...
        @JvmStatic
        fun encodeCallRingStopRetryArguments(invite: CallInviteRegistry.Invite): List<String> =
            listOf(
                CONTROL_CHANNEL,
                encodeCallRingStop(invite.token(), invite.callerId(), RingDirection.OUTGOING),
                encodeCallRingStop(invite.token(), invite.targetId(), RingDirection.INCOMING),
                invite.expiresAtMillis().toString(),
            )
        ...
        @JvmStatic
        fun callTargetValue(groupId: UUID?, generation: String?): String {
            requireValidRingToken(generation)
        ...
        @JvmStatic
        fun encodeCallGroupDefinition(call: CallInviteRegistry.CallCredentials?): String {
            require(call != null && isOpaqueToken(call.password())) { "Invalid call group" }
            return listOf(encodeText(CALL_GROUP_NAME), "1", encodeText(call.password()), "OPEN", "1", "0")
                .joinToString(SEPARATOR)
        }

        @JvmStatic
        fun decodeGroupDefinition(groupId: UUID?, encoded: String?): GroupDefinition? {
            if (groupId == null || encoded == null) return null
            val fields = encoded.split(SEPARATOR)
            if (
                fields.size != 6 ||
                    (fields[1] != "0" && fields[1] != "1") ||
                    (fields[3] != "NORMAL" && fields[3] != "OPEN" && fields[3] != "ISOLATED") ||
            ...
            return try {
                ...
                GroupDefinition(groupId, decodeText(fields[0]), password, fields[3], fields[4] == "1", fields[5] == "1")
            } catch (_: IllegalArgumentException) {
                null
            }
        }
        ...
        @JvmStatic
        fun isCallGroupDefinition(definition: GroupDefinition?): Boolean =
            definition != null &&
                definition.name() == CALL_GROUP_NAME &&
                definition.hidden() &&
                !definition.permanent() &&
                definition.type() == "OPEN" &&
                isOpaqueToken(definition.password())

        private fun encodeText(value: String): String =
            Base64.getEncoder().encodeToString(value.toByteArray(Charsets.UTF_8))

        private fun decodeText(value: String): String = String(Base64.getDecoder().decode(value), Charsets.UTF_8)

        @JvmStatic
        fun encodeCallRingStart(
            token: String?,
            playerId: UUID?,
        ...
        @JvmStatic
        fun shouldRetryRingtoneStop(expiresAtMillis: Long, nowMillis: Long): Boolean = nowMillis < expiresAtMillis

        @JvmStatic
        fun ringtoneStopRetryDelayMillis(expiresAtMillis: Long, nowMillis: Long): Long {
            if (!shouldRetryRingtoneStop(expiresAtMillis, nowMillis)) return 0L
            return minOf(RING_STOP_RETRY_INTERVAL.toMillis(), maxOf(1L, (expiresAtMillis - nowMillis) / 2L))
        }

        private fun requireValidRingToken(token: String?) {
            require(isOpaqueToken(token)) { "Invalid call ringtone token" }
        }

        @JvmStatic fun acceptCommand(token: String): String = "/call accept $token"

        @JvmStatic fun declineCommand(token: String): String = "/call decline $token"

        private fun playerGroupKey(playerId: UUID): String = PLAYER_GROUP_KEY_PREFIX + playerId

        private fun callTargetKey(playerId: UUID): String = CALL_TARGET_KEY_PREFIX + playerId
