...
/** Persists award scores and medal rankings to PostgreSQL. */
class AwardDbWriter(private val dataSource: HikariDataSource, private val logger: Logger) {

    /**
     * Writes one player's scores without refreshing season-wide medals. Callers that process many players should
     * batch/debounce {@link #recomputeMedals(String)} instead of running it once per player.
     */
    ...
    fun recomputeAllMedals() {
        try {
            dataSource.connection.use { conn ->
                conn.autoCommit = false
                try {
                    val seasons: MutableList<String> = ArrayList()
                    conn.prepareStatement("SELECT DISTINCT season FROM player_award_scores").use { stmt ->
    ...
    companion object {
        private val UPSERT_SCORE: String =
            ("""
        ...
        private val RESET_MEDALS: String =
            ("""
        ...
        private val RECOMPUTE_MEDALS: String =
            ("""
            WITH ranked AS (
                SELECT
                    scores.id,
