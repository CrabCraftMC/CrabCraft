...
open class PostgresStatsWriter {
    enum class NicknameLoadStatus {
        FOUND,
        ABSENT,
        FAILED,
    }
    ...
    data class NicknameLoadResult(val status: NicknameLoadStatus, val rawNickname: String?) {
        companion object {
            @JvmStatic
            fun found(rawNickname: String?): NicknameLoadResult {
                return NicknameLoadResult(NicknameLoadStatus.FOUND, rawNickname)
            }

            @JvmStatic
            fun absent(): NicknameLoadResult {
                return NicknameLoadResult(NicknameLoadStatus.ABSENT, null)
            }

            @JvmStatic
            fun failed(): NicknameLoadResult {
                return NicknameLoadResult(NicknameLoadStatus.FAILED, null)
            }
        }
    ...
    constructor(jdbcUrl: String, username: String, password: String, logger: Logger) {
        this.logger = logger
    ...
    /**
     * Updates player info (username + nickname) by minecraft_uuid. Only updates existing players — no-op if UUID not in
     * DB.
     */
    open fun upsertPlayer(uuid: String, username: String, nickname: String?, nicknameRaw: String?): Boolean {
        val selectSql: String = "SELECT last_mc_login_at FROM players WHERE minecraft_uuid = ? FOR UPDATE"
        val updateSql: String =
            "UPDATE players SET minecraft_username = ?, nickname = ?, nickname_raw = ?, " +
                ...
                try {
                    ...
                    conn.prepareStatement(selectSql).use { stmt ->
                        ...
                        stmt.executeQuery().use { rs ->
                            if (rs.next()) {
                                val storedLastLoginAt: Long = rs.getLong("last_mc_login_at")
                                val lastLoginAt: Long? = if (rs.wasNull()) null else storedLastLoginAt
                                wasInactive =
                                    isInactiveForLeaderboard(
                                        lastLoginAt,
    ...
    open fun upsertAltUsername(uuid: String, username: String) {
        val sql: String = "UPDATE player_alts SET minecraft_username = ? WHERE minecraft_uuid = ?"
        try {
            dataSource.connection.use { conn ->
     ...
     *
     * Fail-safe: on any SQLException (pool exhaustion, slow query, DB outage) this returns {@code true}, so the join
     * broadcaster falls through to the regular "joined the game" message rather than wrongly announcing a returning
     * player as a first-time visitor.
     */
    open fun hasJoinedBefore(uuid: String): Boolean {
        ...
        try {
            ...
            logger.error(
                "Failed to check join status for {} — defaulting to 'joined before' to avoid wrongly announcing them as a first-time player",
                uuid,
                e,
            )
            return true
    ...
    /**
     * Records a Minecraft login in mc_login_history. Inserts on first sight, otherwise bumps last_seen_at. Independent
     * of Discord verification — every connecting UUID is tracked here so the "first join" check works for unverified
     * players.
     */
    ...
    open fun updateNickname(uuid: String, nickname: String?, nicknameRaw: String?) {
        val sql: String =
            "UPDATE players SET nickname = ?, nickname_raw = ?, updated_at = EXTRACT(EPOCH FROM NOW())::INTEGER WHERE minecraft_uuid = ?"
        try {
            dataSource.connection.use { conn ->
                conn.prepareStatement(sql).use { stmt ->
    ...
    /**
     * Loads the stored raw nickname for a player by minecraft_uuid, keeping a missing nickname distinct from a database
     * failure.
     */
    open fun loadRawNickname(uuid: String): NicknameLoadResult {
        val sql: String = "SELECT nickname_raw FROM players WHERE minecraft_uuid = ?"
        try {
            dataSource.connection.use { conn ->
                conn.prepareStatement(sql).use { stmt ->
                    stmt.setString(1, uuid)
                    stmt.executeQuery().use { rs ->
                        if (rs.next()) {
                            ...
                            return if (raw == null || raw.isEmpty()) NicknameLoadResult.absent()
                            else NicknameLoadResult.found(raw)
                        }
                    }
                }
            }
    ...
    /**
     * Shared HikariCP pool. Exposed so auxiliary writers (awards, crown scores) can reuse the same connection pool
    ...
    companion object {
        private val LEADERBOARD_ACTIVITY_WINDOW_SECONDS: Long = 30L * 24L * 60L * 60L

        @JvmStatic
        fun isInactiveForLeaderboard(lastLoginAt: Long?, now: Long): Boolean {
            return lastLoginAt == null || lastLoginAt < now - LEADERBOARD_ACTIVITY_WINDOW_SECONDS
        }
