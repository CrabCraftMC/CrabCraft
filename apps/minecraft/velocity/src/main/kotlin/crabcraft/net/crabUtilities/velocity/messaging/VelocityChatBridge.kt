...
/** Routes Paper-processed chat through Velocity and returns final components to the active backend. */
class VelocityChatBridge(private val plugin: CrabUtilitiesVelocity) {
    ...
    fun onPluginMessage(event: PluginMessageEvent) {
        ...
        if (source !is ServerConnection || player !is Player || player.currentServer.filter(source::equals).isEmpty) {
            plugin.getLogger().warn("Ignored chat bridge payload without a matching backend player")
            return
        }
        val packet =
            try {
                ChatBridgeProtocol.decode(event.data)
            } catch (e: IllegalArgumentException) {
                plugin.getLogger().warn("Ignored malformed chat bridge payload: {}", e.message)
                return
            }
    ...
    fun onServerPostConnect(event: ServerPostConnectEvent) {
        ...
        syncStaffState(event.player, manager == null || manager.isEnabled(event.player.uniqueId))
    }

    fun deliver(player: Player, component: Component): Boolean {
        val payload =
            try {
                ChatBridgeProtocol.delivery(player.uniqueId, GSON.serialize(component))
            } catch (_: IllegalArgumentException) {
                plugin.getLogger().warn("Chat component for {} was too large for the Paper bridge", player.username)
                return false
            }
    ...
    private fun handleStaffRequest(player: Player, componentJson: String) {
        if (!plugin.getStaffChatManager()!!.hasPermission(player)) {
            plugin
                ...
                .deliver(
                    player,
                    Component.text("You do not have permission to use staff chat.", NamedTextColor.RED),
                )
            return
        }
        val message =
            try {
                ...
                plugin.getLogger().warn("Ignored invalid staff-chat component from {}", player.username)
                return
            }
        val senderName = plugin.getNicknameCache().getRawNickname(player.uniqueId) ?: player.username
