...
/**
 * Keeps the Kotlin-side database bootstrap in lockstep with the Gallery tables
 ...
 */
class GallerySchema private constructor() {
    companion object {
        private val CREATE_SCHEMA_SQL = arrayOf(
            """
            ...
            try {
                dataSource.connection.use { connection ->
                    connection.createStatement().use { statement ->
                        ...
                        try {
                            for (sql in CREATE_SCHEMA_SQL) statement.execute(sql)
                            connection.commit()
