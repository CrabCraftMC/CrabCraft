...
class CallInviteRegistry {
    ...
    data class Invite(
        ...
        private val callerSession: Any?,
        private val targetSession: Any?,
        private val call: CallCredentials,
        private val callerWasInCall: Boolean,
        private val expiresAtMillis: Long,
    ) {
        constructor(
            token: String,
            ...
            callerWasInCall: Boolean,
            expiresAtMillis: Long,
        ) : this(token, callerId, callerName, targetId, targetName, null, null, call, callerWasInCall, expiresAtMillis)

        fun token(): String = token

        fun callerId(): UUID = callerId

        fun callerName(): String = callerName

        fun targetId(): UUID = targetId

        fun targetName(): String = targetName

        fun callerSession(): Any? = callerSession

        fun targetSession(): Any? = targetSession

        fun call(): CallCredentials = call

        fun callerWasInCall(): Boolean = callerWasInCall
    ...
    @Synchronized
    fun add(invite: Invite, nowMillis: Long): Boolean {
    ...
    @Synchronized
    fun take(token: String, targetId: UUID, nowMillis: Long): Optional<Invite> = take(token, targetId, null, nowMillis)

    @Synchronized
    fun take(token: String, targetId: UUID, targetSession: Any?, nowMillis: Long): Optional<Invite> {
        val invite = byToken[token] ?: return Optional.empty()
        if (
            invite.targetId() != targetId ||
                (invite.targetSession() != null && invite.targetSession() !== targetSession)
        )
    ...
    @Synchronized
    fun expire(token: String, nowMillis: Long): Optional<Invite> {
        val invite = byToken[token] ?: return Optional.empty()
        if (invite.expiresAtMillis() > nowMillis) return Optional.empty()
        remove(invite)
        return Optional.of(invite)
    }

    @Synchronized
    fun remove(token: String, targetId: UUID): Optional<Invite> {
        val invite = byToken[token] ?: return Optional.empty()
        if (invite.targetId() != targetId) return Optional.empty()
        remove(invite)
        return Optional.of(invite)
    }

    @Synchronized
    fun isPending(token: String, targetId: UUID, nowMillis: Long): Boolean {
        val invite = byToken[token] ?: return false
        return invite.targetId() == targetId && invite.expiresAtMillis() > nowMillis
    }

    @Synchronized
    fun hasOutgoingCapacity(callerId: UUID, maximum: Int, nowMillis: Long): Boolean =
        byToken.values.count { it.callerId() == callerId && it.expiresAtMillis() > nowMillis } < maximum

    @Synchronized
    fun removeSession(playerId: UUID, session: Any?): Collection<Invite> {
    ...
    @Synchronized
    fun provisionalFor(callerId: UUID, nowMillis: Long, factory: Supplier<CallCredentials>): CallCredentials =
    ...
    fun provisionalFor(
        ...
        callerSession: Any?,
        nowMillis: Long,
        factory: Supplier<CallCredentials>,
    ): CallCredentials {
        val existing = provisionalByCaller[callerId]
        if (
            existing != null && existing.session === callerSession && existing.credentials.expiresAtMillis() > nowMillis
        ) {
    ...
    @Synchronized
    fun activate(callerId: UUID, groupId: UUID) {
        ...
        for (entry in byToken.entries) {
            val invite = entry.value
            if (invite.callerId() == callerId && invite.call().groupId() == groupId && !invite.callerWasInCall()) {
                entry.setValue(invite.copy(callerWasInCall = true))
            }
        }
    }
