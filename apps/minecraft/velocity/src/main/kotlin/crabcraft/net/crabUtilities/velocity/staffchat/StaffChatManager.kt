...
open class StaffChatManager(
    private val plugin: CrabUtilitiesVelocity,
    private val redis: RedisStaffChat,
    private val discordWebhook: DiscordWebhook?,
    private val discordAvatarUrlTemplate: String?,
) {
    ...
    private fun sendToDiscord(senderName: String, senderUuid: UUID?, message: Component) {
        ...
        val avatarUrl =
            if (senderUuid != null && !discordAvatarUrlTemplate.isNullOrEmpty()) {
                discordAvatarUrlTemplate.replace("{uuid}", senderUuid.toString())
            } else null
    ...
    open fun displayMessage(senderName: String, message: Component) {
        ...
        val component =
            MINI_MESSAGE.deserialize(
                plugin.getConfig().getStaffChatFormat(),
                Placeholder.component("sender", senderComponent),
                Placeholder.component("message", message),
            )
        for (player in plugin.getServer().allPlayers) {
            if (player.hasPermission(PERMISSION) && isEnabled(player.uniqueId)) {
                plugin.getMessageManager()!!.deliver(player, component)
            }
        }
        val plain = PlainTextComponentSerializer.plainText()
        plugin.getLogger().info("[StaffChat] {}: {}", plain.serialize(senderComponent), plain.serialize(message))
    }
