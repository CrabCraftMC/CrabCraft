...
object AdvancementLeaderboardRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        check(
            dataSource.sql.all { it.contains("= any (?)") },
            "a leaderboard query does not filter against the advancement registry",
        )
        check(
            dataSource.sql.all { it.contains("is_discord_member") },
            "a leaderboard query does not exclude departed Discord members",
        )
        check(
            dataSource.sql.all { it.contains("last_mc_login_at") },
            "a leaderboard query does not exclude inactive players",
        )
        check(
            dataSource.sql.all { it.contains("2592000") },
            "a leaderboard query does not use the 30-day window",
        )
        check(
            dataSource.sql.all { it.contains("(? or eligible_player.awards_excluded = false)") },
        ...
        check(
            !result.toString().contains(dataSource.hiddenIdentity),
            "hidden identity appears elsewhere in the response",
        )
    }
