...
object AwardAltExclusionRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        check(
            !visibleLeader.get("hidden").asBoolean && visibleLeader.get("uuid").asString == dataSource.visibleUuid,
            "default awards overview lost its visible leader",
        )
        val rankingQueries = dataSource.sql.filter { it.contains("player_award_scores") }
        check(rankingQueries.size == 7, "expected every public award query to be exercised")
        check(rankingQueries.all { it.contains("player_alts") }, "a public award query does not exclude alt accounts")
        check(
            rankingQueries.all { it.contains("is_discord_member") },
            "a public award query does not exclude departed Discord members",
        )
        check(
            rankingQueries.all { it.contains("eligible_player.awards_excluded = false") },
            "a public award query can expose a moderator-excluded player",
        )
        check(
            rankingQueries.all { it.contains("last_mc_login_at") },
            "a public award query does not exclude inactive players",
        )
        check(rankingQueries.all { it.contains("2592000") }, "a public award query does not use the 30-day window")
        check(
            rankingQueries.count { it.contains("rank() over") } == 5,
            "medal-bearing award queries do not derive ranks after filtering alts",
        )
        check(dataSource.visibility.all { !it }, "a leaderboard includes hidden players by default")
        for (response in
            listOf(
            ...
            val hidden =
                if (response.has("awards"))
                    response.getAsJsonArray("awards").get(0).asJsonObject.getAsJsonObject("leader")
                else response.getAsJsonArray("leaderboard").get(0).asJsonObject
            check(hidden.get("hidden").asBoolean, "hidden entry lost its marker")
            check(
                hidden.get("uuid").isJsonNull && hidden.get("nickname").isJsonNull,
                "hidden entry exposes a UUID or nickname",
            )
            check(hidden.get("username").asString == "Hidden player", "hidden entry exposes its username")
            check(
                !response.toString().contains(dataSource.hiddenUuid) &&
        ...
        check(
            dataSource.sql[1].contains("is_discord_member"),
            "departed Discord members can still consume medal positions",
        )
        check(
            dataSource.sql[1].contains("eligible_player.awards_excluded = false"),
        ...
        check(
            dataSource.sql[2].contains("last_mc_login_at"),
            "reactivating a player does not apply the inactivity cutoff",
        )
        check(
            dataSource.sql[2].contains("eligible_player.awards_excluded = false"),
    ...
    private class CapturingDataSource : HikariDataSource() {
        ...
        private val visibleName = "Fixture_" + UUID.randomUUID()
        ...
        private fun preparedStatement(rawSql: String): PreparedStatement {
            val normalised = rawSql.lowercase().replace(Regex("\\s+"), " ").trim()
            sql.add(normalised)
        ...
        private fun resultSet(sql: String, showHidden: Boolean): ResultSet {
            val rows: List<Map<String, Any>> =
                when {
                    sql.contains("select distinct on (scores.award_id)") ->
                        listOf(
                            mapOf(
                                ...
                                "best_score" to Random().nextDouble(1000.0, 2000.0),
                            )
                        )
                    showHidden && (sql.contains("case when ranked.rnk <= 3") || sql.contains("crowns.gold,")) ->
                        listOf(
                            mapOf(
                                "minecraft_uuid" to hiddenUuid,
                                "minecraft_username" to hiddenName,
                                "nickname" to "Alias_" + hiddenName,
                                "hidden" to true,
                                "score" to Random().nextDouble(1000.0, 2000.0),
                                "rnk" to 1,
                                "medal" to 1,
                                "gold" to 2,
                                "silver" to 1,
                    ...
                    sql.contains("from awards where id = ?") || sql.contains("from awards where enabled = true") ->
                        listOf(
                            mapOf(
                                "id" to "test-award",
                                "title" to "Test Award",
                    ...
                    sql.contains("select award_id, score, rank from") ->
                        listOf(mapOf("award_id" to "test-award", "score" to 1.0, "rank" to 1))
                    else -> emptyList()
                }
            var index = -1
