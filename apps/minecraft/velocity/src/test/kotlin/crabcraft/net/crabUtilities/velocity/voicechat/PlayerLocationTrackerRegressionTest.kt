...
object PlayerLocationTrackerRegressionTest {
    ...
    private fun committedHopPublishesWithoutHeartbeat() {
        val listener =
            PlayerLocationTracker::class.java.getMethod("onServerConnected", ServerPostConnectEvent::class.java)
        ...
        val connection =
            proxy(ServerConnection::class.java) { _, method, _ ->
                if (method.name == "getServerInfo") ServerInfo(backend.get(), InetSocketAddress("127.0.0.1", 25565))
                else null
            }
        val player =
            proxy(Player::class.java) { _, method, _ ->
                when (method.name) {
        ...
        val server =
            proxy(ProxyServer::class.java) { _, method, _ ->
        ...
        try {
            ...
            Files.walk(directory).use { files ->
                for (path in files.sorted(Comparator.reverseOrder()).toList()) Files.delete(path)
            }
        }
    }
