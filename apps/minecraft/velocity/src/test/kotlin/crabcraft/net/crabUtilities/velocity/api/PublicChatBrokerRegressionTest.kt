...
object PublicChatBrokerRegressionTest {
    ...
    private fun mapsRedisStreamEntriesToJsonAndSse() {
        ...
        check("1725000000123-4" == event.id(), "stream ID was not preserved")
        check(event.timestamp() == 1_725_000_000_123L, "stream timestamp was not exposed as Unix epoch milliseconds")
        val json = JsonParser.parseString(event.toJson()).asJsonObject
        check(json.size() == 4 && !json.has("id"), "JSON payload did not match the public chat contract")
        check(json.get("timestamp").asLong == 1_725_000_000_123L, "JSON timestamp was not numeric")
        check("hello\nworld" == json.get("message").asString, "JSON message changed")
    ...
    private fun replaysRecentEventsAndResumesAfterAnId() {
        ...
        for (i in 1..8) feed.publish(event(i))
        feed.subscribe(null, 6).use { initial ->
            for (expected in 3..8) check(
                eventId(expected) == requireEvent(initial).id(),
                "default replay was not the newest six events",
            )
        }
        feed.subscribe(eventId(5), 6).use { resumed ->
            for (expected in 6..8) check(
                eventId(expected) == requireEvent(resumed).id(),
                "Last-Event-ID replay did not resume after the supplied ID",
            )
            check(
                resumed.poll(1L, TimeUnit.MILLISECONDS) == null,
    ...
    private fun fansOutLiveEventsAndKeepsTheNewestWhenAClientIsSlow() {
        ...
        feed.subscribe(null, 0).use { first ->
            feed.subscribe(null, 0).use { second ->
                ...
                check(eventId(3) == requireEvent(first).id(), "slow subscriber queue did not discard its oldest event")
                check(eventId(4) == requireEvent(first).id(), "slow subscriber queue did not retain the newest event")
            }
        }
        feed.close()
    }
    ...
    private fun closesAndUnregistersSubscriptions() {
        ...
        check(duringShutdown.poll(1L, TimeUnit.MILLISECONDS) == null, "closed subscription returned an event")
    }
    ...
    private fun eventId(sequence: Int) = "${1_000L + sequence}-0"
