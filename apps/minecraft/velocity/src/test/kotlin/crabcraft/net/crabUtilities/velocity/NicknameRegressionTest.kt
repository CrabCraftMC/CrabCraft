...
object NicknameRegressionTest {
    ...
    private fun parsesSupportedNicknameFormatsWithoutInteractiveTags() {
        ...
        val interactive = NicknameComponentParser.parse("<click:run_command:'/op @s'><red>Crab</red></click>")
    ...
    private fun distinguishesUnknownFromLoadedEmptyState() {
        ...
        check(cache.commitIfVersion(id, unknown.version(), ""), "empty Redis tombstone was not committed")
        ...
        check(cache.snapshot(id).rawNickname() == "", "empty tombstone was not preserved")
    }
    ...
    private fun rejectsStaleCacheGenerations() {
        ...
        cache.remove(id)
        check(
            !cache.commitIfVersion(id, unknown.version(), "<red>Old session</red>"),
        ...
        check(
            cache.commitIfVersion(id, currentUnknown.version(), "<green>New</green>"),
            "current session could not commit after invalidation",
        )
        check(
            !cache.commitIfVersion(id, unknown.version(), "<red>Old</red>"),
            "stale seed replaced a newer Redis update",
        )
        check(cache.getRawNickname(id) == "<green>New</green>", "newer nickname was not preserved")
        val disconnectedVersion = cache.snapshot(id).version()
        cache.remove(id)
        cache.setNickname(id, "<blue>Reconnected</blue>")
        check(!cache.isVersion(id, disconnectedVersion), "cache generation was reused after reconnect")
        val cleanupCache = NicknameCache()
        val cleanupId = UUID.randomUUID()
        val abandoned = cleanupCache.beginLoad(cleanupId)
        check(
            cleanupCache.discardIfUnloadedVersion(cleanupId, abandoned.version()),
            "abandoned unloaded generation was not discarded",
        )
        check(cleanupCache.snapshot(cleanupId).version() == 0L, "read-only snapshot retained an abandoned UUID")
        val newerLoad = cleanupCache.beginLoad(cleanupId)
        cleanupCache.setNickname(cleanupId, "<aqua>Live</aqua>")
        check(
            !cleanupCache.discardIfUnloadedVersion(cleanupId, newerLoad.version()),
            "cleanup discarded a newer live nickname",
        )
        check(
            cleanupCache.getRawNickname(cleanupId) == "<aqua>Live</aqua>",
    ...
    private fun doesNotCommitFailedDatabaseLoads() {
        ...
        val id = UUID.randomUUID()
        check(
            !ConnectionListener.commitNicknameLoad(
                cache,
                id,
                cache.beginLoad(id).version(),
        ...
        check(
            cache.isLoaded(id) && cache.getRawNickname(id) == null,
    ...
    private fun reconcilesAtomicPublicationResult() {
        ...
        check(cache.commitIfVersion(id, loading.version(), "<red>Database</red>"), "database seed setup did not commit")
        val proposed = cache.snapshot(id)
        check(
            NicknameListener.reconcilePublishedNickname(
                cache,
                id,
                proposed.version(),
                proposed.rawNickname()!!,
                "<green>Live</green>",
            ),
            "existing Redis nickname did not supersede the database seed",
        )
        check(cache.getRawNickname(id) == "<green>Live</green>", "actual Redis nickname was not adopted locally")
        ...
        check(cache.getRawNickname(id) == "<aqua>Newer</aqua>", "newer local nickname was not preserved")
        val newer = cache.snapshot(id)
        check(
            NicknameListener.reconcilePublishedNickname(cache, id, newer.version(), newer.rawNickname()!!, ""),
            "existing Redis clear tombstone was not reconciled",
        )
        check(
            cache.isLoaded(id) && cache.getRawNickname(id) == null && cache.snapshot(id).rawNickname() == "",
            "Redis clear tombstone was not preserved locally",
        )
    }

    private fun checkPlain(raw: String, expected: String) {
        check(expected == NicknameComponentParser.plain(raw), "nickname format did not parse: $raw")
    }

    private fun hasClickEvent(component: Component): Boolean =
        component.clickEvent() != null || component.children().any(::hasClickEvent)
