...
object CallManagerRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        joinAndAcceptMessagesContainOnlyRoutingTokens()
    ...
    private fun targetBoundOneUseInvites() {
        ...
        check(
            registry.take(invite.token(), stranger, 100L).isEmpty(),
            "another player consumed the invitation",
        )
        check(
            registry.take(invite.token(), target, 100L).orElseThrow() === invite,
            "the intended target could not accept",
        )
        check(
            registry.take(invite.token(), target, 100L).isEmpty(),
        ...
        check(
            registry.remove(removable.token(), stranger).isEmpty(),
            "another player removed the invitation during terminal cleanup",
        )
        check(
            registry.remove(removable.token(), target).orElseThrow() === removable,
            "terminal cleanup could not remove the intended invitation",
        )
        check(
            registry.remove(removable.token(), target).isEmpty(),
    ...
    private fun invitationsAreBoundToProxySessions() {
        ...
        check(
            registry.take(invite.token(), targetId, quickRelog, 100L).isEmpty(),
            "a quick relog adopted an earlier session's invitation",
        )
        check(
            registry.take(invite.token(), targetId, targetSession, 100L).orElseThrow() === invite,
        ...
        check(
            registry.add(oldInvite, 0L) && registry.add(newInvite, 0L),
            "relogin cleanup invitations were rejected",
        )
        check(
            registry.removeSession(callerId, callerSession).contains(oldInvite),
        ...
        check(
            registry
                .take(
                    newInvite.token(),
                    newTarget,
                    newInvite.targetSession(),
                    100L,
                )
                .orElseThrow() === newInvite,
    ...
    private fun expiringInvites() {
        ...
        check(
            registry.take(invite.token(), target, 500L).isEmpty(),
    ...
    private fun provisionalCallsAreReused() {
        ...
        check(
            registry.take(pending.token(), target, 300L).orElseThrow().callerWasInCall(),
        ...
        check(
            beforeRelog !== afterRelog,
            "a quick relog inherited the old session's provisional password",
        )
    }

    private fun joinAndAcceptMessagesContainOnlyRoutingTokens() {
        val token = "click-token"
        val groupId = UUID.randomUUID()
        val playerId = UUID.randomUUID()
        val generation = "AbCdEfGhIjKlMnOpQrStUvWx"

        val wire = CallManager.encodeCallJoin(groupId, playerId, generation).split(CallManager.SEPARATOR)
        ...
        check(
            PlayerLocationTracker.playerHomeKey(playerId).equals("crabcraft:svc:player-home:" + playerId),
            "route guard key changed",
        )

        val target = CallManager.callTargetValue(groupId, generation)
        check(
            target.equals(groupId.toString() + CallManager.SEPARATOR + generation),
    ...
    private fun callGroupsUseTheSharedRegistryEncoding() {
        ...
        val call =
            CallInviteRegistry.CallCredentials(
                groupId,
                "server_only_password_1234567890",
                Long.MAX_VALUE,
            )
        ...
        check(
            CallManager.isCallGroupDefinition(definition),
            "call definition was not recognised as a call",
        )

        val ordinary =
            CallManager.GroupDefinition(
                groupId,
                "Secret Club",
                call.password(),
                "OPEN",
        ...
        check(
            !CallManager.isCallGroupDefinition(ordinary),
            "an ordinary passworded group was co-opted as a call",
        )
        check(
            CallManager.CONTROL_CHANNEL.equals("crabcraft:svc:lifecycle"),
    ...
    private fun outgoingInvitesCanBeBounded() {
        ...
        check(
            !registry.hasOutgoingCapacity(caller, 5, 100L),
            "outgoing invitation cap was not enforced",
        )
        check(
            registry.hasOutgoingCapacity(caller, 5, 1_000L),
    ...
    private fun ringtoneMessagesAreStrictAndTokenBound() {
        ...
        val callerStart =
            CallManager.encodeCallRingStart(
                token,
                callerId,
        ...
        val callerStop =
            CallManager.encodeCallRingStop(
                token,
                callerId,
                CallManager.RingDirection.OUTGOING,
            )
        ...
        check(
            CallManager.OP_CALL_RING_START.equals(callerStartWire[0]),
            "CALL_RING_START opcode changed",
        )
        check(token.equals(callerStartWire[1]), "CALL_RING_START token changed")
        check(
            callerId.toString().equals(callerStartWire[2]),
            "CALL_RING_START player changed",
        )
        check(
            "OUTGOING".equals(callerStartWire[3]),
            "caller did not receive the outgoing ringtone",
        )
        check(
            expiresAtMillis.toString().equals(callerStartWire[4]),
        ...
        check(
            targetId.toString().equals(targetStartWire[2]),
            "incoming CALL_RING_START player changed",
        )
        check(
            "INCOMING".equals(targetStartWire[3]),
            "target did not receive the incoming ringtone",
        )
        check(
            targetStartWire[4].equals(callerStartWire[4]),
        ...
        check(
            callerStopWire.size == 4 && targetStopWire.size == 4,
            "CALL_RING_STOP field count changed",
        )
        check(
            CallManager.OP_CALL_RING_STOP.equals(callerStopWire[0]),
            "CALL_RING_STOP opcode changed",
        )
        check(
            token.equals(callerStopWire[1]) && token.equals(targetStopWire[1]),
            "CALL_RING_STOP was not bound to its invitation token",
        )
        check(
            "OUTGOING".equals(callerStopWire[3]) && "INCOMING".equals(targetStopWire[3]),
        ...
        check(
            retryArguments.size == 4,
            "CALL_RING_STOP retry argument count changed",
        )
        check(
            CallManager.CONTROL_CHANNEL.equals(retryArguments.get(0)),
            "CALL_RING_STOP retry channel changed",
        )
        check(
            callerStop.equals(retryArguments.get(1)) && targetStop.equals(retryArguments.get(2)),
            "CALL_RING_STOP retry was not token-bound to both players",
        )
        check(
            expiresAtMillis.toString().equals(retryArguments.get(3)),
            "CALL_RING_STOP retry did not retain the original deadline",
        )
        check(
            retryArguments.none { it.contains(password) },
            "call password leaked into CALL_RING_STOP retry arguments",
        )
        check(
            CallManager.shouldRetryRingtoneStop(expiresAtMillis, expiresAtMillis - 1L),
            "CALL_RING_STOP retry ended before the invitation deadline",
        )
        check(
            !CallManager.shouldRetryRingtoneStop(expiresAtMillis, expiresAtMillis),
        ...
        check(
            finalRetryDelay > 0L && 29_000L + finalRetryDelay < 30_000L,
            "final CALL_RING_STOP retry was not bounded by the original deadline",
        )
        check(
            CallManager.ringtoneStopRetryDelayMillis(30_000L, 30_000L) == 0L,
    ...
    private fun expectIllegalArgument(operation: () -> Unit) {
        try {
            operation()
