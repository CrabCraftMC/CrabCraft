...
object PlayerProgressTest {
    private val EXPECTED_IDS = listOf(
        "hang_four_by_four_painting", "outline_hanging_sign", "fill_campfire_four_slots",
        "shoot_button_with_arrow", "fish_treasure_and_junk", "carpet_llama", "enchant_five_items",
        "throw_mending_book_in_lava", "stun_ravager", "fully_power_conduit", "poison_bee",
        "place_fish_in_nether", "named_ghast_overworld", "fill_ender_chest", "apply_armour_trim",
    ...
    private fun cardSixIdsAreUniqueAndInOrder() {
        val tasks = BingoTask.cardSix()
        val actualIds = tasks.map { it.id() }
        check(actualIds.size == EXPECTED_IDS.size, "Bingo #6 task count changed: ${actualIds.size}")
        for (index in EXPECTED_IDS.indices) {
            check(actualIds[index] == EXPECTED_IDS[index],
                "Bingo #6 task ${index + 1} changed: actual=${actualIds[index]}, expected=${EXPECTED_IDS[index]}")
        }
        check(tasks.toSet().size == 16, "Bingo #6 tasks must be unique")
    ...
    private fun constructorRequiresExactlySixteenUniqueTasks() {
        expectIllegalArgument({ PlayerProgress(BingoTask.cardSix().subList(0, 15)) }, "A 15-task test card was accepted")
        expectIllegalArgument({ PlayerProgress(BingoTask.allDeployed()) }, "The deployed catalogue was accepted as one test card")
        val duplicated = ArrayList(BingoTask.cardSix())
        duplicated[duplicated.lastIndex] = duplicated[0]
        expectIllegalArgument({ PlayerProgress(duplicated) }, "A Card #6 task list containing a duplicate was accepted")
    }

    private fun constructorDefensivelyCopiesAllowedTasks() {
        val mutableTasks = ArrayList(BingoTask.cardSix())
    ...
    private fun earlierCardCompletionIsRejected() {
        val progress = PlayerProgress(BingoTask.cardSix())
        for (earlierTask in listOf(BingoTask.cardOne()[0], BingoTask.cardTwo()[0], BingoTask.cardThree()[0],
            BingoTask.cardFour()[0], BingoTask.cardFive()[0])) {
    ...
    private fun checklistCompletesOnlyAfterAllCardSixTasks() {
        ...
        for (index in 0 until tasks.size - 1) check(progress.complete(tasks[index]), "Card #6 task was rejected")
    ...
    private fun completedTasksViewIsImmutable() {
        ...
        try {
            completed.add(BingoTask.cardSix()[1])
    ...
    private fun expectIllegalArgument(action: () -> Unit, message: String) {
        try {
            action()
