...
/** Mutable, memory-only progress for one Card #2 test player. */
public final class PlayerProgress {
    ...
    public PlayerProgress(List<BingoTask> allowedTasks) {
        ...
        if (allowedTasks.size() != 16 || this.allowedTasks.size() != 16) {
            throw new IllegalArgumentException("Card #2 progress requires 16 unique tasks");
        }
    }

    /** Returns true only for the first accepted completion of a Card #2 task. */
