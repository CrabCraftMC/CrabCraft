...
internal class BingoTestManager(
    ...
    fun sendChecklist(player: Player) {
        ...
        player.sendMessage(
            CrabMessages.text("Bingo #$cardNumber detector checklist")
                .append(Component.space())
                .append(CrabMessages.highlight("${progress.completedCount()}/${tasks.size}"))
        )
        tasks.forEachIndexed { index, task ->
            val status =
                if (progress.isComplete(task)) CrabMessages.success("Complete: ${task.description()}")
                else CrabMessages.text("Incomplete: ${task.description()}")
            player.sendMessage(CrabMessages.muted("${index + 1}. ").append(status))
        }
        player.sendMessage(
            CrabMessages.muted(
        ...
        player.sendMessage(
            CrabMessages.muted("Progress is memory-only and resets when this plugin or the server restarts.")
        )
    }

    fun sendDetails(player: Player, number: Int) {
        if (number !in 1..tasks.size) {
        ...
        player.sendMessage(
            if (complete) CrabMessages.success("Status: complete") else CrabMessages.warning("Status: incomplete")
        )
    ...
    private fun announceCompletion(player: Player, task: BingoTask, progress: PlayerProgress) {
        player.sendMessage(
            CrabMessages.muted("Completed").append(Component.space()).append(CrabMessages.highlight(task.description()))
        )
        plugin.logger.info("${player.name} completed Card #$cardNumber detector ${task.id()}")
        if (progress.isChecklistComplete()) {
            player.sendMessage(
                CrabMessages.success("All 16 Bingo #$cardNumber detectors have passed!").decorate(TextDecoration.BOLD)
            )
        }
    }
}
