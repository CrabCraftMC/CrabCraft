...
class PlayerProgress(allowedTasks: List<BingoTask>) {
    ...
    init {
        require(allowedTasks.size == 16 && this.allowedTasks.size == 16) { "Bingo progress requires 16 unique tasks" }
    }
    ...
    fun completedTasks(): Set<BingoTask> = Collections.unmodifiableSet(EnumSet.copyOf(completedTasks))
