...
final class SignMarkerListener implements Listener {
    ...
    public void onSignChangeRecolor(SignChangeEvent event) {
        ...
        if (event.getPlayer().hasPermission(PERMISSION)) {
            event.line(0, CrabMessages.accent(keyword));
        }
    }
    ...
    public void onSignChange(SignChangeEvent event) {
        ...
        if (!isKeyword(event.line(0))) {
            ...
            if (service.removeMarker(block)) {
                player.sendMessage(CrabMessages.success("Map marker removed."));
            }
            return;
        }
        ...
        if (!player.hasPermission(PERMISSION)) {
            player.sendMessage(CrabMessages.error(
                    "You don't have permission to create map markers."));
            return;
        }
        ...
        if (service.addMarker(block, markerLabel)) {
            player.sendMessage(CrabMessages.success(
                    "Map marker \"" + markerLabel + "\" added to BlueMap."));
        } else {
            player.sendMessage(CrabMessages.warning(
                    "Map marker \"" + markerLabel
                            + "\" saved — it will show once BlueMap has a map for this world."));
        }
    }
