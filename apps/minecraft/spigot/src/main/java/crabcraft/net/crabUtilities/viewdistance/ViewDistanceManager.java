...
public final class ViewDistanceManager implements Listener {
    ...
    private final AdjustmentSwitch adjustmentSwitch = new AdjustmentSwitch();
    private final ClientViewRadiusBridge clientViewRadiusBridge;
    private BukkitTask initialClampTask;
    ...
    public ViewDistanceManager(CrabUtilities plugin) {
        ...
        this.clientViewRadiusBridge = new ClientViewRadiusBridge(
                plugin,
                Math.max(simulationBounds.maximum(), viewBounds.maximum()));
    }
    ...
    public boolean isPaused() {
        return adjustmentSwitch.isPaused();
    }

    public boolean pause() {
        if (!adjustmentSwitch.pause()) {
            return false;
        }
    ...
    public void setManualSimulationDistance(World world, int distance) {
        requireInBounds("simulation", distance, simulationBounds);
        setSimulationDistance(world, distance);
    }

    public void setManualViewDistance(World world, int distance) {
    ...
    public void start() {
        ...
        initialClampTask = Bukkit.getScheduler().runTask(plugin, () -> {
    ...
    public void shutdown(boolean restoreActualRadius) {
        if (initialClampTask != null) {
    ...
    static void requireInBounds(String name, int distance, Bounds bounds) {
        if (!bounds.contains(distance)) {
            throw new IllegalArgumentException(name + " distance must be between "
    ...
    record Bounds(int minimum, int maximum) {
        ...
        boolean contains(int distance) {
            return distance >= minimum && distance <= maximum;
        }
    }

    static final class AdjustmentSwitch {
