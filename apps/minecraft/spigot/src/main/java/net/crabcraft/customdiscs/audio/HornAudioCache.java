...
public final class HornAudioCache {
  ...
  public HornAudioCache(File dir) {
    ...
    if (!dir.exists() && !dir.mkdirs()) {
      CustomDiscs.warn("Could not create horn cache directory {}", dir.getAbsolutePath());
    }
  }
  ...
  public List<short[]> read(String key) {
    ...
    try {
      ...
      if (!f.setLastModified(System.currentTimeMillis())) { // touch -> least-recently-played eviction
        CustomDiscs.debug("Could not touch horn cache {} for LRU ordering", f.getName());
      }
      return frames;
    } catch (IOException e) {
      CustomDiscs.warn("Failed reading horn cache {}: {}", f.getName(), e.getMessage());
      return null;
    }
  }
  ...
  public void write(String key, List<short[]> frames) {
    ...
    try (OutputStream out = new BufferedOutputStream(new java.io.FileOutputStream(tmp))) {
      ...
      CustomDiscs.warn("Failed writing horn cache {}: {}", f.getName(), e.getMessage());
      tmp.delete();
    }
  }
  ...
  private void evict() {
    ...
    for (int i = 0; i < files.length - maxFiles; i++) {
      if (!files[i].delete()) CustomDiscs.debug("Could not evict horn cache {}", files[i].getName());
    }
  }
