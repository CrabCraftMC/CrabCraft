...
import net.crabcraft.customdiscs.api.event.AudioStartPlayingEvent;
import net.crabcraft.customdiscs.api.event.AudioStopPlayingEvent;
...
public class AudioEngine implements AudioManager {
  ...
  private boolean submitAudioWork(Runnable task, String description) {
    ...
    CustomDiscs.warn("Audio work queue full; rejected {}", description);
    return false;
  }
  ...
  public OpenedStream openStream(String identifier, float volume, boolean forceRefresh) throws IOException {
    ...
    FfmpegPcmStream pcm = new FfmpegPcmStream(currentBinaries.getFfmpegPath(), track.streamUrl(),
      volume, plugin.getCDConfig().getYtDlpProxy());
  ...
  public void play(@NotNull Block block, @NotNull String identifier, float volume, int discDistance) {
    ...
    try {
      ...
      if (!isReady()) {
        CustomDiscs.warn("Cannot play {}: yt-dlp/ffmpeg unavailable", identifier);
        return;
      }
      ...
      int distance = discDistance > 0 ? discDistance : plugin.getCDConfig().getDiscRangeDefault();
      ...
      if (!sessionLimiter.tryAcquire()) {
        messagePlayers(playersAtStart, plugin.getLanguage().PComponent("error.play.busy"));
        CustomDiscs.warn("Custom audio session limit ({}) reached; rejected disc at {}",
      ...
      try {
        AudioStartPlayingEvent event = new AudioStartPlayingEvent(block, identifier);
        ...
        channel.setCategory(CDVoiceAddon.MUSIC_DISC_CATEGORY);
        ...
        plugin.getFoliaLib().getScheduler().runAtLocationTimer(block.getLocation(), task -> {
          if (!sessions.containsKey(uuid)) { task.cancel(); return; }
  ...
  public void stopPlaying(@NotNull Block block) {
    stop(LegacyUtil.getBlockUUID(block));
  }
  ...
  public void playHorn(@NotNull Player player, @NotNull String identifier, float volume) {
    if (!isReady()) {
      CustomDiscs.warn("Cannot play horn {}: yt-dlp/ffmpeg unavailable", identifier);
      return;
    }
    ...
    if (!sessionLimiter.tryAcquire()) {
      CustomDiscs.sendMessage(player, plugin.getLanguage().PComponent("error.play.busy"));
      CustomDiscs.warn("Custom audio session limit ({}) reached; rejected horn for {}",
    ...
    try {
      ...
      if (channel == null) return;
      channel.setCategory(CDVoiceAddon.GOAT_HORN_CATEGORY);
      channel.setDistance(plugin.getCDConfig().getHornRange()); // fixed range for every horn

      HornSession session = new HornSession(playerId, identifier, channel,
        plugin.getCDConfig().getHornVolume() * volume, plugin.getCDConfig().getHornMaxLengthSeconds());
      ...
      try {
        ...
        if (cache != null && track != null && !cache.has(key)) {
          cache.write(key, decodeFrames(track.streamUrl(), effectiveVolume,
            plugin.getCDConfig().getHornMaxLengthSeconds()));
        }
      } catch (Throwable t) {
        CustomDiscs.warn("Horn pre-warm failed for {}: {}", identifier, t.getMessage());
      } finally {
  ...
  private HornAudioCache hornCacheIfEnabled() {
    ...
    if (cache == null || !plugin.getCDConfig().isHornCacheEnabled()) return null;
    cache.setMaxFiles(plugin.getCDConfig().getHornCacheSize());
    return cache;
  }
  ...
  private List<short[]> decodeFrames(String streamUrl, float volume, int maxSeconds) throws java.io.IOException {
    FfmpegPcmStream pcm = new FfmpegPcmStream(binaries.getFfmpegPath(), streamUrl, volume,
      plugin.getCDConfig().getYtDlpProxy(), maxSeconds);
    try {
  ...
  private void fillHornCacheAsync(String identifier, float effectiveVolume) {
    ...
    Runnable work = () -> {
      try {
        ...
        CustomDiscs.warn("Horn cache fill failed for {}: {}", identifier, t.getMessage());
      } finally {
  ...
  public boolean isPlaying(@NotNull Block block) {
    return sessions.containsKey(LegacyUtil.getBlockUUID(block));
  }

  @Override
  public @Nullable LocationalAudioChannel getAudioChannel(@NotNull Block block) {
    Session s = sessions.get(LegacyUtil.getBlockUUID(block));
  ...
  public @Nullable Collection<ServerPlayer> getPlayersInRangeAtStart(@NotNull Block block) {
    Session s = sessions.get(LegacyUtil.getBlockUUID(block));
    return s == null ? null : s.playersAtStart;
  }
  ...
  private static void messagePlayers(Collection<ServerPlayer> players, Component message) {
    for (ServerPlayer serverPlayer : players) {
      if (serverPlayer.getPlayer() instanceof Player bukkitPlayer) {
        CustomDiscs.sendMessage(bukkitPlayer, message);
      }
    }
  }
  ...
  private class Session {
    ...
    void startAudio() {
      ...
      try {
        ...
        if (track == null || stopped) {
          if (!stopped) messageInRange(plugin.getLanguage().PComponent("error.play.no-matches"));
        ...
        this.stream = new FfmpegPcmStream(binaries.getFfmpegPath(), track.streamUrl(),
          plugin.getCDConfig().getMusicDiscVolume() * volume, plugin.getCDConfig().getYtDlpProxy());
        ...
        CustomDiscs.error("Audio session error for {}: ", e, identifier);
        if (!stopped) messageInRange(plugin.getLanguage().PComponent("error.play.while-playing"));
        AudioEngine.this.stop(uuid);
      }
    }

    private void startNowPlayingLoop(String title) {
      Component nowPlaying = plugin.getLanguage().component("now-playing", title);
    ...
    void rejectBusy() {
      ...
      messageInRange(plugin.getLanguage().PComponent("error.play.busy"));
    }
    ...
    void stop() {
      ...
      // Fire the (synchronous) stop event + reset the jukebox on the region thread, since
      // stop() can be invoked from an async thread.
      plugin.getFoliaLib().getScheduler().runAtLocationLater(block.getLocation(), task -> {
        AudioStopPlayingEvent event = new AudioStopPlayingEvent(block, identifier);
        plugin.getServer().getPluginManager().callEvent(event);
        if (block.getState() instanceof Jukebox jukebox) jukebox.stopPlaying();
      }, 1);
    }
  }
  ...
  private class HornSession {
    ...
    void startAudio() {
      ...
      try {
        ...
        TrackResolver.ResolvedTrack track = resolver.resolve(identifier);
        if (track == null || stopped) {
          if (!stopped) messagePlayer(plugin.getLanguage().PComponent("error.play.no-matches"));
        ...
        this.stream = new FfmpegPcmStream(binaries.getFfmpegPath(), track.streamUrl(),
          volume, plugin.getCDConfig().getYtDlpProxy(), maxSeconds);
        this.encoder = api.createEncoder(OpusEncoderMode.AUDIO);
        this.player = api.createAudioPlayer(channel, encoder, stream.frames());
        if (stopped) {
          try { player.stopPlaying(); } catch (Exception ignored) {}
        ...
        CustomDiscs.error("Horn audio session error for {}: ", e, identifier);
        if (!stopped) messagePlayer(plugin.getLanguage().PComponent("error.play.while-playing"));
        AudioEngine.this.stopHorn(playerId);
      }
    }
    ...
    private void messagePlayer(Component message) {
      ...
      if (bukkitPlayer != null) CustomDiscs.sendMessage(bukkitPlayer, message);
    }

    void rejectBusy() {
      if (!markStopped()) return;
      messagePlayer(plugin.getLanguage().PComponent("error.play.busy"));
    }
