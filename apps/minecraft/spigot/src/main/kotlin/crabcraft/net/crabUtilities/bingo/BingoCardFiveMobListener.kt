...
class BingoCardFiveMobListener(
    ...
    fun onCakeDropped(event: PlayerDropItemEvent) {
        ...
        val player = event.player
        if (
            !isEligibleDroppedPandaCake(item.type, Tag.ITEMS_PANDA_EATS_FROM_GROUND.isTagged(item.type)) ||
    ...
    fun onItemPickedUp(event: EntityPickupItemEvent) {
        ...
        val panda = event.entity as? Panda
        if (cake != null && panda != null) confirmPandaAcceptedCake(panda, cake)
        val dropped =
            droppedHelmets.remove(itemId)
        ...
        val enemy = event.entity as? Enemy ?: return
        val previousHelmet = enemy.equipment!!.getItem(EquipmentSlot.HEAD).clone()
        val enemyId = enemy.uniqueId
        Bukkit.getScheduler().runTask(plugin, Runnable { confirmHelmetEquipped(enemyId, previousHelmet, dropped) })
    }

    private fun confirmHelmetEquipped(enemyId: UUID, previousHelmet: ItemStack, dropped: DroppedHelmet) {
        val enemy = Bukkit.getEntity(enemyId) as? Enemy ?: return
        val player = Bukkit.getPlayer(dropped.marker.playerId) ?: return
        if (
            !enemy.isValid ||
                enemy.isDead ||
            ...
            return
        val equipped = enemy.equipment!!.getItem(EquipmentSlot.HEAD)
    ...
    private fun beginGolemRepairAttempt(event: PlayerInteractEntityEvent, golem: IronGolem) {
        ...
        val maxHealth = golem.getAttribute(Attribute.MAX_HEALTH) ?: return
        if (
            !canStartGolemRepair(item.type, golem.health, maxHealth.value) ||
        ...
        Bukkit.getScheduler()
            .runTask(plugin, Runnable { confirmGolemRepaired(playerId, golemId, previousHealth, token) })
    }

    private fun confirmGolemRepaired(playerId: UUID, golemId: UUID, previousHealth: Double, token: AttemptToken) {
        val player = Bukkit.getPlayer(playerId) ?: return
        val golem = Bukkit.getEntity(golemId) as? IronGolem ?: return
        if (
            golem.isValid &&
                !golem.isDead &&
                healthIncreased(previousHealth, golem.health) &&
    ...
    fun onHoglinTransformed(event: EntityTransformEvent) {
        val hoglin = event.entity as? Hoglin ?: return
        if (
            event.transformReason != EntityTransformEvent.TransformReason.PIGLIN_ZOMBIFIED ||
                event.transformedEntity !is Zoglin
        )
            return
    ...
    private fun markerIsCurrent(marker: PersistentMarker, now: Long, maximumAge: Long): Boolean =
        markerMatchesCard(marker.cardId, activeCardId.asInt) &&
            isFreshAttribution(marker.timestamp, now, playerResetAtMillis.getOrDefault(marker.playerId, 0L), maximumAge)
    ...
    companion object {
        private const val MAX_DROPPED_ITEM_AGE_MILLIS = 10L * 60 * 1000
        ...
        @JvmStatic
        fun isEligibleDroppedPandaCake(itemType: Material?, pandaGroundFood: Boolean): Boolean =
            itemType == Material.CAKE && pandaGroundFood

        @JvmStatic
        fun canStartGolemRepair(itemType: Material?, currentHealth: Double, maximumHealth: Double): Boolean =
            itemType == Material.IRON_INGOT && currentHealth + HEALTH_EPSILON < maximumHealth

        @JvmStatic
        fun healthIncreased(previousHealth: Double, currentHealth: Double): Boolean =
