...
class ModelCommand private constructor(private val plugin: JavaPlugin, private val nexoItems: NexoItemLookup?) :
    ...
    private enum class DestinationKind {
        MAIN_HAND,
        OFF_HAND,
        STORAGE,
    }

    private data class Destination(val kind: DestinationKind, val slot: Int = -1)

    private data class HeldPair(
        val target: ItemStack,
        val targetInMainHand: Boolean,
        val cosmetic: ItemStack,
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<out String>): Boolean {
        ...
        when (args[0].lowercase(Locale.ROOT)) {
            "merge" -> if (hasPermission(sender, MERGE_PERMISSION)) merge(sender)
            "split" -> if (hasPermission(sender, SPLIT_PERMISSION)) split(sender)
    ...
    private fun split(player: Player) {
        ...
        val merged = if (mainMerged) mainHand else offHand
        if (merged.amount != 1) {
        ...
        val destination = findDestination(inventory, mainMerged, cosmetic)
        if (destination == null) {
        ...
        if (!commitSplit(player, inventory, mainMerged, destination, cosmetic, restored)) {
            player.sendMessage(CrabMessages.error("Split failed; nothing changed."))
            return
        }
        player.sendMessage(
            CrabMessages.success("Restored ")
                .append(targetName)
                .append(CrabMessages.muted(" + "))
                .append(cosmeticName)
                .append(CrabMessages.success("."))
    ...
    private fun inspectHeldPair(player: Player): HeldPair {
        ...
        try {
            ...
            throw inspectionFailed(exception)
        } catch (exception: RuntimeException) {
            throw inspectionFailed(exception)
        }
        ...
        if (!codec.hasApplicableModel(cosmetic))
            throw MergeInputException("That custom item has no usable resource-pack model.")
    ...
    private fun inspectionFailed(exception: Throwable): MergeInputException {
        plugin.logger.warning("Nexo could not inspect held items: ${exception.message}")
        return MergeInputException("Couldn't inspect those custom items. Try again shortly.")
    }

    private fun commitHands(
        player: Player,
        inventory: PlayerInventory,
        newMainHand: ItemStack,
        newOffHand: ItemStack,
        operation: String,
    ): Boolean {
    ...
    private fun commitSplit(
        player: Player,
        inventory: PlayerInventory,
        mergedInMainHand: Boolean,
        ...
        val oldOffHand = inventory.itemInOffHand.clone()
        val oldStorage =
            if (destination.kind == DestinationKind.STORAGE) inventory.getItem(destination.slot)?.clone() else null
        return try {
            ...
            if (destination.kind == DestinationKind.STORAGE) {
                try {
                    ...
                    plugin.logger.log(
                        Level.SEVERE,
                        "Could not roll back model split storage for ${player.uniqueId}",
                        rollbackException,
                    )
                }
            }
    ...
    private fun rollbackHands(
        player: Player,
        inventory: PlayerInventory,
        oldMainHand: ItemStack,
        oldOffHand: ItemStack,
        operation: String,
    ) {
        try {
            ...
            plugin.logger.log(
                Level.SEVERE,
                "Could not roll back main hand after model $operation for ${player.uniqueId}",
                exception,
            )
        }
        try {
            ...
            plugin.logger.log(
                Level.SEVERE,
                "Could not roll back off-hand after model $operation for ${player.uniqueId}",
                exception,
            )
        }
    }
    ...
    private fun sendUsage(player: Player) {
        ...
        if (isNexoAvailable() && player.hasPermission(MERGE_PERMISSION)) {
            player.sendMessage(
                CrabMessages.highlight("/model merge").append(CrabMessages.muted(" — apply held custom model"))
            )
            sent = true
        }
        if (player.hasPermission(SPLIT_PERMISSION)) {
            player.sendMessage(
                CrabMessages.highlight("/model split").append(CrabMessages.muted(" — restore helmet and cosmetic"))
            )
            sent = true
        }
    ...
    override fun onTabComplete(
        ...
        if (isNexoAvailable() && sender.hasPermission(MERGE_PERMISSION) && "merge".startsWith(prefix))
            options.add("merge")
    ...
    fun onPlayerItemBreak(event: PlayerItemBreakEvent) {
        ...
        val cosmetic =
            try {
                ...
                reportRecoveryFailed(event, exception)
                return
            } catch (exception: RuntimeException) {
                reportRecoveryFailed(event, exception)
                return
        ...
        if (destination != null) {
            try {
                ...
                player.sendMessage(
                    CrabMessages.warning("Recovered ")
                ...
                plugin.logger.log(
                    Level.SEVERE,
                    "Could not return the cosmetic from a broken item to ${player.uniqueId}",
                    exception,
                )
            }
        }
        ...
        player.sendMessage(
            CrabMessages.warning("Helmet broke; dropped ")
                .append(itemName(cosmetic))
                .append(CrabMessages.warning(" at your feet."))
        )
    }

    private fun reportRecoveryFailed(event: PlayerItemBreakEvent, exception: Exception) {
        plugin.logger.log(
            Level.SEVERE,
            "Could not recover the cosmetic from a broken merged item belonging to ${event.player.uniqueId}",
            exception,
        )
    ...
    companion object {
        ...
        fun create(plugin: JavaPlugin): ModelCommand {
            ...
            return try {
                ...
                ModelCommand(plugin, lookup)
            } catch (error: LinkageError) {
                plugin.logger.warning(
                    "Nexo is present but its API could not be loaded; model merging is unavailable: ${error.message}"
                )
                ModelCommand(plugin, null)
            }
        }

        private fun decrement(item: ItemStack): ItemStack =
            if (item.amount == 1) ItemStack.empty() else item.clone().apply { amount = item.amount - 1 }
        ...
        private fun findDestination(
            ...
            return if (canAccept(inventory, otherHand, returned))
                Destination(if (mergedInMainHand) DestinationKind.OFF_HAND else DestinationKind.MAIN_HAND)
            else findStorageDestination(inventory, returned)
        }

        private fun canAccept(inventory: PlayerInventory, existing: ItemStack?, incoming: ItemStack): Boolean {
            if (existing == null || existing.isEmpty)
                return incoming.amount <= minOf(inventory.maxStackSize, incoming.maxStackSize)
            if (!existing.isSimilar(incoming)) return false
            val maximum = minOf(inventory.maxStackSize, existing.maxStackSize, incoming.maxStackSize)
        ...
        private fun placeReturnedItem(inventory: PlayerInventory, destination: Destination, returned: ItemStack) {
            when (destination.kind) {
                ...
                DestinationKind.STORAGE ->
                    inventory.setItem(destination.slot, combine(inventory.getItem(destination.slot), returned))
            }
        }

        private fun combine(existing: ItemStack?, incoming: ItemStack): ItemStack =
            if (existing == null || existing.isEmpty) incoming.asOne()
            else existing.clone().apply { amount = existing.amount + incoming.amount }

        private fun itemName(item: ItemStack): Component =
            Component.text()
        ...
        private fun findStorageDestination(inventory: PlayerInventory, returned: ItemStack): Destination? {
            for ((slot, item) in inventory.storageContents.withIndex()) {
                if (canAccept(inventory, item, returned)) return Destination(DestinationKind.STORAGE, slot)
            }
            return null
        }
    }
