    ...
    companion object {
        ...
        fun readVec3(tag: CompoundTag?): Vec3? =
            if (tag != null && tag.contains("dx") && tag.contains("dy") && tag.contains("dz")) {
                Vec3(tag.getDoubleOr("dx", 0.0), tag.getDoubleOr("dy", 0.0), tag.getDoubleOr("dz", 0.0))
            } else null

        @JvmStatic
        fun readEntityPositionFromTag(tag: CompoundTag?): Vec3? {
            if (tag == null || !tag.contains("Pos")) return null
            val list = tag.getListOrEmpty("Pos")
            if (list.size != 3) return null
            return Vec3(list.getDoubleOr(0, 0.0), list.getDoubleOr(1, 0.0), list.getDoubleOr(2, 0.0))
        }

        @JvmStatic
        fun readVec3iFromTag(tag: CompoundTag?): Vec3i? =
            if (tag != null && tag.contains("x") && tag.contains("y") && tag.contains("z")) {
                Vec3i(tag.getIntOr("x", 0), tag.getIntOr("y", 0), tag.getIntOr("z", 0))
            } else null
