...
enum class BingoTask(private val id: String, private val description: String, private val detail: String) {
    GROW_TREE_IN_NETHER(
        "grow_tree_in_nether",
    ...
    companion object {
        ...
        private val ALL_DEPLOYED = java.util.List.copyOf(entries)
        private val BY_ID = buildMap {
            for (task in BingoTask.entries) {
                check(put(task.id, task) == null) { "Duplicate bingo task ID: ${task.id}" }
            }
        }

        @JvmStatic fun cardOne(): List<BingoTask> = CARD_ONE

        @JvmStatic fun cardTwo(): List<BingoTask> = CARD_TWO

        @JvmStatic fun cardThree(): List<BingoTask> = CARD_THREE

        @JvmStatic fun cardFour(): List<BingoTask> = CARD_FOUR

        @JvmStatic fun cardFive(): List<BingoTask> = CARD_FIVE

        @JvmStatic fun cardSix(): List<BingoTask> = CARD_SIX

        @JvmStatic fun allDeployed(): List<BingoTask> = ALL_DEPLOYED

        @JvmStatic fun fromId(id: String?): Optional<BingoTask> = Optional.ofNullable(BY_ID[id])
    }
}
