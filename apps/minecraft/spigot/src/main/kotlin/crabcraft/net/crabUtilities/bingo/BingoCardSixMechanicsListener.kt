...
class BingoCardSixMechanicsListener(
    ...
    fun onFishCaught(event: PlayerFishEvent) {
        ...
        if (
            event.state != PlayerFishEvent.State.CAUGHT_FISH || !tracking.test(player, BingoTask.FISH_TREASURE_AND_JUNK)
        )
            return
        ...
        if (flags == ALL_FISH_CATEGORIES) completion.accept(player, BingoTask.FISH_TREASURE_AND_JUNK)
    }
    ...
    fun onEnderChestClick(event: InventoryClickEvent) {
        ...
        if (
            event.view.topInventory.type != InventoryType.ENDER_CHEST ||
                !canChangeInventory(event.action) ||
                !tracking.test(player, BingoTask.FILL_ENDER_CHEST) ||
                occupiedSlots(event.view.topInventory) >= 27
        )
            return
    ...
    fun onSmithingResultTaken(event: SmithItemEvent) {
        ...
        if (!appliesNewTrim(inventory.inputEquipment, result)) return
    ...
    fun onDecoratedPotCraftAttempt(event: CraftItemEvent) {
        ...
        if (
            !isResultTakeAction(event.action) ||
                result.type != Material.DECORATED_POT ||
                !tracking.test(player, BingoTask.FOUR_SHERD_DECORATED_POT) ||
                !hasFourDistinctSherds(crafting.matrix)
        )
            return
        val playerId = player.uniqueId
        val pending = PendingResult(result.clone(), attemptToken(playerId))
    ...
    override fun resetPlayer(playerId: UUID) {
        ...
        Bukkit.getPlayer(playerId)?.let { clearPlayerProgress(it) }
    }
    ...
    private fun cardProgress(player: Player): PersistentDataContainer {
        ...
        if (data.get(progressCardKey, PersistentDataType.INTEGER) != cardId) {
    ...
    private data class PendingResult(val result: ItemStack, val token: AttemptToken)
    ...
    companion object {
        ...
        fun fishingCategory(material: Material, enchanted: Boolean): Int =
            when {
                // Vanilla's unenchanted rod is junk, while an enchanted rod is treasure.
                material == Material.FISHING_ROD -> if (enchanted) FISH_TREASURE else FISH_JUNK
                material in TREASURE_ITEMS -> FISH_TREASURE
                material in JUNK_ITEMS -> FISH_JUNK
                else -> 0
            }

        @JvmStatic
        fun decodeMaterials(encoded: String?): MutableSet<Material> =
            EnumSet.noneOf(Material::class.java).apply {
                if (!encoded.isNullOrBlank()) encoded.split(',').mapNotNullTo(this) { Material.matchMaterial(it) }
            }
        ...
        @JvmStatic
        fun occupiedSlots(inventory: Inventory): Int = inventory.storageContents.count { it != null && !it.type.isAir }
        ...
        fun appliesNewTrim(before: ItemStack?, result: ItemStack?): Boolean {
            ...
            return !beforeMeta.hasTrim() || beforeMeta.trim != resultMeta.trim
        }

        @JvmStatic
        fun hasFourDistinctSherds(matrix: Array<out ItemStack?>): Boolean =
            hasFourDistinctSherdMaterials(Array(matrix.size) { matrix[it]?.type?.takeUnless(Material::isAir) })
        ...
        fun hasFourDistinctSherdMaterials(matrix: Array<out Material?>): Boolean {
            ...
            return slots == 4 && sherds.size == 4
        }

        @JvmStatic
        fun smithingInputs(inventory: SmithingInventory): Array<StackSnapshot?> =
            snapshots(arrayOf(inventory.inputTemplate, inventory.inputEquipment, inventory.inputMineral))

        @JvmStatic
        fun snapshots(items: Array<out ItemStack?>): Array<StackSnapshot?> =
            Array(items.size) { index ->
                items[index]?.takeUnless { it.type.isAir }?.let { StackSnapshot(it.type, it.amount) }
            }
        ...
        fun everyInputWasConsumed(before: Array<out StackSnapshot?>, after: Array<out StackSnapshot?>): Boolean {
            ...
            for (index in before.indices) {
                ...
                if (
                    current != null &&
                        (current.material() != previous.material() || current.amount() >= previous.amount())
                )
                    return false
            }
            return hadInput
        ...
        @JvmStatic
        fun matchingItemCount(items: Array<out ItemStack?>, expected: ItemStack): Int = items.sumOf {
            if (it != null && it.isSimilar(expected)) it.amount else 0
        }

        @JvmStatic fun resultCountIncreased(before: Int, after: Int): Boolean = before >= 0 && after > before
