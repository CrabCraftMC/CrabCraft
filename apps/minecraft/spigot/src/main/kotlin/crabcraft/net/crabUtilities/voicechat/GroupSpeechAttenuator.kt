...
class GroupSpeechAttenuator(
    ...
    fun onStaticSoundPacket(event: StaticSoundPacketEvent) {
        ...
        val receiver = event.getReceiverConnection() ?: return
        if (!shouldAttenuate(event.getSource(), groupId, sender)) return
        val original = event.getPacket() ?: return
        try {
            val scaled =
                scaler.scale(
                    sender!!.getPlayer().getUuid(),
    ...
    companion object {
        @JvmStatic
        fun shouldAttenuate(source: String?, groupId: UUID, sender: VoicechatConnection?): Boolean =
            SoundPacketEvent.SOURCE_GROUP == source && sender?.getGroup()?.getId() == groupId
    }
}
