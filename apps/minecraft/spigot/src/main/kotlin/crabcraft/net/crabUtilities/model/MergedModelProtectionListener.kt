...
class MergedModelProtectionListener(private val codec: MergedModelCodec) : Listener {
    ...
    fun onPrepareSmithing(event: PrepareSmithingEvent) {
        val inventory = event.inventory
        if (shouldClearResult(event.result, inventory.inputTemplate, inventory.inputEquipment, inventory.inputMineral))
            event.result = null
    }
    ...
    fun onCrafterCraft(event: CrafterCraftEvent) {
        ...
        if (shouldClearResult(event.result, *crafter.inventory.contents)) event.isCancelled = true
    }
    ...
    fun onResultClick(event: InventoryClickEvent) {
        if (
            event.slotType != InventoryType.SlotType.RESULT ||
                ...
                !hasUnsafeResult(event.view.topInventory)
        )
            return
    ...
    private fun hasUnsafeResult(inventory: Inventory): Boolean =
        when (inventory) {
            ...
            is SmithingInventory ->
                shouldClearResult(
                    inventory.result,
                    inventory.inputTemplate,
                    inventory.inputEquipment,
                    inventory.inputMineral,
                )
