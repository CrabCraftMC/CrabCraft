...
class BingoCardFourMobListener(
    ...
    fun onSulfurCubeBucketed(event: PlayerBucketEntityEvent) {
        ...
        if (
            isDiamondSulfurCubeBucketTarget(cube.isAdult, cube.fuseTicks, bodyItem, event.originalBucket.type) &&
    ...
    fun onSilverfishNamed(event: PlayerNameEntityEvent) {
        val silverfish = event.entity as? Silverfish ?: return
        if (!hasName(event.name)) return
        val player = event.player
    ...
    fun onPowderSnowPlaced(event: PlayerBucketEmptyEvent) {
        if (
            event.bucket != Material.POWDER_SNOW_BUCKET || !tracking.test(event.player, BingoTask.FREEZE_SKELETON_STRAY)
        )
            return
    ...
    fun onSkeletonFrozen(event: EntityTransformEvent) {
        ...
        if (
            event.transformReason != EntityTransformEvent.TransformReason.FROZEN ||
                event.transformedEntity !is Stray ||
    ...
    private fun newestIntersectingPowderSnow(skeleton: Skeleton): OwnedPowderSnow? {
        prunePowderSnowOwners(System.currentTimeMillis())
        ...
        while (iterator.hasNext()) {
            val (key, candidate) = iterator.next()
    ...
    fun onMooshroomInteracted(event: PlayerInteractEntityEvent) {
        val mooshroom = event.rightClicked as? MushroomCow ?: return
        if (!mooshroom.isAdult || mooshroom.variant != MushroomCow.Variant.BROWN) return
        val player = event.player
        when (player.inventory.getItem(event.hand).type) {
            Material.WITHER_ROSE -> beginMooshroomPrime(player, mooshroom)
            Material.BOWL -> beginMooshroomBowl(player, mooshroom)
            else -> {}
        }
    }
    ...
    private fun beginMooshroomPrime(player: Player, mooshroom: MushroomCow) {
        ...
        Bukkit.getScheduler()
            .runTask(plugin, Runnable { confirmMooshroomPrimed(playerId, mooshroomId, preliminaryMarker, token) })
    }

    private fun confirmMooshroomPrimed(
        playerId: UUID,
        mooshroomId: UUID,
        preliminaryMarker: PersistentMarker,
        token: AttemptToken,
    ) {
    ...
    private fun beginMooshroomBowl(player: Player, mooshroom: MushroomCow) {
        ...
        val marker = mooshroomMarker(mooshroom) ?: return
    ...
    private fun confirmMooshroomBowl(
        playerId: UUID,
        mooshroomId: UUID,
        expectedMarker: PersistentMarker,
        token: AttemptToken,
    ) {
        val mooshroom = Bukkit.getEntity(mooshroomId) as? MushroomCow ?: return
        if (mooshroom.hasEffectsForNextStew()) return
        if (expectedMarker != mooshroomMarker(mooshroom)) return
        clearMooshroomAttribution(mooshroom)
        val player = Bukkit.getPlayer(playerId)
        if (
            player != null &&
                expectedMarker.playerId == playerId &&
    ...
    fun onBlockPlaced(event: BlockPlaceEvent) {
        ...
        if (
            event.canBuild() &&
                event.blockPlaced.type == Material.POWDER_SNOW &&
                event.itemInHand.type == Material.POWDER_SNOW_BUCKET &&
                tracking.test(event.player, BingoTask.FREEZE_SKELETON_STRAY)
        ) {
            schedulePowderSnowConfirmation(event.player, key)
        }
    }
    ...
    fun onEntityChangedBlock(event: EntityChangeBlockEvent) {
        val silverfish = event.entity as? Silverfish
        if (silverfish != null && isSilverfishInfestation(event.block.type, event.to))
    ...
    private fun completeSilverfishInfestation(silverfish: Silverfish) {
        val marker = silverfishMarker(silverfish) ?: return
        if (!markerIsCurrent(marker)) return
    ...
    fun onBlockExplosion(event: BlockExplodeEvent) {
        event.blockList().forEach { removePowderSnow(BlockKey.from(it)) }
    }
    ...
    private fun removePowderSnow(key: BlockKey) {
    ...
    private fun mooshroomMarker(mooshroom: MushroomCow): PersistentMarker? {
        ...
        val owner = data.get(mooshroomOwnerKey, PersistentDataType.STRING) ?: return null
        val primedAt = data.get(mooshroomPrimedAtKey, PersistentDataType.LONG) ?: return null
        return try {
            PersistentMarker(UUID.fromString(owner), primedAt)
    ...
    private fun silverfishMarker(silverfish: Silverfish): PersistentMarker? {
        val data = silverfish.persistentDataContainer
        val owner = data.get(silverfishOwnerKey, PersistentDataType.STRING) ?: return null
        val namedAt = data.get(silverfishNamedAtKey, PersistentDataType.LONG) ?: return null
        return try {
            PersistentMarker(UUID.fromString(owner), namedAt)
        } catch (_: IllegalArgumentException) {
            null
        }
    }
    ...
    private fun markerIsCurrent(marker: PersistentMarker, now: Long = System.currentTimeMillis()): Boolean =
        isFreshAttribution(
            marker.timestamp,
            now,
            clearedAtMillis,
            playerResetAtMillis.getOrDefault(marker.playerId, 0L),
            MAX_ATTRIBUTION_MILLIS,
        )

    private data class OwnedPowderSnow(val playerId: UUID, val placedAtMillis: Long)

    private data class PersistentMarker(val playerId: UUID, val timestamp: Long)

    companion object {
        private const val MAX_ATTRIBUTION_MILLIS = 7L * 24 * 60 * 60 * 1000

        @JvmStatic
        fun isDiamondSulfurCubeBucketTarget(
            adult: Boolean,
            fuseTicks: Int,
            bodyItem: Material?,
            originalBucket: Material?,
        ): Boolean = adult && fuseTicks < 0 && bodyItem == Material.DIAMOND_BLOCK && originalBucket == Material.BUCKET
        ...
        private fun isPrimedBrownMooshroom(mooshroom: MushroomCow): Boolean =
            mooshroom.isAdult &&
                mooshroom.variant == MushroomCow.Variant.BROWN &&
                mooshroom.hasEffectForNextStew(PotionEffectType.WITHER)

        @JvmStatic
        fun isSilverfishInfestation(from: Material, to: Material?): Boolean =
            when (from) {
