...
class BingoManager(private val plugin: JavaPlugin) {
    ...
    private val sourceBackend =
        plugin.config.getString("bingo.source-backend", "")?.takeUnless { it.isBlank() } ?: "unknown"
    private val excludedWorlds = plugin.config.getStringList("bingo.excluded-worlds").toSet()
    ...
    fun start() {
        ...
        jedisPool =
            if (password.isNullOrEmpty()) JedisPool(poolConfig, host, port, 2_000)
            else JedisPool(poolConfig, host, port, 2_000, password)
        detectors =
            listOf(
                HardBingoListener(plugin, ::isTracking, ::complete, ::logHornProgress),
                BingoCardTwoListener(plugin, ::isTracking, ::complete),
        ...
        detectors.forEach { plugin.server.pluginManager.registerEvents(it, plugin) }
        running = true
        activeCardRefreshTask =
            Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, Runnable(::refreshActiveCard), 0L, 20L * 30L)
        completionFlushTask =
            Bukkit.getScheduler().runTaskTimerAsynchronously(plugin, Runnable(::flushPending), 20L, 20L)
    ...
    fun isEligible(player: Player): Boolean {
        ...
        return enabled &&
            ...
            !excludedWorlds.contains(player.world.name) &&
            card != null &&
            card.isLive()
    }
    ...
    private fun logHornProgress(player: Player, progress: HardBingoListener.HornProgress) {
        plugin.logger.info(
            "Bingo goat horn progress for ${player.name}: ${progress.uniqueCount()}/5 (${progress.instrument()})"
        )
    }
    ...
    private fun refreshActiveCard() {
        ...
        try {
            pool.resource.use { jedis ->
                ...
                var next = json?.let(BingoActiveCard::fromJson)
                if (next != null) {
                    val unsupported = next.taskIds().filter { BingoTask.fromId(it).isEmpty }
                    if (next.taskIds().size != 16 || unsupported.isNotEmpty()) {
                        plugin.logger.severe(
                            "Refusing Bingo #${next.number()}: expected 16 deployed task detectors; unsupported=$unsupported"
                        )
                        next = null
                    }
                }
    ...
    private fun applyActiveCard(next: BingoActiveCard?) {
        ...
        activeCard = next
        if (next != null) plugin.logger.info("Tracking Bingo #${next.number()} with ${next.taskIds().size} tasks.")
        else plugin.logger.info("No supported active bingo card; tracking paused.")
    }
    ...
    private fun sendCompletionMessage(completion: PendingCompletion) {
        Bukkit.getScheduler()
            .runTask(
                plugin,
                Runnable {
                    val player = Bukkit.getPlayer(completion.playerId) ?: return@Runnable
                    if (!player.isOnline) return@Runnable
                    if (
                        plugin is crabcraft.net.crabUtilities.CrabUtilities &&
                            !plugin.isBingoMessagesEnabled(completion.playerId)
                    )
                        return@Runnable
                    val message =
                        miniMessage
                            .deserialize(
                                "<#b0b0b0>Completed</#b0b0b0> <#FCD05C>${completion.task.description()}</#FCD05C>"
                            )
