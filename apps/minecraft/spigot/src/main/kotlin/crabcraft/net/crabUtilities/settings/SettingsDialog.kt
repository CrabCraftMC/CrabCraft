...
/** Paper's server-side settings dialog: Done saves all choices, Cancel or Escape discards them. */
...
open class SettingsDialog(private val settingsService: PlayerSettingsService) {
    open fun open(player: Player) {
        ...
        val options = ORDER.map { mode ->
            SingleOptionDialogInput.OptionEntry.create(
                mode.id(),
                mini(mode.coloredLabel()),
                mode == current.getPhantomMode(),
            )
        }
        val base =
            DialogBase.builder(mini(CrabMessages.HIGHLIGHT_TAG + "Settings"))
                .inputs(
                    listOf(
                        DialogInput.singleOption(
                                PHANTOMS_KEY,
                                mini(CrabMessages.HIGHLIGHT_TAG + "Phantoms"),
                                options,
                            )
                            .build(),
                        DialogInput.singleOption(
            ...
            builder
                ...
                .type(
                    DialogType.confirmation(
                        ...
                        ActionButton.create(
                            mini(CrabMessages.SUCCESS_TAG + "Done"),
                            null,
                            100,
    ...
    /** A value omitted by the client preserves the player's current preference. */
    private fun handleSubmit(uuid: UUID, view: DialogResponseView?, audience: Audience) {
        if (view == null) return
        val current = settingsService.get(uuid)
        val mode = view.getText(PHANTOMS_KEY)?.let(PhantomMode::fromId) ?: current.getPhantomMode()
        settingsService.setAll(
            uuid,
            mode,
            selectedToggle(view.getText(MENTION_PINGS_KEY), current.isMentionPings()),
            selectedToggle(view.getText(ACCEPT_MESSAGES_KEY), current.isAcceptMessages()),
            selectedToggle(view.getText(LOCATOR_BAR_KEY), current.isLocatorBar()),
            selectedToggle(view.getText(BINGO_MESSAGES_KEY), current.isBingoMessages()),
            selectedToggle(view.getText(COORDINATE_HUD_KEY), current.isCoordinateHud()),
        )
        audience.sendMessage(CrabMessages.success("Settings saved."))
    }

    private fun mini(text: String) = CrabMessages.mini(text)

    private fun toggleOptions(enabled: Boolean) =
        listOf(
    ...
    companion object {
        ...
        private const val COORDINATE_HUD_KEY = "coordinateHud"
        ...
        private fun selectedToggle(selected: String?, fallback: Boolean) =
            when (selected) {
                TOGGLE_ON -> true
                TOGGLE_OFF -> false
                else -> fallback
