...
class CustomPortalRegistry {
    ...
    fun register(world: World, axis: PortalAxis, portalBlocks: Collection<Block>) {
        register(world, CustomPortalBounds(axis, portalBlocks.map(::positionOf)))
    }

    fun findOrDiscover(seed: Block): Optional<CustomPortalBounds> {
        val axis = axisOf(seed) ?: return Optional.empty()
        val cached = portalsByWorld[seed.world.uid]?.get(blockKey(seed.x, seed.y, seed.z))
        if (cached != null && cached.axis() == axis && cached.contains(positionOf(seed))) return Optional.of(cached)
        return discover(seed, axis).map { bounds ->
            register(seed.world, bounds)
            bounds
        }
    }
    ...
    fun findPortalsTouchingFrame(frame: Block): List<CustomPortalBounds> {
        ...
        for (face in FRAME_NEIGHBOURS) {
            ...
            val axis = axisOf(adjacent) ?: continue
            if (!axis.isInPlane(face)) continue
            val position = positionOf(adjacent)
            if (found.any { it.contains(position) }) continue
            discover(adjacent, axis).ifPresent { bounds ->
                register(frame.world, bounds)
                found.add(bounds)
            }
        }
    ...
    private fun discover(seed: Block, axis: PortalAxis): Optional<CustomPortalBounds> {
        ...
        return if (portalBlocks.isEmpty()) Optional.empty() else Optional.of(CustomPortalBounds(axis, portalBlocks))
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun axisOf(block: Block): PortalAxis? {
            if (block.type != Material.NETHER_PORTAL) return null
            val data = block.blockData as? Orientable ?: return null
            return PortalAxis.from(data.axis)
        }
        ...
        private fun positionOf(block: Block) = CustomPortalBounds.BlockPosition(block.x, block.y, block.z)
