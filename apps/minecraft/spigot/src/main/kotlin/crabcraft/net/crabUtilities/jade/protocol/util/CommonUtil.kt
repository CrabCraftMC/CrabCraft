...
open class CommonUtil {
    companion object {
        @JvmStatic
        fun wrapPartEntityParent(target: Entity?): Entity? = if (target is EnderDragonPart) target.parentMob else target

        @JvmStatic
        fun getPartEntity(parent: Entity?, index: Int): Entity? {
            if (parent == null || index < 0) return parent
            if (parent is EnderDragon) {
                val parts = parent.subEntities
