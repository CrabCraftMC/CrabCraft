...
class BingoCardFiveChallengeListener
@JvmOverloads
constructor(
    ...
    // Exclusively for the loopback test harness; production retains actual player target attribution.
    ...
    fun onEndermanNamed(event: PlayerNameEntityEvent) {
        ...
        if (event.name != null && tracking.test(player, BingoTask.ENDERMAN_KILLED_BY_ENDERMITES_ONLY)) {
    ...
    fun onEndermanDamaged(event: EntityDamageEvent) {
        val enderman = event.entity as? Enderman ?: return
        val marker = markerFrom(enderman, endermanOwnerKey, endermanNamedAtKey, endermanCardIdKey) ?: return
        if (
    ...
    private fun detectEndCrystalHostileKill(event: EntityDeathEvent) {
        ...
        val marker = markerFrom(crystal, endCrystalOwnerKey, endCrystalPlacedAtKey, endCrystalCardIdKey) ?: return
        if (!markerIsCurrent(marker)) return
        val player = Bukkit.getPlayer(marker.playerId)
    ...
    fun onPillagerShot(event: EntityShootBowEvent) {
        ...
        val player = (pillager.target as? Player) ?: creativeObserverForTesting(pillager) ?: return
        if (!tracking.test(player, BingoTask.DISARM_PILLAGER)) return
        val pillagerId = pillager.uniqueId
        val attempt =
            PillagerShotAttempt(player.uniqueId, bow.clone(), Bukkit.getCurrentTick(), attemptToken(player.uniqueId))
    ...
    fun onPillagerCrossbowDamaged(event: EntityDamageItemEvent) {
        ...
        if (
            attempt.shotTick != Bukkit.getCurrentTick() ||
                ...
                !ChallengePolicy.damageBreaksItem(damagedItem, event.damage)
        )
            return
        val pillagerId = pillager.uniqueId
        Bukkit.getScheduler()
            .runTask(plugin, Runnable { confirmPillagerDisarmed(pillagerId, attempt.playerId, attempt.token) })
    }

    private fun confirmPillagerDisarmed(pillagerId: UUID, playerId: UUID, token: AttemptToken) {
        val pillager = Bukkit.getEntity(pillagerId) as? Pillager ?: return
        val player = Bukkit.getPlayer(playerId) ?: return
        if (
            pillager.isValid &&
                !pillager.isDead &&
                isCurrent(playerId, token) &&
                ChallengePolicy.isUnarmed(pillager.equipment.itemInMainHand, pillager.equipment.itemInOffHand) &&
    ...
    fun onEggHatched(event: ThrownEggHatchEvent) {
        val egg = event.egg
        val marker = thrownEggs.remove(egg.uniqueId) ?: markerFrom(egg, eggOwnerKey, eggThrownAtKey, eggCardIdKey)
        clearMarker(egg, eggOwnerKey, eggThrownAtKey, eggCardIdKey)
        if (
            marker == null ||
                !markerIsCurrent(marker) ||
                !ChallengePolicy.isChickenHatch(event.isHatching, event.numHatches.toInt(), event.hatchingType)
        )
            return
    ...
    override fun resetPlayer(playerId: UUID) {
        ...
        thrownEggs.entries.removeIf { entry ->
            if (entry.value.playerId != playerId) false
                ...
                val egg = Bukkit.getEntity(entry.key) as? Egg
                if (egg != null) clearMarker(egg, eggOwnerKey, eggThrownAtKey, eggCardIdKey)
                true
            }
        }
    }
    ...
    private fun markerIsCurrent(marker: PersistentMarker): Boolean {
        ...
        return markerMatchesCard(marker.cardId, activeCardId.asInt) &&
            isFreshAttribution(marker.timestamp, now, resetAt, MAX_PERSISTENT_ATTRIBUTION_MILLIS)
    }

    private data class PersistentMarker(val playerId: UUID, val timestamp: Long, val cardId: Int)

    private data class PillagerShotAttempt(
        val playerId: UUID,
        val crossbow: ItemStack,
        val shotTick: Int,
    ...
    object ChallengePolicy {
        ...
        fun isEndermiteMeleeDamage(source: DamageSource): Boolean {
            ...
            if (type != DamageType.MOB_ATTACK && type != DamageType.MOB_ATTACK_NO_AGGRO) return false
        ...
        fun isExplosionDamage(source: DamageSource): Boolean =
            source.damageType == DamageType.EXPLOSION || source.damageType == DamageType.PLAYER_EXPLOSION

        @JvmStatic
        fun retainsEndermanMarker(finalDamage: Double, endermiteMeleeDamage: Boolean): Boolean =
        ...
        fun damageBreaksItem(item: ItemStack, appliedDamage: Int): Boolean {
            ...
            val maximumDamage = if (damageable.hasMaxDamage()) damageable.maxDamage else item.type.maxDurability.toInt()
            return damageBreaksItem(damageable.damage, maximumDamage, appliedDamage)
        }

        @JvmStatic
        fun damageBreaksItem(currentDamage: Int, maximumDamage: Int, appliedDamage: Int): Boolean =
            currentDamage >= 0 &&
                maximumDamage > 0 &&
                appliedDamage > 0 &&
        ...
        @JvmStatic
        fun isChickenHatch(hatching: Boolean, numberOfHatches: Int, hatchType: EntityType?): Boolean =
            hatching && numberOfHatches > 0 && hatchType == EntityType.CHICKEN

        @JvmStatic
        fun isUnarmed(mainHand: ItemStack?, offHand: ItemStack?): Boolean = isEmpty(mainHand) && isEmpty(offHand)
    ...
    companion object {
        ...
        @JvmStatic
        fun isFreshAttribution(timestamp: Long, now: Long, playerResetAt: Long, maximumAge: Long): Boolean =
        ...
        private fun isUsableCardId(cardId: Int): Boolean = cardId != Int.MIN_VALUE
