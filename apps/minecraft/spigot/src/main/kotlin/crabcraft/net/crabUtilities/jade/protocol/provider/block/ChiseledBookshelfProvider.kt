...
enum class ChiseledBookshelfProvider : StreamServerDataProvider<BlockAccessor, ItemStack> {
    ...
    override fun streamData(accessor: BlockAccessor): ItemStack? {
        val slot =
            (accessor.getBlock() as ChiseledBookShelfBlock)
                .getHitSlot(accessor.getHitResult(), accessor.getBlockState().getValue(FACING))
                .orElse(-1)
    ...
    companion object {
        private val UID = JadeProtocol.mc_id("shelf")
    }
}
