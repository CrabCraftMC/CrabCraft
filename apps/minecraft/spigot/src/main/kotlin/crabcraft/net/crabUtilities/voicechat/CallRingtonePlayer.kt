...
class CallRingtonePlayer(
    ...
    constructor(
        ...
        api,
        loadClip(INCOMING_RESOURCE),
        loadClip(OUTGOING_RESOURCE),
    ...
    fun start(start: VoiceMessages.CallRingStart) {
        ...
        val connection = api.getConnectionOf(start.playerId()) ?: return
        if (!connection.isInstalled() || !connection.isConnected()) return
        // An invalid absolute deadline can never exceed the invitation's 30-second lifetime.
        val maximumDeadline =
            if (now > Long.MAX_VALUE - MAX_RING_DURATION_MILLIS) Long.MAX_VALUE else now + MAX_RING_DURATION_MILLIS
        val safeDeadline = minOf(start.expiresAtMillis(), maximumDeadline)
        val key = SessionKey(start.playerId(), start.direction())
        endpoints
            .compute(key) { _, current -> current ?: createEndpoint(key) }
            ?.start(start.token(), safeDeadline, connection)
    }

    fun stop(stop: VoiceMessages.CallRingStop) {
        endpoints[SessionKey(stop.playerId(), stop.direction())]?.stopToken(stop.token())
    }

    fun removePlayer(playerId: UUID) {
        for (direction in VoiceMessages.RingDirection.values()) {
            endpoints.remove(SessionKey(playerId, direction))?.close()
        }
    }
    ...
    private fun createEndpoint(key: SessionKey): Endpoint? {
        ...
        try {
            ...
            if (channel == null) {
                logger.warning(
                    "Could not create the ${key.direction.name.lowercase(java.util.Locale.getDefault())} " +
            ...
            channel.setFilter { it.getUuid() == key.playerId }
            return Endpoint(channel, clips.getValue(key.direction))
        } catch (_: Exception) {
    ...
    private inner class Endpoint(private val channel: StaticAudioChannel, private val clip: ShortArray) :
        AutoCloseable {
        ...
        @Synchronized
        fun start(token: String, deadline: Long, connection: VoicechatConnection) {
            if (closed || endpointClosed || !setTarget(connection)) return
            ...
            try {
                ...
                val callbackPlayback = Playback(player, encoder, frames)
                opened = callbackPlayback
        ...
        fun stopToken(token: String) {
            val stopped =
                synchronized(this) {
                    ...
                    if (current?.frames?.removeToken(token) == true) {
                        playback = null
                        current
                    } else null
                }
        ...
        override fun close() {
            ...
            if (removedTarget != null) {
                try {
                    channel.removeTarget(removedTarget)
                } catch (_: Exception) {}
            }
            try {
                channel.flush()
    ...
    class LoopingFrames(private val clip: ShortArray, private val clock: LongSupplier) : Supplier<ShortArray?> {
        ...
        init {
            require(clip.isNotEmpty()) { "Ringtone clip is empty" }
        }

        @Synchronized
        fun addToken(token: String, deadline: Long): Boolean {
        ...
        override fun get(): ShortArray? {
            ...
            val remainingMillis = (deadlines.values.maxOrNull() ?: now) - now
            if (remainingMillis <= 0L) {
                stopped = true
                return null
            }
            val audibleSamples = minOf(FRAME_SAMPLES.toLong(), remainingMillis * SAMPLES_PER_MILLISECOND).toInt()
    ...
    companion object {
        ...
        private fun channelId(key: SessionKey): UUID =
            UUID.nameUUIDFromBytes(
                "crabcraft:svc:call-ringtone:${key.playerId}:${key.direction.name}".toByteArray(StandardCharsets.UTF_8)
            )

        @JvmStatic
        @Throws(IOException::class)
        fun loadClip(resource: String): ShortArray {
            val input =
                CallRingtonePlayer::class.java.classLoader.getResourceAsStream(resource)
                    ?: throw IOException("Missing bundled call ringtone $resource")
            input.use {
                val encoded = it.readAllBytes()
                if (encoded.isEmpty() || encoded.size and 1 != 0) {
                    throw IOException("Call ringtone contains invalid PCM audio: $resource")
                }
