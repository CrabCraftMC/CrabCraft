...
class ViewDistanceCommand(private val managerSupplier: Supplier<ViewDistanceManager?>) {
    fun handle(sender: CommandSender, args: Array<String>): Boolean {
        ...
        return when (if (args.size >= 2) args[1].lowercase(Locale.ROOT) else "status") {
    ...
    private fun sendStatus(sender: CommandSender, args: Array<String>): Boolean {
        ...
        if (manager == null) sender.sendMessage(CrabMessages.label("State", CrabMessages.error("disabled")))
        else {
            sender.sendMessage(
                CrabMessages.label(
                    "State",
                    if (manager.isPaused()) CrabMessages.warning("paused") else CrabMessages.success("running"),
                )
            )
            sender.sendMessage(
                CrabMessages.label(
                    "Configured simulation range",
                    "${manager.getMinimumSimulationDistance()}–${manager.getMaximumSimulationDistance()}",
                )
            )
            sender.sendMessage(
                CrabMessages.label(
                    "Configured view range",
                    "${manager.getMinimumViewDistance()}–${manager.getMaximumViewDistance()}",
                )
            )
        }
        val worlds =
        ...
        for (world in worlds) {
            val distances =
                CrabMessages.text("view ${world.viewDistance}")
                    .append(CrabMessages.muted(", "))
                    .append(CrabMessages.text("simulation ${world.simulationDistance}"))
            sender.sendMessage(CrabMessages.label(world.name, distances))
        }
        return true
    }
    ...
    private fun setDistance(sender: CommandSender, args: Array<String>): Boolean {
        ...
        val minimum =
            if (kind == DistanceKind.VIEW) manager.getMinimumViewDistance() else manager.getMinimumSimulationDistance()
        val maximum =
            if (kind == DistanceKind.VIEW) manager.getMaximumViewDistance() else manager.getMaximumSimulationDistance()
        if (distance !in minimum..maximum) {
            sender.sendMessage(
                CrabMessages.error("${kind.displayName} distance must be between $minimum and $maximum.")
            )
            return true
        }
        val world = resolveWorld(sender, if (args.size == 5) args[4] else null) ?: return true
        if (kind == DistanceKind.VIEW) manager.setManualViewDistance(world, distance)
        else manager.setManualSimulationDistance(world, distance)
        sender.sendMessage(CrabMessages.success("Set ${world.name}'s ${kind.displayName} distance to $distance."))
        if (!manager.isPaused())
            sender.sendMessage(CrabMessages.warning("Dynamic adjustment is still running and may change this value."))
        return true
    }
    ...
    private fun pause(sender: CommandSender, args: Array<String>): Boolean {
        ...
        sender.sendMessage(
            CrabMessages.success("Paused dynamic view-distance adjustment; current world values will remain fixed.")
        )
        return true
    }
    ...
    private fun requireManager(sender: CommandSender): ViewDistanceManager? {
        val manager = managerSupplier.get()
        if (manager == null)
            sender.sendMessage(CrabMessages.error("Adaptive view distance is disabled or has invalid configuration."))
        return manager
    }
    ...
    private enum class DistanceKind(val displayName: String) {
        ...
        companion object {
            fun parse(value: String): DistanceKind? =
    ...
    companion object {
        ...
        private fun resolveWorld(sender: CommandSender, worldName: String?): World? {
            ...
            if (sender is Player) return sender.world
            sender.sendMessage(CrabMessages.error("Specify a world when running this command from console."))
            return null
        }

        private fun worldNames(prefix: String) = filter(Bukkit.getWorlds().map { it.name }, prefix)

        private fun integerRange(minimum: Int, maximum: Int) = (minimum..maximum).map(Int::toString)
        ...
        private fun sendUsage(sender: CommandSender) {
            sender.sendMessage(
                CrabMessages.error(
                    "Usage: /crabutilities viewdistance <status [world]|set <view|simulation> <distance> [world]|pause|resume>"
                )
            )
        }
    }
