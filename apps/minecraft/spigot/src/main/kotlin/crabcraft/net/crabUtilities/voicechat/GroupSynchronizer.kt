...
/** Keeps Redis definitions and locally persistent SVC replicas converged. */
class GroupSynchronizer(
    ...
    private val reconcileCreator: Consumer<UUID>,
) {
    ...
    private val registryExecutor = Executors.newSingleThreadExecutor {
        Thread(it, "CrabUtilities-VoiceGroups").apply { isDaemon = true }
    }

    fun seedPermanent(group: Group) {
        val definition = definitionOf(group, null, true)
        known[group.id] = definition
        persist(definition)
    }

    fun onCreateGroup(event: CreateGroupEvent) {
        val group = event.group ?: return
        if (event.isCancelled || group.id in applying) return
        val password =
            try {
                passwordOf(group)
        ...
        val creator = event.connection?.player?.uuid
        // Wait for SVC to commit the cancellable pre-event and later listeners to finish.
        Bukkit.getScheduler()
            .runTask(
                plugin,
                Runnable {
                    if (event.isCancelled || findLocal(definition.id) == null || apply(definition) == null)
                        return@Runnable
            ...
            Bukkit.getScheduler()
                .runTask(
                    ...
                    Runnable {
                        ...
                        definitions.values.forEach(::apply)
                        for ((id, local) in known.toMap()) {
                            if (id in definitions) continue
                            if (local.permanent || id in pendingWrites) persist(local) else removeLocal(id)
                        }
                    },
                )
        }
    ...
    fun fetch(groupId: UUID) = bus.fetchGroup(groupId)

    fun definition(groupId: UUID) = known[groupId]

    fun onRegistryWrite(definitionChanged: Boolean) {
        if (definitionChanged) registryRevision.incrementAndGet()
    }

    fun findLocal(groupId: UUID): Group? = api.groups.firstOrNull { it.id == groupId }
            ...
            Bukkit.getScheduler()
                .runTask(
                    ...
                    Runnable {
                        ...
                        if (local != null && (local.permanent || groupId in pendingWrites)) {
                            persist(local)
                            return@Runnable
                        }
                        removeLocal(groupId)
    ...
    private fun removeLocal(groupId: UUID) {
        if (findLocal(groupId) == null || api.removeGroup(groupId)) known.remove(groupId)
    }

    private fun warnUnsupportedPasswordGroup(event: CreateGroupEvent, group: Group) {
        logger.severe(
            "Blocked password-protected voice group '${group.name}': this Simple Voice Chat runtime does not expose the password needed " +
                "to secure replicas on every backend"
        )
        val player = event.connection?.player?.player
        if (player is Player)
            player.sendMessage(
                "Could not create that voice group because its password " +
    ...
    companion object {
        ...
        fun passwordOf(group: Group): String? {
            if (!group.hasPassword()) return null
            val internalGroup =
                group.javaClass.getMethod("getGroup").invoke(group)
                    ?: throw ReflectiveOperationException("missing internal group")
            return internalGroup.javaClass.getMethod("getPassword").invoke(internalGroup) as? String
                ?: throw ReflectiveOperationException("missing group password")
        }

        /** Preserve Redis's call marker while exposing calls in upgraded clients' group lists. */
        @JvmStatic
        fun isHiddenLocally(definition: VoiceMessages.GroupDefinition) =
            definition.hidden && !CallTargetSynchronizer.isCallGroup(definition)

        private fun definitionOf(group: Group, password: String?, permanent: Boolean) =
            VoiceMessages.GroupDefinition(group.id, group.name, password, group.type, group.isHidden, permanent)
    }
}
