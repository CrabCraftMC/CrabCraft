...
class MediaMessages {
    ...
    fun prefixedComponent(key: String, vararg replacements: Any?): Component =
        plain(MINI_MESSAGE.deserialize(template("prefix") + replaceNumberedMarkers(template(key), *replacements)))

    fun string(key: String, vararg replacements: Any?) = replaceNumberedMarkers(template(key), *replacements)
    ...
    companion object {
        ...
        private val TEMPLATES =
            mapOf(
                ...
                "command.horn.create.dialog.body" to
                    "<#b0b0b0>Paste a link to YouTube, SoundCloud, Twitch, or a direct audio file. " +
                ...
                "command.horn.create.messages.error.not-holding-horn" to
                ...
                "command.horn.create.messages.length-warning" to
        ...
        private fun template(key: String) = TEMPLATES[key] ?: "<red>Missing media message: $key"

        private fun replaceNumberedMarkers(template: String, vararg replacements: Any?): String {
            var result = template
            for (index in replacements.indices) result =
                result.replace("{$index}", java.lang.String.valueOf(replacements[index]))
            return result
        }

        @JvmStatic
        fun literalReplacement(template: String, replacement: Component): Component {
