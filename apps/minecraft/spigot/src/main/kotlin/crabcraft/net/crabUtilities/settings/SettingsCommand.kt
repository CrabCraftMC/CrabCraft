...
/** Opens the settings dialog or changes a single preference from chat. */
open class SettingsCommand(
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        ...
        // Do not show/save defaults while the player's persisted record is still loading.
        ...
        when (args[0].lowercase(Locale.ROOT)) {
            ...
            "mentions" ->
                handleToggle(sender, args, "Chat pings", settingsService.isMentionPingsEnabled(uuid)) {
                    settingsService.setMentionPings(uuid, it)
                }
            "messages" ->
                handleToggle(sender, args, "Private messages", settingsService.isAcceptingMessages(uuid)) {
                    settingsService.setAcceptingMessages(uuid, it)
                }
            "locatorbar",
            "locator-bar",
            "locator" ->
                handleToggle(sender, args, "Locator bar", settingsService.isLocatorBarEnabled(uuid)) {
                    settingsService.setLocatorBar(uuid, it)
                }
            "bingo" ->
                handleToggle(sender, args, "Bingo messages", settingsService.isBingoMessagesEnabled(uuid)) {
                    settingsService.setBingoMessages(uuid, it)
                }
            "coordinatehud" ->
                handleToggle(sender, args, "Coordinate hud", settingsService.isCoordinateHudEnabled(uuid)) {
                    settingsService.setCoordinateHud(uuid, it)
                }
            else ->
                sender.sendMessage(
                    CrabMessages.error(
    ...
    private fun handlePhantoms(player: Player, uuid: UUID, args: Array<String>) {
        if (args.size == 1) {
            player.sendMessage(
                CrabMessages.label(
                    "Phantoms",
                    CrabMessages.mini(settingsService.getPhantomMode(uuid).coloredLabel()),
                )
            )
            return
        }
    ...
    companion object {
        ...
        private fun parseMode(token: String): PhantomMode? =
            when (token.lowercase(Locale.ROOT)) {
                ...
                "disable",
                "disabled",
                "false" -> PhantomMode.OFF
                "safe",
                "dontattack",
                "dont-attack",
                "noattack",
                "no-attack" -> PhantomMode.SAFE
                else -> null
            }

        private fun parseToggle(token: String): Boolean? =
            when (token.lowercase(Locale.ROOT)) {
                "on",
                "enable",
                "enabled",
        ...
        private fun onOff(value: Boolean) = if (value) CrabMessages.success("On") else CrabMessages.error("Off")
    }
}
