...
/** The horizontal plane used to flood a portal and orient its blocks. */
enum class PortalAxis(
    @JvmField val axis: Axis,
    @JvmField val positive: Vector,
    @JvmField val negative: Vector,
    @JvmField val left: BlockFace,
    @JvmField val right: BlockFace,
) {
    ...
    fun isInPlane(face: BlockFace) = face == BlockFace.UP || face == BlockFace.DOWN || face == left || face == right

    /** Both directions must reach frame within the configured maximum width. */
    fun isEnclosedOn(world: World, source: Location, settings: PortalSettings): Boolean {
        val positiveHit =
            world.rayTraceBlocks(
                source,
                positive,
                settings.maxPortalWidth().toDouble(),
    ...
    companion object {
        ...
        private fun isFrameHit(result: RayTraceResult?, settings: PortalSettings) =
