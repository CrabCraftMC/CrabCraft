...
open class ViewGroup<T>(
    ...
    open fun getExtraData(): CompoundTag = extraData ?: CompoundTag().also { extraData = it }
    ...
    companion object {
        ...
        fun <B : ByteBuf, T : Any> codec(viewCodec: StreamCodec<B, T>): StreamCodec<B, ViewGroup<T>> =
            StreamCodec.composite(
                ...
                { group -> Optional.ofNullable(group.id) },
                ByteBufCodecs.optional(ByteBufCodecs.COMPOUND_TAG),
                { group -> Optional.ofNullable(group.extraData) },
                { views, id, extra -> ViewGroup(views, id, extra) },
            )

        @JvmStatic
        fun <B : ByteBuf, T : Any> listCodec(
            viewCodec: StreamCodec<B, T>
        ): StreamCodec<B, Map.Entry<Identifier, List<ViewGroup<T>>>> =
            StreamCodec.composite(
