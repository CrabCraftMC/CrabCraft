...
enum class MediaSourceKind(private val permissionSuffix: String) {
    ...
    fun permissionSuffix() = permissionSuffix

    fun itemModel(config: MediaConfig) =
        when (this) {
    ...
    companion object {
        @JvmStatic
        fun classify(source: String?, config: MediaConfig): MediaSourceKind? {
        ...
        @JvmStatic
        fun matchesEntireSource(source: String?, expression: String?): Boolean {
            if (source == null || expression == null) return false
            return try {
                Pattern.compile(expression).matcher(source).matches()
