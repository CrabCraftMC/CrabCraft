...
import crabcraft.net.crabUtilities.bingo.BingoTracking.removeFromOwnerIndex
...
constructor(
    ...
    fun onHornInteract(event: PlayerInteractEvent) {
        ...
        val instrument = instrumentKey(item) ?: return
    ...
    private fun confirmHornPlayed(playerId: UUID, instrument: String, usedHorn: ItemStack) {
        ...
        if (!tracking.test(player, BingoTask.PLAY_FIVE_GOAT_HORNS) || player.getCooldown(usedHorn) <= 0) return
        recordHorn(player, instrument)
    }

    private fun recordHorn(player: Player, instrument: String) {
        val horns = hornsByPlayer.getOrPut(player.uniqueId) { HashSet() }
    ...
    fun onResurrect(event: EntityResurrectEvent) {
        (event.entity as? Player)?.let { completion.accept(it, BingoTask.ACTIVATE_TOTEM) }
    }
                ...
                .runTask(
                    ...
                    Runnable {
                        ...
                        if (
                            current is Fox &&
                                currentPlayer != null &&
                                (isTrustedBy(current.firstTrustedPlayer, playerId) ||
                                    isTrustedBy(current.secondTrustedPlayer, playerId))
                        )
    ...
    fun onProjectileLaunched(event: ProjectileLaunchEvent) {
        ...
        if (player is Player && tracking.test(player, BingoTask.IGNITE_CAMPFIRE_FROM_DISTANCE))
            projectileShots[projectile.uniqueId] =
                ProjectileShot(player.uniqueId, projectile.location.clone(), Bukkit.getCurrentTick())
    }
    ...
    fun onProjectileHit(event: ProjectileHitEvent) {
        ...
        if (
            !isCampfire(block.type) ||
                projectile.fireTicks <= 0 ||
                !sameWorld(shot.origin, block.location) ||
                shot.origin.distanceSquared(block.location.toCenterLocation()) < CAMPFIRE_DISTANCE_SQUARED
        )
            return
        val key = BlockKey.from(block)
        Bukkit.getScheduler()
    ...
    fun onSnifferFertilised(event: EntityFertilizeEggEvent) {
        ...
        if (
            breeder != null &&
                event.entity is Sniffer &&
                event.mother is Sniffer &&
                event.father is Sniffer &&
                tracking.test(breeder, BingoTask.BREED_SNIFFERS_COLLECT_EGG)
        ) {
            pruneTransientState(Bukkit.getCurrentTick())
            pendingSnifferEggs.add(
                PendingSnifferEgg(breeder.uniqueId, event.entity.location.clone(), Bukkit.getCurrentTick())
            )
        }
    }
    ...
    fun onItemSpawned(event: ItemSpawnEvent) {
        ...
        for (pending in pendingSnifferEggs) {
            if (
                isFresh(pending.tick, tick, SHORT_CORRELATION_TICKS) &&
                    sameWorld(pending.location, event.location) &&
                    pending.location.distanceSquared(event.location) <= 16.0
            )
                matchingPlayers.add(pending.playerId)
        }
        if (matchingPlayers.size == 1)
            snifferEggOwners[event.entity.uniqueId] = TimedPlayer(matchingPlayers.first(), tick)
    }
            ...
            Bukkit.getScheduler()
                .runTask(
                    ...
                    Runnable {
                        ...
                        if (
                            current is PiglinBrute &&
                                player != null &&
                                current.equipment.itemInMainHand.isSimilar(axe.item)
                        ) {
    ...
    fun onPotionEffect(event: EntityPotionEffectEvent) {
        ...
        if (
            player is Player &&
                event.cause == EntityPotionEffectEvent.Cause.AXOLOTL &&
    ...
    fun onItemDropped(event: PlayerDropItemEvent) {
        ...
        if (
            stack.type == Material.GOLDEN_AXE &&
                stack.enchantments.isNotEmpty() &&
                tracking.test(event.player, BingoTask.EQUIP_PIGLIN_BRUTE_AXE)
        ) {
            pruneTransientState(Bukkit.getCurrentTick())
            droppedAxes[event.itemDrop.uniqueId] =
                DroppedAxe(event.player.uniqueId, singleItem(stack), Bukkit.getCurrentTick())
        }
    }
    ...
    fun onAllayInteracted(event: PlayerInteractEntityEvent) {
        ...
        if (
            event.player.inventory.itemInMainHand.type != Material.AMETHYST_SHARD ||
                !allay.isDancing ||
                !allay.canDuplicate() ||
                !tracking.test(event.player, BingoTask.DUPLICATE_ALLAY)
        )
            return
        ...
        Bukkit.getScheduler()
            .runTask(
                ...
                Runnable {
                    ...
                    val newAllayExists =
                        original.world.getNearbyEntities(location, 4.0, 4.0, 4.0).any {
                            it is Allay && !previousAllays.contains(it.uniqueId)
                        }
    ...
    override fun resetPlayer(playerId: UUID) {
        ...
        fallingAnvilOwners.values.removeIf { it == playerId }
        anvilDamageByVictim.values.removeIf { it.playerId == playerId }
        boatOwners.values.removeIf { it == playerId }
    ...
    private fun hasAllOreFamilies(playerId: UUID, anchor: BlockKey): Boolean {
        val anchorPlacement = oreByBlock[anchor] ?: return false
        if (anchorPlacement.playerId != playerId) return false
        val visited = HashSet<BlockKey>()
        val families = EnumSet.noneOf(OreFamily::class.java)
        val queue = ArrayDeque<BlockKey>()
        queue.add(anchor)
        while (queue.isNotEmpty()) {
            val current = queue.removeFirst()
            if (!visited.add(current)) continue
            val placement = oreByBlock[current] ?: continue
            if (placement.playerId != playerId) continue
            families.add(placement.family)
            if (families.size == ORE_FAMILY_COUNT) return true
    ...
    private fun hasOwnedScaffoldingTower(playerId: UUID, base: Block): Boolean {
        for (offset in 0 until SCAFFOLDING_HEIGHT) {
            ...
            if (current.type != Material.SCAFFOLDING || playerId != scaffoldingOwnerByBlock[BlockKey.from(current)])
                return false
        }
        return true
    }
    ...
    private fun setOre(key: BlockKey, placement: OrePlacement) {
        ...
        if (previous != null && previous.playerId != placement.playerId)
            removeFromOwnerIndex(oresByPlayer, previous.playerId, key)
        val blocks = oresByPlayer.getOrPut(placement.playerId) { LinkedHashSet() }
        blocks.add(key)
    ...
    private fun pruneTransientState(tick: Int) {
        ...
        snifferEggOwners.entries.removeIf {
    ...
    private enum class OreFamily {
        ...
        companion object {
            fun from(material: Material): OreFamily? =
                when (material) {
                    Material.COAL_ORE,
                    Material.DEEPSLATE_COAL_ORE -> COAL
                    Material.IRON_ORE,
                    Material.DEEPSLATE_IRON_ORE -> IRON
                    Material.COPPER_ORE,
                    Material.DEEPSLATE_COPPER_ORE -> COPPER
                    Material.GOLD_ORE,
                    Material.DEEPSLATE_GOLD_ORE -> GOLD
                    Material.REDSTONE_ORE,
                    Material.DEEPSLATE_REDSTONE_ORE -> REDSTONE
                    Material.LAPIS_ORE,
                    Material.DEEPSLATE_LAPIS_ORE -> LAPIS
                    Material.DIAMOND_ORE,
                    Material.DEEPSLATE_DIAMOND_ORE -> DIAMOND
                    Material.EMERALD_ORE,
                    Material.DEEPSLATE_EMERALD_ORE -> EMERALD
    ...
    companion object {
        ...
        private val ORE_FAMILY_COUNT = OreFamily.entries.size
        ...
        @JvmStatic fun serialiseInstrumentKey(key: NamespacedKey?): String? = key?.asString()

        private fun nearbyAllayIds(allay: Allay): Set<UUID> {
            val ids = HashSet<UUID>()
            ids.add(allay.uniqueId)
            allay.getNearbyEntities(4.0, 4.0, 4.0).filterIsInstance<Allay>().forEach { ids.add(it.uniqueId) }
            return ids
        }
        ...
        private fun isCampfire(material: Material): Boolean =
            material == Material.CAMPFIRE || material == Material.SOUL_CAMPFIRE

        private fun isTrustedBy(tamer: org.bukkit.entity.AnimalTamer?, playerId: UUID): Boolean =
            tamer != null && playerId == tamer.uniqueId
    }
}
