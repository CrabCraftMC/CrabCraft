...
open class JadeProtocol {
    companion object {
        ...
        @JvmField
        var blockDataProviders =
            PairHierarchyLookup<ServerDataProvider<BlockAccessor>>(
                HierarchyLookup(Block::class.java),
                HierarchyLookup(BlockEntity::class.java),
            )
        ...
        @JvmStatic
        fun id(path: String): Identifier = Identifier.fromNamespaceAndPath(PROTOCOL_ID, path)

        @org.jetbrains.annotations.Contract("_ -> new")
        @JvmStatic
        fun mc_id(path: String): Identifier = Identifier.withDefaultNamespace(path)
        ...
        fun init(inventoryDataEnabled: Boolean) {
            ...
            blockDataProviders =
                PairHierarchyLookup(HierarchyLookup(Block::class.java), HierarchyLookup(BlockEntity::class.java))
            ...
            if (inventoryDataEnabled) {
                ...
                blockDataProviders.register(
                    ChiseledBookShelfBlockEntity::class.java,
                    ChiseledBookshelfProvider.INSTANCE,
                )
            }
        ...
        fun clientHandshake(player: ServerPlayer, payload: ClientHandshakePayload) {
            ...
            if (payload.protocolVersion() != PROTOCOL_VERSION) {
                player.sendSystemMessage(
                    Component.literal(
                            "You are using a different version of Jade than the server. Please update Jade or report to the server operator"
                        )
                        .withColor(0xff0000)
                )
                return
            }
            JadeMessenger.send(player, handshake())
        ...
        private fun handshake() =
            ServerHandshakePayload(
                emptyMap(),
                shearableBlocks,
                blockDataProviders.mappedIds(),
                entityDataProviders.mappedIds(),
            )
        ...
        @JvmStatic
        fun getClientProtocol(player: Player): Int =
            clientProtocols.getOrDefault(
                player.uuid,
                (player.bukkitEntity as org.bukkit.entity.Player).protocolVersion,
            )

        @JvmStatic fun getServerProtocol() = SharedConstants.getProtocolVersion()

        @JvmStatic fun isPacketEventsAvailable() = Bukkit.getPluginManager().isPluginEnabled("packetevents")

        @JvmStatic
        fun requestEntityData(player: ServerPlayer, payload: RequestEntityPayload) {
            if (!active || player !in enabledPlayers) return
            Bukkit.getScheduler()
                .runTask(
                    JadeBootstrap.INSTANCE,
                    Runnable {
                        if (!active || player !in enabledPlayers) return@Runnable
                        ...
                        try {
                            val requested =
                                CommonUtil.wrapPartEntityParent(
                                    CommonUtil.getPartEntity(
                                        player.level().getEntity(requestedId),
                                        payload.data().partIndex(),
                                    )
                                )
                            ...
                            if (!isValidEntityTarget(player, entity)) return@Runnable
                            identity.putInt("EntityId", entity!!.id)
                            tag.putInt("EntityId", entity.id)
                            for (provider in entityDataProviders.get(entity)) {
                                if (provider !in payload.dataProviders()) continue
                                try {
        ...
        fun requestBlockData(player: ServerPlayer, payload: RequestBlockPayload) {
            ...
            try {
                ...
                val providers: List<ServerDataProvider<BlockAccessor>> =
                    if (blockEntity != null) {
                        blockDataProviders.getMerged(block, blockEntity)
        ...
        fun onServerReload() {
            if (!active) return
            rebuildShearableBlocks()
            for (player in enabledPlayers) JadeMessenger.send(player, handshake())
        }
            ...
            try {
                shearableBlocks =
                    Collections.unmodifiableList(
                        LootTableMineableCollector.execute(
        ...
        private fun createBlockIdentity(pos: BlockPos) = CompoundTag().also { JadeNbtUtils.writeBlockPosToTag(pos, it) }
