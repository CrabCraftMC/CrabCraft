...
open class ItemCollector<T>(private val iterator: ItemIterator<T>?) {
    ...
    open fun update(request: Accessor<*>): List<ViewGroup<ItemStack>>? {
        ...
        val previous = mergedResult
        if (previous != null && iterator.isFinished()) {
            if (version == currentVersion) return previous // Content not changed.
            if (lastTimeFinished + 5 > gameTime) return previous // Avoid updating too frequently.
            iterator.reset()
        }
        ...
        val current = mergedResult
        if (current != null && !iterator.isFinished()) {
            updateCollectingProgress(current.first())
            return current
        }
        val partialResult =
            items.object2IntEntrySet().stream().limit(MAX_SIZE.toLong()).map { it.key.toStack(it.intValue) }.toList()
    ...
    data class ItemDefinition(private val item: Item, private val components: DataComponentPatch) {
        ...
        fun toStack(count: Int): ItemStack = ItemStack(item, count).also { it.applyComponents(components) }
    }
        ...
        private val SHOWN =
            Predicate<ItemStack> { stack ->
                when {
                    ...
                    stack.hasNonDefault(DataComponents.CUSTOM_MODEL_DATA) ||
                        stack.hasNonDefault(DataComponents.ITEM_MODEL) ->
