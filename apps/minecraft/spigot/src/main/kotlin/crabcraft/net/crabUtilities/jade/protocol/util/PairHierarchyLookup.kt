...
open class PairHierarchyLookup<T : JadeProvider>(
            ...
            return mergedCache.get(Pair.of(first.javaClass, second.javaClass)) {
                ...
                when {
                    ...
                    secondList.isEmpty() -> firstList
                        ...
                        ImmutableList.sortedCopyOf(IHierarchyLookup.COMPARATOR, Iterables.concat(firstList, secondList))
                }
            } as List<ANY>
    ...
    override fun get(clazz: Class<*>): List<T> = first.get(clazz).ifEmpty { second.get(clazz) }

    override fun isEmpty(): Boolean = first.isEmpty() && second.isEmpty()

    override fun entries(): Stream<Map.Entry<Class<*>, Collection<T>>> =
        Stream.concat(first.entries(), second.entries())
