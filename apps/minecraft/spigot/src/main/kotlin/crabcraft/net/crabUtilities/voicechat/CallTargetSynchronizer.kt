...
/** Applies durable Redis call intents; CALL_JOIN is only a wake-up hint. */
class CallTargetSynchronizer(
    private val plugin: CrabUtilities,
    private val api: VoicechatServerApi,
    private val bus: RedisVoiceBus,
    private val groups: GroupSynchronizer,
    private val backend: String,
    private val sessionLookup: Function<UUID, Long?>,
    private val routeLookup: Function<UUID, String?>,
    private val cancelRestore: Consumer<UUID>,
    private val reconcileMembership: Consumer<UUID>,
    ...
    private val recoveryExecutor = Executors.newSingleThreadExecutor {
        Thread(it, "CrabUtilities-CallTargets").apply { isDaemon = true }
    }
    ...
    fun reconcile(playerIds: Set<UUID>) {
        if (!closed && playerIds.isNotEmpty()) request(playerIds)
    }
    ...
    fun onRouteReady(playerId: UUID) {
        if (!closed && sessionLookup.apply(playerId) != null) request(setOf(playerId))
    }

    fun isApplying(playerId: UUID) = playerId in applying

    /** Prevent a delayed join from undoing an explicit local group choice. */
    ...
    fun onMembershipReconciled(playerId: UUID, groupId: UUID?) {
        ...
        // The refreshed player-group lease owns membership and hop recovery after joining.
    ...
    private fun read(requests: Map<UUID, Request>) {
        ...
        try {
            Bukkit.getScheduler().runTask(plugin, Runnable { applyRead(requests, read.value()!!, definitions) })
        } catch (_: Exception) {
            /* Plugin is stopping. */
        }
    }
    ...
    private fun applyRead(
        requests: Map<UUID, Request>,
        targets: Map<UUID, VoiceMessages.CallTarget>,
        definitions: Map<UUID, VoiceMessages.GroupDefinition>,
    ) {
        if (closed) return
        for ((playerId, request) in requests) {
            ...
            if (isCallGroup(definition)) applyTarget(playerId, request, target, definition!!)
        }
    }
    ...
    private fun applyTarget(
        ...
        definition: VoiceMessages.GroupDefinition,
    ) {
        ...
        if (
            player == null ||
                ...
                backend != VoiceMessages.routeBackend(route)
        )
            return
        val current = connection.group
        if (current != null && target.groupId == current.id) {
            confirm(playerId, request, target)
            return
        }
        val callGroup = groups.findLocal(target.groupId) ?: groups.apply(definition) ?: return
        if (!isCurrent(playerId, request)) return
        cancelRestore.accept(playerId)
        applying.add(playerId)
        try {
            connection.group = callGroup
        } catch (_: Exception) {
        ...
        try {
            Bukkit.getScheduler().runTask(plugin, Runnable { confirm(playerId, request, target) })
        } catch (_: Exception) {
            /* Plugin is stopping. */
        }
    }
    ...
    private fun confirm(playerId: UUID, request: Request, target: VoiceMessages.CallTarget) {
        ...
        if (
            connection == null ||
                !connection.isConnected ||
                backend != VoiceMessages.routeBackend(routeLookup.apply(playerId)) ||
                group == null ||
    ...
    private fun isCurrent(playerId: UUID, request: Request) =
        !closed &&
            sessionLookup.apply(playerId) == request.session &&
            requestGenerations[playerId] == request.generation

    private fun clearTarget(playerId: UUID, target: VoiceMessages.CallTarget, reconcileAfter: Boolean) {
        bus.clearCallTarget(
            playerId,
            target,
            routeLookup.apply(playerId),
            Runnable { if (reconcileAfter) reconcileMembership.accept(playerId) },
        )
    }

    private fun invalidateRequests(playerId: UUID) {
        requestGenerations.merge(playerId, 1L, Long::plus)
    }
    ...
    companion object {
        ...
        @JvmStatic
        fun isCallGroup(definition: VoiceMessages.GroupDefinition?) =
            definition != null &&
                definition.name == CALL_GROUP_NAME &&
                !definition.password.isNullOrBlank() &&
                definition.type == Group.Type.OPEN &&
                definition.hidden &&
                !definition.permanent

        @JvmStatic
        fun isNewAcceptedTarget(
            target: VoiceMessages.CallTarget?,
            hinted: VoiceMessages.CallTarget?,
            suppressed: VoiceMessages.CallTarget?,
        ) = target != null && target == hinted && target != suppressed

        @JvmStatic
        fun manualSuppressionTarget(active: VoiceMessages.CallTarget?, hinted: VoiceMessages.CallTarget?) =
            hinted ?: active
    }
}
