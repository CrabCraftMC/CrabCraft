...
/** Protects blocks, containers and entities from unverified players, with live permissions. */
class RestrictedAreaListener(
    private val plugin: CrabUtilities?,
    @Volatile private var settings: RestrictedAreaSettings,
) : Listener {
    ...
    fun refresh() {
        settings =
            try {
                RestrictedAreaSettings.load(plugin!!.config)
            } catch (exception: IllegalArgumentException) {
                plugin!!.logger.severe("Unverified-player protection disabled: ${exception.message}")
                RestrictedAreaSettings.disabled()
            }
    }

    @EventHandler(priority = EventPriority.MONITOR)
    fun onQuit(event: PlayerQuitEvent) {
        reminder.forget(event.player.uniqueId)
    }

    @EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = true)
    fun onInteract(event: PlayerInteractEvent) {
        if (!isRestricted(event.player)) return
        ...
        event.setUseInteractedBlock(Event.Result.DENY)
        val blockedItem = event.hasItem() && !isPersonalItem(event.material)
        if (action == Action.RIGHT_CLICK_BLOCK && blockedItem) event.setUseItemInHand(Event.Result.DENY)
        if (action == Action.LEFT_CLICK_BLOCK || blockedItem || event.clickedBlock?.type?.isInteractable == true)
            remind(event.player)
    }
    ...
    fun onInventoryOpen(event: InventoryOpenEvent) {
        if (!isPersonalInventory(event.inventory.type)) cancel(event, player(event.player))
    }

    @EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = true)
    fun onInventoryClick(event: InventoryClickEvent) {
        if (!isPersonalInventory(event.view.type)) cancel(event, player(event.whoClicked))
    }
    ...
    fun onTarget(event: EntityTargetLivingEntityEvent) {
        ...
        if (isRestricted(player)) event.isCancelled = true
    }

    @EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = true)
    fun onProjectileLaunch(event: ProjectileLaunchEvent) {
        val shooter = responsiblePlayer(event.entity as Entity)
        if (isRestricted(shooter) && event.entity !is EnderPearl) {
    ...
    fun onPotionSplash(event: PotionSplashEvent) {
        if (isRestricted(responsiblePlayer(event.potion as Entity))) event.isCancelled = true
    }

    @EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = true)
    fun onAreaEffectCloud(event: AreaEffectCloudApplyEvent) {
        if (isRestricted(responsiblePlayer(event.entity.source))) event.isCancelled = true
    }

    private fun isRestricted(player: Player?) = isRestricted(player, settings)
    ...
    private fun remind(player: Player) {
        reminder.send(player.uniqueId, player)
    }

    companion object {
        @JvmStatic
        fun isRestricted(player: Player?, current: RestrictedAreaSettings) =
            player != null && current.enabled() && !player.isOp && !player.hasPermission(current.permission())

        @JvmStatic
        fun isPersonalItem(material: Material) =
            material.isEdible ||
        ...
        @JvmStatic
        fun isPersonalInventory(type: InventoryType) =
        ...
        private fun responsiblePlayer(entity: Entity?): Player? =
            when (entity) {
                is Player -> entity
                is Projectile -> responsiblePlayer(entity.shooter)
                is Tameable -> player(entity.owner)
                else -> null
            }
