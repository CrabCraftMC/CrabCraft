...
object StaffChatComponents {
    ...
    @JvmStatic fun hasPrefix(message: Component): Boolean = PLAIN.serialize(message).startsWith("#")
    ...
    @JvmStatic fun isEmpty(message: Component): Boolean = PLAIN.serialize(message).all { Character.isWhitespace(it) }
}
