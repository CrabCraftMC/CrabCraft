...
class OpusVolumeScaler(private val api: VoicechatServerApi, gain: Double) : AutoCloseable {
    ...
    private fun scale(speakerId: UUID, sequence: Long, cacheBySequence: Boolean, opus: ByteArray?): ByteArray? {
        ...
        val state =
            speakers.computeIfAbsent(speakerId) {
                SpeakerState(api.createDecoder(), api.createEncoder(OpusEncoderMode.VOIP))
            }
        synchronized(state) {
    ...
    fun remove(speakerId: UUID) {
        speakers.remove(speakerId)?.close()
    }
    ...
    companion object {
        @JvmStatic
        fun applyGain(samples: ShortArray, gain: Double) {
            val bounded = clampGain(gain)
            for (i in samples.indices) {
                val scaled = Math.round(samples[i] * bounded)
                samples[i] = scaled.coerceIn(Short.MIN_VALUE.toLong(), Short.MAX_VALUE.toLong()).toShort()
            }
        }
