...
class LofiStreamPlayer(
    ...
    private val executor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "CrabUtilities-Lofi").apply { isDaemon = true }
    }
    ...
    fun openChannel(): Boolean {
        ...
        opened.setFilter { serverPlayer ->
            api.getConnectionOf(serverPlayer.getUuid())?.getGroup()?.getId() == groupId
        }
        channel = opened
        return true
    }
    ...
    fun reconcileTarget(connection: VoicechatConnection?) {
        ...
        updateTarget(connection, connection.getGroup()?.getId())
    }
            ...
            try {
                ...
                if (opened == null) {
                    if (!resolutionFailureLogged) {
                        logger.warning(
                            "Could not resolve the 24/7 Lofi stream; retrying every " +
