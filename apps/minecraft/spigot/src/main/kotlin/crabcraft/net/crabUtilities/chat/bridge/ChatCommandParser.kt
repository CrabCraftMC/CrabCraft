...
object ChatCommandParser {
    ...
    data class Parsed(private val type: Type, private val target: String?, private val message: String?) {
        ...
        fun valid(): Boolean =
            message != null && !message.all { Character.isWhitespace(it) } && (type != Type.PRIVATE || target != null)
    }
    ...
    private fun parseTarget(input: String, start: Int): ParsedTarget? {
        if (input[start] != '"') {
            val separator = firstWhitespace(input, start)
            val end = if (separator < 0) input.length else separator
    ...
    private fun firstWhitespace(input: String, start: Int): Int {
        for (i in start until input.length) if (Character.isWhitespace(input[i])) return i
        return -1
    }
    ...
    private data class ParsedTarget(val value: String, val end: Int)
}
