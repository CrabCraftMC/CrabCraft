...
class JukeboxPlaybackListener : Listener {
    ...
    fun onPlayerUsesJukebox(event: PlayerInteractEvent) {
        ...
        if (event.player.isSneaking && !(held ?: ItemStack.empty()).isEmpty) return
    ...
    fun onHopperInsert(event: InventoryMoveItemEvent) {
        ...
        if (
            block.type != Material.JUKEBOX ||
                ...
                !MediaItemCodec.isDisc(event.item)
        )
            return
        val expected = event.item.asOne()
        Bukkit.getScheduler()
            .runTaskLater(MediaFeature.get().getJavaPlugin(), Runnable { beginIfTransferArrived(block, expected) }, 1L)
    }

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    fun onHopperRemove(event: InventoryMoveItemEvent) {
        val source = event.source.location ?: return
        if (source.block.type != Material.JUKEBOX) return
        if (MediaItemCodec.isDisc(event.item)) AudioEngine.getInstance().stopPlaying(source.block)
    }

    @EventHandler(priority = EventPriority.NORMAL) fun onBlockBreak(event: BlockBreakEvent) = stopIfJukebox(event.block)
    ...
    fun onVanillaRecordExpiry(event: ItemSpawnEvent) {
        ...
        val jukeboxBlock =
            if (spawnBlock.type == Material.JUKEBOX) spawnBlock else spawnBlock.getRelative(BlockFace.DOWN)
    ...
    companion object {
        private fun insertByPlayer(event: PlayerInteractEvent, block: Block, held: ItemStack) {
            ...
            val placed = held.clone().apply { amount = 1 }
            held.subtract()
            val jukebox = block.state as Jukebox
        ...
        private fun beginIfTransferArrived(block: Block, expected: ItemStack) {
            ...
            if (
                !transferCompleted(actual.isEmpty, actual.isSimilar(expected)) ||
                    AudioEngine.getInstance().isPlaying(block)
            )
                return
        ...
        @JvmStatic
        fun transferCompleted(placedDiscEmpty: Boolean, placedDiscMatches: Boolean) =
