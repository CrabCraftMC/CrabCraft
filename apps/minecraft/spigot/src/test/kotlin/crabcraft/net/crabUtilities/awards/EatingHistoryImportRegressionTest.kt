            ...
            repeat(2) {
                ...
                val result =
                    EatingAwardTracker.decode(
                        imported
                            .getCompound("BukkitValues")
                            .orElseThrow()
                            .getString(EatingAwardTracker.DATA_KEY.toString())
                            .orElseThrow()
                    )
                check(
                    result.scores(progress.cakeSlicesAtStart + 1)["eat_veggie"] == baseline + meals + 1,
                    "Import/retry discarded tracked meals or double-counted the baseline",
                )
                check(
                    imported.getString("unrelated") == playerData.getString("unrelated"),
                    "Unrelated player data changed",
                )
            }
            Files.list(world).use { backups ->
                for (backup in
                    backups.filter { it.fileName.toString().startsWith("eating-history-backup-") }.toList()) {
