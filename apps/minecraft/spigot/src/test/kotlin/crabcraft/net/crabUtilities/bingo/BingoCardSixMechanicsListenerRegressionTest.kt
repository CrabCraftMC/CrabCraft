...
object BingoCardSixMechanicsListenerRegressionTest {
    ...
    private fun classifiesVanillaFishingLoot() {
        check(
            BingoCardSixMechanicsListener.fishingCategory(
        ...
        check(
            BingoCardSixMechanicsListener.fishingCategory(Material.COD, false) == 0,
    ...
    private fun roundTripsEnchantedMaterialProgress() {
        ...
        check(
            BingoCardSixMechanicsListener.decodeMaterials(encoded).equals(materials),
            "Five distinct enchanted item materials must survive persistent encoding",
        )
        check(
            BingoCardSixMechanicsListener.decodeMaterials("not_a_material,,BOW").equals(EnumSet.of(Material.BOW)),
    ...
    private fun requiresFourDistinctPotterySherds() {
        ...
        check(
            BingoCardSixMechanicsListener.hasFourDistinctSherdMaterials(distinct),
        ...
        check(
            !BingoCardSixMechanicsListener.hasFourDistinctSherdMaterials(repeated),
        ...
        check(
            !BingoCardSixMechanicsListener.hasFourDistinctSherdMaterials(withBrick),
    ...
    private fun acceptsOnlyResultTakeActions() {
        check(
            BingoCardSixMechanicsListener.isResultTakeAction(InventoryAction.PICKUP_ALL),
            "Picking up a result must count",
        )
        check(
            BingoCardSixMechanicsListener.isResultTakeAction(InventoryAction.MOVE_TO_OTHER_INVENTORY),
            "Shift-clicking a result must arm a post-transfer confirmation",
        )
        check(
            !BingoCardSixMechanicsListener.isResultTakeAction(InventoryAction.PLACE_ALL),
            "Placing into a slot must not look like taking a result",
        )
        check(
            !BingoCardSixMechanicsListener.canChangeInventory(InventoryAction.NOTHING),
            "A no-op cannot trigger an Ender Chest confirmation",
        )
        check(
            BingoCardSixMechanicsListener.isDropResultAction(InventoryAction.DROP_ALL_SLOT),
    ...
    private fun requiresTheCraftedResultToReachThePlayer() {
        check(
            BingoCardSixMechanicsListener.resultCountIncreased(1, 2),
            "A newly received Smithing result must confirm success",
        )
        check(
            !BingoCardSixMechanicsListener.resultCountIncreased(1, 1),
            "Closing a failed full-inventory craft must not look successful",
        )
        check(
            !BingoCardSixMechanicsListener.resultCountIncreased(2, 1),
    ...
    private fun confirmsEveryRecipeInputWasActuallyConsumed() {
        val before =
            arrayOf<BingoCardSixMechanicsListener.StackSnapshot?>(
        ...
        check(
            BingoCardSixMechanicsListener.everyInputWasConsumed(before, consumed),
            "A successful craft must consume at least one of every input",
        )
        check(
            !BingoCardSixMechanicsListener.everyInputWasConsumed(before, before),
        ...
        check(
            !BingoCardSixMechanicsListener.everyInputWasConsumed(before, partlyConsumed),
