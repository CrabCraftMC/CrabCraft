...
object SimpleVoiceAnimationsProtocolRegressionTest {
    ...
    fun main(args: Array<String>) {
        val preferences =
            SimpleVoiceAnimationsIntegration.decodePreferences(
                ByteBuffer.allocate(9).put(4).putFloat(2.5F).putFloat(1.25F).array()
            )
        ...
        val clamped =
            SimpleVoiceAnimationsIntegration.decodePreferences(
                ByteBuffer.allocate(9).put(1).putFloat(Float.POSITIVE_INFINITY).putFloat(Float.NaN).array()
            )
        check(clamped.splitHeight == 0.5F, "non-finite split height did not use the mod fallback")
        check(clamped.intensity == 1F, "non-finite intensity did not use the mod fallback")
        ...
        val actual =
            SimpleVoiceAnimationsIntegration.encodePlayerPreferences(
                UUID.fromString("00112233-4455-6677-8899-aabbccddeeff"),
                SimpleVoiceAnimationsIntegration.Preferences(5, 2F, 1.25F),
            )
