...
object RosterLifecycleRegressionTest {
    ...
    private val plugin = proxy(Plugin::class.java) { _, _ -> null }

    @JvmStatic
    fun main(args: Array<String>) {
        val scheduler =
            proxy(BukkitScheduler::class.java) { method, arguments ->
        ...
        Bukkit::class
            ...
            .set(
                null,
                proxy(Server::class.java) { method, arguments ->
                    when (method) {
                        ...
                        "getName",
                        "getVersion",
                        "getBukkitVersion" -> "voice-regression"
                        else -> null
                    }
                },
            )
    ...
    private fun delayedLeaveCannotRemoveReturnToSameBackend() {
        ...
        drain()
        check(SPEAKER in fixture.packets.states, "a delayed leave from the previous visit removed the return hop")
    ...
    private fun leaveStopsAudioAndRemovesState() {
        ...
        fixture.roster.onLifecycleMessage(VoiceMessages.encodeRosterLeave(GROUP, SPEAKER, FIRST_ROUTE))
        drain()
        check(SPEAKER !in fixture.packets.states, "departed speaker remained in the client roster")
    ...
    private fun catchUpCannotResurrectQueuedLeave() {
        ...
        drain()
        check(fixture.packets.states[SPEAKER] == GROUP, "catch-up lost the latest hop")
    ...
    private fun groupMoveStopsOldAudio() {
        ...
        fixture.roster.onLifecycleMessage(VoiceMessages.encodeRosterLeave(GROUP, SPEAKER, FIRST_ROUTE))
        drain()
        check(fixture.packets.states[SPEAKER] == nextGroup, "old-group leave removed the new group")
    ...
    private fun legacyRosterStillDecodes() {
        ...
        check(legacy?.backend() == "survival", "legacy backend-only roster stopped decoding")
        val current =
            VoiceMessages.decodeRosterJoin(VoiceMessages.encodeRosterJoin(GROUP, SPEAKER, "Crab", FIRST_ROUTE))
        check(current?.route == FIRST_ROUTE, "roster discarded its hop token")
    }
    ...
    private fun audioSequenceSurvivesRosterChange(nextGroup: UUID, nextRoute: String, leaveFirst: Boolean) {
        ...
        val api =
            proxy(VoicechatServerApi::class.java) { method, arguments ->
                when (method) {
                    "getConnectionOf" -> if (arguments!![0] == SPEAKER) null else connection(arguments[0] as UUID)
        ...
        try {
            ...
            sendAudio(relay, GROUP, FIRST_ROUTE)
            ...
            check(
                movedListener.accepted - before == 10,
                "listener rejected audio after missing the old group's reset (leave=$leaveFirst, route=${VoiceMessages.routeBackend(nextRoute)})",
            )
        } finally {
            relay.shutdown()
        }
    ...
    private fun seedRouteCache(relay: AudioRelay, route: String) {
        ...
        val constructor =
            cacheType.getDeclaredConstructor(String::class.java, Long::class.javaPrimitiveType).apply {
                isAccessible = true
            }
        val cache = AudioRelay::class.java.getDeclaredField("playerHomeCache").apply { isAccessible = true }
        (cache.get(relay) as MutableMap<UUID, Any>)[SPEAKER] =
    ...
    private fun sequenceChannel(
        clients: Map<UUID, SequenceClient>,
        channelTargets: MutableList<Set<UUID>>,
    ): StaticAudioChannel {
        ...
        return proxy(StaticAudioChannel::class.java) { method, arguments ->
            when (method) {
                ...
                "flush" -> {
                    ...
                    for (target in targets) clients.getValue(target).receive(current, data)
                }
            }
            null
        }
    ...
    private fun player(id: UUID): Player =
        proxy(Player::class.java) { method, _ ->
            when (method) {
                "getUniqueId" -> id
                "getName" -> "Listener"
                "isOnline" -> true
    ...
    private class Fixture {
        ...
        val roster = RosterTracker(plugin, packets, "lobby", Logger.getAnonymousLogger(), { invalidated.add(it) })
