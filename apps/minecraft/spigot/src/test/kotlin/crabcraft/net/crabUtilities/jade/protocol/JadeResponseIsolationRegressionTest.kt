...
object JadeResponseIsolationRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        val requestData = identity.copy()
        requestData.putByteArray("minecraft:item_storage", byteArrayOf(1, 2, 3))
        ...
        val response = JadeProtocol.createResponseTag(identity, requestData)

        check(
            response.getIntOr("x", 0) == 12 && response.getIntOr("y", 0) == 64 && response.getIntOr("z", 0) == -8,
            "Jade response lost its trusted target identity",
        )
        check(
            response.getBooleanOr("SortItems", false),
            "Jade response discarded the supported item sorting request",
        )
        check(
            !response.contains("minecraft:item_storage"),
            "Jade response retained stale chest contents",
        )
        check(
            !response.contains("Loot") && !response.contains("Locked"),
            "Jade response retained stale container state",
        )
        check(
            !response.contains("untrusted-provider"),
