...
object StatsPushPathRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        check(
            playersDirectory == levelDirectory.resolve("players"),
            "player storage was not resolved from the level directory",
        )
        check(
            !playersDirectory.startsWith(dimensionDirectory),
            "player storage was incorrectly resolved inside the dimension directory",
        )
        check(
            playersDirectory.resolve("stats") == levelDirectory.resolve("players/stats"),
            "stats directory does not match the Minecraft 26.1 layout",
        )
        check(
            playersDirectory.resolve("advancements") == levelDirectory.resolve("players/advancements"),
            "advancements directory does not match the Minecraft 26.1 layout",
        )
        check(
            playersDirectory.resolve("data") == levelDirectory.resolve(LevelResource.PLAYER_DATA_DIR.id()),
