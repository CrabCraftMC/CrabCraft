...
object BingoCardFiveChallengeListenerRegressionTest {
    ...
    private fun endermanMarkerSurvivesOnlyPermittedDamage() {
        check(
    ...
    private fun checksCrossbowBreakBoundariesWithoutOverflow() {
        check(
        ...
        check(
                BingoCardFiveChallengeListener.ChallengePolicy.damageBreaksItem(
                        Int.MAX_VALUE - 5, Int.MAX_VALUE, 10),
                "Durability addition must not overflow before the break comparison")
    }

    private fun requiresAnActualThrownChickenHatch() {
        check(
    ...
    private fun keepsPersistentAttributionOnlyForTheSameCard() {
        check(
    ...
    private fun expiresPersistentAttributionAcrossTimeAndPlayerResetBoundaries() {
        val now = 10_000L
        check(
