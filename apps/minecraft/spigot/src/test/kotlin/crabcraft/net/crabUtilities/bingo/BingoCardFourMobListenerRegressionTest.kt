...
object BingoCardFourMobListenerRegressionTest {
    @JvmStatic
    fun main(args: Array<String>) {
        sulfurCubeBucketRequiresEveryCommittedPrecondition()
        silverfishInfestationRequiresAnExactHostPair()
        powderSnowIntersectionExcludesTouchingFaces()
        attributionExpiresAcrossTimeAndLifecycleBoundaries()
    }

    private fun sulfurCubeBucketRequiresEveryCommittedPrecondition() {
        check(
    ...
    private fun silverfishInfestationRequiresAnExactHostPair() {
        check(
    ...
    private fun powderSnowIntersectionExcludesTouchingFaces() {
        val skeleton = BoundingBox(0.2, 0.0, 0.2, 0.8, 1.99, 0.8)
        check(
        ...
        val exactBlock = BoundingBox(0.0, 0.0, 0.0, 1.0, 1.0, 1.0)
        check(
    ...
    private fun attributionExpiresAcrossTimeAndLifecycleBoundaries() {
        val now = 10_000L
        check(
