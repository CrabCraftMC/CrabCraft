...
object BingoCardSixMobListenerRegressionTest {
    ...
    private fun acceptsOnlyStoredMendingBooks() {
        check(
            BingoCardSixMobListener.isMendingBook(Material.ENCHANTED_BOOK, true),
            "An Enchanted Book with stored Mending must be accepted",
        )
        check(
            !BingoCardSixMobListener.isMendingBook(Material.ENCHANTED_BOOK, false),
            "An ordinary Enchanted Book must not count",
        )
        check(
            !BingoCardSixMobListener.isMendingBook(Material.DIAMOND_PICKAXE, true),
            "A Mending tool is not the requested Mending Book",
        )
    }

    private fun requiresLethalPositiveLavaDamage() {
        check(
            BingoCardSixMobListener.itemDamageIsLethal(5, 5.0, 5.0),
            "Damage equal to item health must be lethal",
        )
        check(
            BingoCardSixMobListener.itemDamageIsLethal(5, 4.5, 4.5),
            "Paper's integer item-health cast makes a 0.5 remainder lethal",
        )
        check(
            !BingoCardSixMobListener.itemDamageIsLethal(5, 4.0, 4.0),
            "A book surviving with one health must not complete",
        )
        check(
            BingoCardSixMobListener.itemDamageIsLethal(4, 4.0, 1.0),
            "Paper kills from original NMS damage even when a plugin lowers event damage",
        )
        check(
            !BingoCardSixMobListener.itemDamageIsLethal(5, 5.0, 0.0),
            "An event reduced to zero damage is aborted by Paper",
        )
        check(
            !BingoCardSixMobListener.itemDamageIsLethal(5, Double.NaN, 5.0),
            "Non-finite damage must be rejected",
        )
    }

    private fun acceptsOnlyCarpetActionsThatCanReachTheLlamaDecorSlot() {
        check(
            BingoCardSixMobListener.clickCanDecorateLlama(
                1,
                2,
    ...
    private fun verifiesTheRavagerActuallyBecameStunned() {
        check(
            BingoCardSixMobListener.canStartRavagerStun(true, true, 0),
            "A raised Shield against an unstunned Ravager must arm verification",
        )
        check(
            !BingoCardSixMobListener.canStartRavagerStun(false, true, 0),
            "Merely holding a Shield must not arm the task",
        )
        check(
            !BingoCardSixMobListener.canStartRavagerStun(true, false, 0),
            "A custom blocking item plus an idle Shield must not arm the task",
        )
        check(
            !BingoCardSixMobListener.canStartRavagerStun(true, true, 20),
            "An already-stunned Ravager must not be attributed again",
        )
        check(
            BingoCardSixMobListener.becameStunned(0, 39),
            "A positive post-attack stun timer proves success",
        )
        check(
            !BingoCardSixMobListener.becameStunned(0, 0),
            "A failed random stun attempt must not complete",
        )
    }

    private fun resolvesPaperSplashPotionSources() {
        check(
            BingoCardSixMobListener.splashSourceCanResolvePlayer(true, false),
            "Paper reports the owning Player directly as a splash-potion effect source",
        )
        check(
            BingoCardSixMobListener.splashSourceCanResolvePlayer(false, true),
            "A projectile source with a Player shooter must remain supported",
        )
        check(
            !BingoCardSixMobListener.splashSourceCanResolvePlayer(false, false),
            "An unattributed splash source must not receive player credit",
        )
    }

    private fun acceptsOnlySuccessfulPoisonApplications() {
        check(
            BingoCardSixMobListener.poisonApplicationAccepted(
    ...
    private fun isolatesPersistentMarkersByCardResetAndAge() {
        val now = 20_000L
        check(
            BingoCardSixMobListener.markerIsCurrent(
                6,
                6,
                19_000L,
                now,
    ...
    private fun defersPersistentCompletionsAcrossTheStartupCardGap() {
        check(
            BingoCardSixMobListener.shouldDeferPersistentCompletion(
                6,
                Int.MIN_VALUE,
                true,
                true,
