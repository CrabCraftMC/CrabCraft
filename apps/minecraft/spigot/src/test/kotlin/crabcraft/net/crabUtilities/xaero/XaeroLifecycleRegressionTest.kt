...
object XaeroLifecycleRegressionTest {
    @JvmStatic
    fun main(args: Array<String>) {
        ...
        check(
            XaeroIntegration.JOIN_SEND_DELAYS_TICKS == listOf(0L, 20L, 40L),
    ...
    private fun checkChannels() {
        check(
            XaeroIntegration.CHANNELS == listOf("xaerominimap:main", "xaeroworldmap:main"),
            "Xaero channel names changed",
        )
        for (channel in XaeroIntegration.CHANNELS) check(
            XaeroIntegration.isXaeroChannel(channel),
    ...
    private fun checkEncoding() {
        check(
            XaeroIntegration.encodeServerId(0x01020304).contentEquals(byteArrayOf(0, 1, 2, 3, 4)),
            "positive server id was not encoded as a type byte and big-endian int",
        )
        check(
            XaeroIntegration.encodeServerId(-1).contentEquals(byteArrayOf(0, -1, -1, -1, -1)),
    ...
    private fun checkJoinRetryGuards() {
        ...
        check(
            !XaeroIntegration.canRetryJoinSend(null, joinedPlayer, expectedWorldId),
            "a disconnected session received a join retry",
        )
        check(
            !XaeroIntegration.canRetryJoinSend(player(true, expectedWorldId), joinedPlayer, expectedWorldId),
            "a replacement session received an old join retry",
        )
        val offlinePlayer = player(false, expectedWorldId)
        check(
            !XaeroIntegration.canRetryJoinSend(offlinePlayer, offlinePlayer, expectedWorldId),
            "an offline player received a join retry",
        )
        val changedWorldPlayer = player(true, UUID(0L, 2L))
        check(
            !XaeroIntegration.canRetryJoinSend(changedWorldPlayer, changedWorldPlayer, expectedWorldId),
            "a player in a different world received a stale join retry",
        )
        check(
            XaeroIntegration.canRetryJoinSend(joinedPlayer, joinedPlayer, expectedWorldId),
