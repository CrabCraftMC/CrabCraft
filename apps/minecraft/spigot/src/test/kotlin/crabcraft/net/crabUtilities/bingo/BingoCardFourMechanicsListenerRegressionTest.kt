...
object BingoCardFourMechanicsListenerRegressionTest {
    @JvmStatic
    fun main(args: Array<String>) {
        ...
        val twelvePushedAndOneBroken = ArrayList(Collections.nCopies(12, PistonMoveReaction.MOVE))
        twelvePushedAndOneBroken.add(PistonMoveReaction.BREAK)
        check(
            BingoCardFourMechanicsListener.countPushedReactions(twelvePushedAndOneBroken) == 12,
        ...
        check(
            owner.equals(BingoCardFourMechanicsListener.singleDistinctOwner(listOf(owner, owner))),
            "Duplicate evidence for one player must remain attributable",
        )
        check(
            BingoCardFourMechanicsListener.singleDistinctOwner(listOf(owner, UUID.randomUUID())) == null,
