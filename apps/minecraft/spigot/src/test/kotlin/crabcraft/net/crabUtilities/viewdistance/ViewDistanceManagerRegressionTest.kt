...
object ViewDistanceManagerRegressionTest {
    ...
    private fun checkTickDurationMedian() {
        ...
        window.clear()
        check(window.median(25.0) == 25.0, "cleared MSPT window retained stale samples")
        window.add(42.0)
        check(window.median(25.0) == 42.0, "reused MSPT window included samples from before the clear")
    }

    private fun checkAdjustmentHysteresis() {
        val policy = ViewDistanceManager.AdjustmentPolicy()
        val worldId = UUID.randomUUID()
        for (index in 1 until ViewDistanceManager.PASSED_CHECKS_FOR_INCREASE) {
            check(
                policy.choose(worldId, ViewDistanceManager.INCREASE_MSPT_THRESHOLD) ==
        ...
        check(
            policy.choose(worldId, 25.0) == ViewDistanceManager.Adjustment.INCREASE,
        ...
        check(
            policy.choose(worldId, 25.0) == ViewDistanceManager.Adjustment.STAY,
            "neutral MSPT did not reset the increase streak",
        )
        check(
            policy.choose(worldId, ViewDistanceManager.DECREASE_MSPT_THRESHOLD) ==
                ViewDistanceManager.Adjustment.DECREASE,
            "high MSPT did not decrease the distance immediately",
        )
        check(
            policy.choose(worldId, Double.NaN) == ViewDistanceManager.Adjustment.STAY,
            "invalid MSPT caused an adjustment",
        )
    }

    private fun checkAdjustmentSwitch() {
        val control = ViewDistanceManager.AdjustmentSwitch()
        check(!control.isPaused(), "dynamic adjustment started paused")
        check(!control.resume(), "running adjustment reported a resume transition")
        check(control.pause(), "running adjustment could not be paused")
        check(control.isPaused(), "pause transition did not persist")
        check(!control.pause(), "paused adjustment reported a second pause transition")
        check(control.resume(), "paused adjustment could not be resumed")
        check(!control.isPaused(), "resume transition did not persist")
    }
    ...
    private fun checkCommandSurface() {
        ...
        check(
            completions == listOf("status", "set", "pause", "resume"),
    ...
    private fun commandSender(permitted: Boolean, messages: MutableList<String>): CommandSender =
        Proxy.newProxyInstance(
            ...
            when (method.name) {
                ...
                "getName",
                "toString" -> "view-distance test sender"
                "equals" -> proxy === args!![0]
    ...
    private fun checkAdvertisedRadiusRewrite() {
        ...
        check(
            rewritten is ClientboundSetChunkCacheRadiusPacket,
            "view-radius packet was replaced with a different packet type",
        )
        check(
            (rewritten as ClientboundSetChunkCacheRadiusPacket).radius == 16,
        ...
        check(
            ClientViewRadiusBridge.keepAdvertisedRadius(matching, 16) === matching,
            "matching advertised radius caused an unnecessary packet replacement",
        )
        val unrelated = Any()
        check(
            ClientViewRadiusBridge.keepAdvertisedRadius(unrelated, 16) === unrelated,
    ...
    private fun checkClientBridgeLifecycle() {
        ...
        for (method in ClientViewRadiusBridge::class.java.declaredMethods) {
            ...
            check(
                eventType != PlayerLoginEvent::class.java,
                "view-radius bridge attaches before the player connection is available",
            )
            handlesJoin = handlesJoin or (eventType == PlayerJoinEvent::class.java)
        }
    ...
    private fun checkConfigurationContractAndAttribution() {
        ...
        check(
            settings.simulationBounds() == ViewDistanceManager.Bounds(6, 12),
        ...
        check(
            settings.simulationBounds() == ViewDistanceManager.Bounds(4, 10),
        ...
        check(
            pluginYml.contains("<reload|update|viewdistance>"),
    ...
    private fun readResource(name: String): String =
        ViewDistanceManagerRegressionTest::class.java.classLoader.getResourceAsStream(name).use {
            check(it != null, "bundled resource is missing: $name")
            String(it!!.readAllBytes(), StandardCharsets.UTF_8)
        }

    private class WorldStub(simulation: Int, view: Int) {
        val simulationDistance = AtomicInteger(simulation)
        val viewDistance = AtomicInteger(view)
        val simulationWrites = AtomicInteger()
        val viewWrites = AtomicInteger()
        val world =
            Proxy.newProxyInstance(World::class.java.classLoader, arrayOf(World::class.java)) { proxy, method, args ->
                when (method.name) {
                    "getSimulationDistance" -> simulationDistance.get()
                    "setSimulationDistance" -> {
                        simulationDistance.set(args!![0] as Int)
                        simulationWrites.incrementAndGet()
                        null
                    }
                    "getViewDistance" -> viewDistance.get()
                    "setViewDistance" -> {
                        viewDistance.set(args!![0] as Int)
                        viewWrites.incrementAndGet()
                        null
                    }
                    "equals" -> proxy === args!![0]
                    "hashCode" -> System.identityHashCode(proxy)
                    "toString" -> "view-distance world"
