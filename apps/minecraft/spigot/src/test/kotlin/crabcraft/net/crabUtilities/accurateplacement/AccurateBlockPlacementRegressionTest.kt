...
object AccurateBlockPlacementRegressionTest {
    @JvmStatic
    ...
    private fun protocolCursorIsDecoded() {
        check(
            AccurateBlockPlacementManager.decodeProtocolValue(1.999F) == -1,
            "a vanilla cursor was treated as an accurate-placement packet",
        )
        check(
            AccurateBlockPlacementManager.decodeProtocolValue(2.0F) == 0,
            "the first encoded protocol value was decoded incorrectly",
        )
        check(
            AccurateBlockPlacementManager.decodeProtocolValue(4.0F) == 1,
        ...
        check(
            AccurateBlockPlacementManager.decodeProtocolValue(Float.POSITIVE_INFINITY) == -1,
            "a non-finite cursor was accepted",
        )
    }

    private fun paperSequenceBridgeIsAvailable() {
        AccurateBlockPlacementPaperIntegration.verify()
    }

    private fun requestedDirectionsAndAxesAreDecoded() {
        val faces =
            arrayOf(BlockFace.DOWN, BlockFace.UP, BlockFace.NORTH, BlockFace.SOUTH, BlockFace.WEST, BlockFace.EAST)
        for (index in faces.indices) check(
            AccurateBlockPlacementManager.faceFor(index) == faces[index],
            "direction index $index changed",
        )
        check(
            AccurateBlockPlacementManager.faceFor(6) == null,
    ...
    private fun packetPositionMatchesEitherPlacementBlock() {
        val packet = AccurateBlockPlacementManager.PacketData(12, 80, -4, 3, 10, System.nanoTime())
    ...
    private fun pendingPacketsPreserveOrder() {
        ...
        check(
            firstSameTarget != null && firstSameTarget.protocolValue() == 3,
            "the first same-target interaction was not consumed in order",
        )
        check(
            secondSameTarget != null && secondSameTarget.protocolValue() == 5,
    ...
    private fun rejectedPacketsAreDiscarded() {
        ...
        check(
            packets.takeMatching(41, block(4, 64, 4), now) === accepted,
            "a rejected packet remained ahead of the accepted interaction",
        )
        check(
            packets.takeMatching(41, block(4, 64, 4), now) == null,
    ...
    private fun candleAirPlacementRequiresAccurateFreshContext() {
        ...
        check(
            AccurateBlockPlacementManager.shouldAllowCandleAirPlacement(true, Material.CANDLE, accurate, now),
            "a fresh accurate candle placement was not allowed",
        )
        check(
            !AccurateBlockPlacementManager.shouldAllowCandleAirPlacement(false, Material.CANDLE, accurate, now),
            "candle air placement ignored the disabled config",
        )
        check(
            !AccurateBlockPlacementManager.shouldAllowCandleAirPlacement(true, Material.STONE, accurate, now),
            "a non-candle was treated as air-placeable",
        )
        check(
            !AccurateBlockPlacementManager.shouldAllowCandleAirPlacement(true, Material.CANDLE, ordinary, now),
            "an ordinary candle placement bypassed its support check",
        )
        check(
            !AccurateBlockPlacementManager.shouldAllowCandleAirPlacement(true, Material.CANDLE, expired, now),
    ...
    private fun blockStatesAreApplied() {
        ...
        val directional =
            proxy(Directional::class.java) { _, method, args ->
                when (method.name) {
                    "getFacing" -> facing.get()
                    "setFacing" -> {
                        facing.set(args!![0] as BlockFace)
                        null
                    }
                    "getFaces" ->
                        setOf(
        ...
        val hopper =
            proxy(Directional::class.java) { _, method, args ->
                when (method.name) {
                    ...
                    "getFaces" ->
                        setOf(BlockFace.DOWN, BlockFace.NORTH, BlockFace.SOUTH, BlockFace.WEST, BlockFace.EAST)
                    else -> defaultValue(method.returnType)
                }
            }
        ...
        val repeater =
            proxy(Repeater::class.java) { _, method, args ->
                when (method.name) {
                    "getMinimumDelay" -> 1
                    "getMaximumDelay" -> 4
                    "setDelay" -> {
                        delay.set(args!![0] as Int)
                        null
                    }
                    else -> defaultValue(method.returnType)
                }
        ...
        val comparator =
            proxy(Comparator::class.java) { _, method, args ->
                when (method.name) {
                    "setMode" -> {
                        mode.set(args!![0] as Comparator.Mode)
                        null
                    }
                    else -> defaultValue(method.returnType)
                }
        ...
        val bisected =
            proxy(Bisected::class.java) { _, method, args ->
                when (method.name) {
                    "setHalf" -> {
                        half.set(args!![0] as Bisected.Half)
                        null
                    }
                    else -> defaultValue(method.returnType)
                }
    ...
    private fun stairHalfIsAppliedBeforeShape() {
        ...
        val placed =
            proxy(Stairs::class.java) { _, method, args ->
                when (method.name) {
                    "getHalf" -> half.get()
                    "setHalf" -> {
                        half.set(args!![0] as Bisected.Half)
                    ...
                    "setShape" -> {
                        shape.set(args!![0] as Stairs.Shape)
                        null
                    }
                    else -> defaultValue(method.returnType)
                }
        ...
        val placedBlock =
            proxy(Block::class.java) { _, method, args ->
                when (method.name) {
    ...
    private fun assertRulePayload(payload: ByteArray, expectedValue: String) {
        ...
        check(
            tags == mapOf("Value" to expectedValue, "Manager" to "carpet", "Rule" to "accurateBlockPlacement"),
    ...
    private fun block(x: Int, y: Int, z: Int): Block =
    ...
    private fun block(data: BlockData?): Block =
        proxy(Block::class.java) { _, method, _ ->
            when (method.name) {
                "getBlockData" -> data
                else -> defaultValue(method.returnType)
            }
    ...
    private fun stair(half: Bisected.Half, facing: BlockFace): Stairs =
        proxy(Stairs::class.java) { _, method, _ ->
            when (method.name) {
                "getHalf" -> half
                "getFacing" -> facing
                else -> defaultValue(method.returnType)
