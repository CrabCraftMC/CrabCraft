...
object BingoCardSixWorldListenerRegressionTest {
    ...
    private fun enumeratesExactlyTheVanillaConduitFrame() {
        var offsets = 0
        for (x in -3..3) {
            for (y in -3..3) {
                for (z in -3..3) {
        ...
        check(
            BingoCardSixWorldListener.isConduitFrameOffset(0, 2, 2),
            "A corner in the vertical ring must be a frame slot",
        )
        check(
            BingoCardSixWorldListener.isConduitFrameOffset(2, 0, -1),
            "An edge in the horizontal ring must be a frame slot",
        )
        check(
            !BingoCardSixWorldListener.isConduitFrameOffset(2, 2, 2),
            "A cube corner is not part of the Conduit frame",
        )
        check(
            !BingoCardSixWorldListener.isConduitFrameOffset(1, 0, 1),
            "The central three-by-three volume is not part of the frame",
        )
    }

    private fun recognizesArrowProjectilesOnly() {
        check(
    ...
    private fun requiresThePlayersFinalFrameForMaximumConduitPower() {
        check(
    ...
    private fun keepsNamedGhastAttributionOnlyForTheCurrentCardAndPlayerRun() {
        val now = 10_000L
        check(
    ...
    private fun defersGhastConfirmationAcrossTheStartupCardGap() {
        check(
            BingoCardSixWorldListener.shouldDeferPersistentConfirmation(
                6,
                Int.MIN_VALUE,
                true,
                true,
        ...
        check(
            BingoCardSixWorldListener.persistentConfirmationNeedsContext(
                6,
                Int.MIN_VALUE,
                true,
            ),
