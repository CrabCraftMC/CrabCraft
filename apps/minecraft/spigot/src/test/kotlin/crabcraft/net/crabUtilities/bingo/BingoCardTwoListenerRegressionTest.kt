...
object BingoCardTwoListenerRegressionTest {
    @JvmStatic
    ...
    private fun directOffhandTransitionsAreCorrelated() {
        check(
            BingoCardTwoListener.transitionedAwayFromTrackedItem("helmet", "helmet", "empty", String::equals),
            "Clearing the tracked Piglin off-hand item must count as a transition",
        )
        check(
            BingoCardTwoListener.transitionedAwayFromTrackedItem("helmet", "helmet", "chestplate", String::equals),
            "Replacing equipped armour with the next pickup in the same tick must count as a transition",
        )
        check(
            !BingoCardTwoListener.transitionedAwayFromTrackedItem("helmet", "helmet", "helmet", String::equals),
            "Keeping the tracked item in the Piglin off hand must not count as a transition",
        )
        check(
            !BingoCardTwoListener.transitionedAwayFromTrackedItem("helmet", "chestplate", "empty", String::equals),
            "An unrelated previous off-hand item must not count as the tracked transition",
        )
    }

    private fun onlyTheMatchingSameTickEquipIsAttributed() {
        check(
    ...
    private fun everyArmourPieceMustHaveTheSamePlayerOwner() {
        ...
        val owners = EnumMap<EquipmentSlot, UUID>(EquipmentSlot::class.java)
        owners[EquipmentSlot.HEAD] = first
        owners[EquipmentSlot.CHEST] = first
        owners[EquipmentSlot.LEGS] = first
        owners[EquipmentSlot.FEET] = first
        check(
            BingoCardTwoListener.allArmourSlotsOwnedBy(owners, first),
            "One player dropping all four pieces must satisfy ownership",
        )
        owners[EquipmentSlot.FEET] = second
        check(
            !BingoCardTwoListener.allArmourSlotsOwnedBy(owners, first),
    ...
    private fun capturedFirePlacementRetainsIgnitionOwner() {
        ...
        check(
            BingoCardTwoListener.shouldRetainFireOwnerAfterPlaceEvent(true, player, player),
            "Paper's captured fire BlockPlaceEvent must not erase the preceding ignition owner",
        )
        check(
            !BingoCardTwoListener.shouldRetainFireOwnerAfterPlaceEvent(true, player, otherPlayer),
            "A fire placement by another player must clear stale ownership",
        )
        check(
            !BingoCardTwoListener.shouldRetainFireOwnerAfterPlaceEvent(false, player, player),
