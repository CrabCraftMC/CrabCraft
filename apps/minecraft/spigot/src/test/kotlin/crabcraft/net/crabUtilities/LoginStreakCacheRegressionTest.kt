...
object LoginStreakCacheRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        check(
            LoginStreakCache.preferNewerSnapshot(twenty, nineteen) === twenty,
            "delayed join refresh replaced a newer pub/sub snapshot",
        )
        check(
            LoginStreakCache.preferNewerSnapshot(nineteen, twenty) === twenty,
            "newer pub/sub snapshot was not accepted",
        )
        check(
            LoginStreakCache.preferNewerSnapshot(twenty, refreshedTwenty) === refreshedTwenty,
            "equal-version refresh was not accepted",
        )
    }

    private fun snapshot(streak: Int, lastLoginAt: Long) =
