...
object BingoCardFiveWorldListenerRegressionTest {
    @JvmStatic
    ...
    private fun protectsSnowLayerOwnership() {
        check(
    ...
    private fun requiresActualMinecartFuelGrowth() {
        check(
    ...
    private fun recognizesBoundCompassDroppedByFullInventory() {
        check(
    ...
    private fun preservesBlockOwnershipAcrossSameCardRestarts() {
        val playerId = UUID.randomUUID()
        val ownership = BingoCardFiveWorldListener.BlockOwnership(51, playerId, 9_000L)
        val encoded = BingoCardFiveWorldListener.encodeOwnership(ownership)
        check(
    ...
    private fun preservesBlockKeyGeometry() {
        val world = UUID.randomUUID()
        val origin = BingoCardFiveWorldListener.BlockKey(world, 10, 64, -3)
        check(
            origin.relative(BlockFace.NORTH).equals(BingoCardFiveWorldListener.BlockKey(world, 10, 64, -4)),
            "Horizontal Snow connectivity must remain in the same world and Y level",
        )
        check(
            origin.withY(72).equals(BingoCardFiveWorldListener.BlockKey(world, 10, 72, -3)),
