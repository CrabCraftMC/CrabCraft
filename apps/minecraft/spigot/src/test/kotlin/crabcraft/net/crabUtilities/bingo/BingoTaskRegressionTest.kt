...
object BingoTaskRegressionTest {
    ...
    private fun cardOneOrderIsStable() {
        ...
        check(idsOf(BingoTask.cardOne()).equals(listOf(
                        "grow_tree_in_nether",
    ...
    private fun cardTwoOrderIsStable() {
        val expected = listOf(
        ...
        check(idsOf(BingoTask.cardTwo()).equals(listOf(
                        "shear_bogged",
    ...
    private fun cardThreeOrderIsStable() {
        val expected = listOf(
        ...
        check(idsOf(BingoTask.cardThree()).equals(listOf(
    ...
    private fun cardFourOrderIsStable() {
        val expected = listOf(
        ...
        check(idsOf(BingoTask.cardFour()).equals(listOf(
    ...
    private fun cardFiveOrderIsStable() {
        val expected = listOf(
        ...
        check(idsOf(BingoTask.cardFive()).equals(listOf(
    ...
    private fun cardSixOrderIsStable() {
        val expected = listOf(
                BingoTask.HANG_FOUR_BY_FOUR_PAINTING,
                BingoTask.OUTLINE_HANGING_SIGN,
                BingoTask.FILL_CAMPFIRE_FOUR_SLOTS,
                BingoTask.SHOOT_BUTTON_WITH_ARROW,
                BingoTask.FISH_TREASURE_AND_JUNK,
                BingoTask.CARPET_LLAMA,
                BingoTask.ENCHANT_FIVE_ITEMS,
                BingoTask.THROW_MENDING_BOOK_IN_LAVA,
                BingoTask.STUN_RAVAGER,
                BingoTask.FULLY_POWER_CONDUIT,
                BingoTask.POISON_BEE,
                BingoTask.PLACE_FISH_IN_NETHER,
                BingoTask.NAMED_GHAST_OVERWORLD,
                BingoTask.FILL_ENDER_CHEST,
                BingoTask.APPLY_ARMOUR_TRIM,
                BingoTask.FOUR_SHERD_DECORATED_POT)
        check(BingoTask.cardSix().equals(expected), "Bingo #6 order changed")
        check(BingoTask.cardSix().size == 16, "Bingo #6 must contain 16 tasks")
        check(idsOf(BingoTask.cardSix()).equals(listOf(
                ...
                "Bingo #6 detector IDs changed")
    }

    private fun deployedCatalogueIsCompleteAndResolvable() {
        val deployed = BingoTask.allDeployed()
        check(deployed.equals(BingoTask.values().toList()),
                "Deployed catalogue does not contain every BingoTask enum value in declaration order")
        check(deployed.size == 96, "Expected 96 deployed bingo tasks")
    ...
    private fun cardCataloguesAreDisjointAndImmutable() {
        val cardOne = HashSet(BingoTask.cardOne())
        val cardTwo = HashSet(BingoTask.cardTwo())
        val cardThree = HashSet(BingoTask.cardThree())
        val cardFour = HashSet(BingoTask.cardFour())
        val cardFive = HashSet(BingoTask.cardFive())
        val cardSix = HashSet(BingoTask.cardSix())
        ...
        check(cardSix.size == 16, "Bingo #6 contains a duplicate task")
        ...
        for (earlier in listOf(cardOne, cardTwo, cardThree, cardFour, cardFive)) {
            check(earlier.stream().noneMatch(cardSix::contains),
                    "A task appears on Bingo #6 and an earlier card")
        }

        val combined = HashSet(cardOne)
        combined.addAll(cardTwo)
        combined.addAll(cardThree)
        combined.addAll(cardFour)
        combined.addAll(cardFive)
        combined.addAll(cardSix)
        check(combined.equals(HashSet(BingoTask.allDeployed())),
        ...
        checkUnmodifiable(BingoTask.cardSix(), "Bingo #6 catalogue is mutable")
        checkUnmodifiable(BingoTask.allDeployed(), "Deployed catalogue is mutable")
    }

    private fun retiredTasksRemainUnsupported() {
        check(BingoTask.fromId("leash_rabbit").isEmpty(), "Retired card task is still advertised")
    }

    private fun idsOf(tasks: List<BingoTask>): List<String> {
        return tasks.stream().map(BingoTask::id).toList()
    }

    private fun checkUnmodifiable(tasks: List<BingoTask>, message: String) {
        try {
            (tasks as MutableList<BingoTask>).add(BingoTask.SHEAR_BOGGED)
