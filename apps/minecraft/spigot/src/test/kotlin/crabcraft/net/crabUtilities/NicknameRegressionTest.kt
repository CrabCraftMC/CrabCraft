...
object NicknameRegressionTest {
    ...
    fun main(args: Array<String>) {
        check(
            !NicknameSync.hasAuthoritativeRedisValue(null),
            "a missing Redis field was treated as an authoritative clear",
        )
        check(NicknameSync.hasAuthoritativeRedisValue(""), "the explicit empty-string clear marker was ignored")
        ...
        check(plain == "[Crabby]", "MiniMessage tags leaked into the Essentials display name: $plain")
        val afk = NicknameSync.decoratedNickname(Component.text("[AFK] $raw"), raw)
        check(PLAIN.serialize(afk) == "[AFK] Crabby", "AFK display refresh reintroduced MiniMessage tags")
    ...
    private fun verifyLegacyDisplayRepair() {
        ...
        val visibleDisplay = Component.empty().append(prefix).append(Component.text("Crabby")).append(suffix)
        for (raw in listOf("&aCrabby", "§aCrabby")) {
            val repaired = NicknameSync.decoratedNickname(visibleDisplay, raw)
            checkPlain(repaired, "[VIP] Crabby!", "legacy component display lost its prefix or suffix")
            check(
                hasStyledText(repaired, "Crabby", NamedTextColor.GREEN),
                "legacy component display did not apply the parsed nickname for $raw",
            )
            check(
                repaired.children()[0] == prefix && repaired.children()[2] == suffix,
                "legacy component display changed its prefix or suffix component",
            )
        }
        val afkPrefix = Component.text("[AFK] ")
        val shortDisplay = Component.empty().append(afkPrefix).append(Component.text("A")).append(suffix)
        val shortRepair = NicknameSync.decoratedNickname(shortDisplay, "&aA")
        checkPlain(shortRepair, "[AFK] A!", "short nickname repair changed matching prefix letters")
        check(
            shortRepair.children()[0] == afkPrefix && shortRepair.children()[2] == suffix,
            "short nickname repair replaced part of the AFK prefix or suffix",
        )
        check(
            hasStyledText(shortRepair, "A", NamedTextColor.GREEN),
            "short visible nickname component was not repaired",
        )
    }

    private fun verifyAdvancement(nickname: Component, label: String) {
        val listener = listener(PlayerFixture(ACCOUNT_ID, "AccountName", nickname))
        val prefix = Component.text("[VIP] ")
        val suffix = Component.text("!")
        val advancement =
        ...
        check(player.children().size == 3, "$label advancement changed the team display wrapper shape")
        check(
            player.children()[0] == prefix && player.children()[2] == suffix,
        ...
        check(
            listener.transformComponent(transformed) == transformed,
            "$label advancement transform was not idempotent",
        )
    }

    private fun verifyTwoPlayerDeath() {
        val killerNickname = NicknameComponentResolver.fromRawNick("<gradient:#00ff00:#0000ff>Lobster</gradient>")!!
        val listener =
            listener(
        ...
        check(listener.transformComponent(transformed) == transformed, "two-player death transform was not idempotent")
    }

    private fun verifySuffixPreservation() {
        val listener = listener(PlayerFixture(ACCOUNT_ID, "AccountName", Component.text("Crabby")))
        ...
        check(listener.transformComponent(transformed) == transformed, "lifecycle suffix transform was not idempotent")
    }
    ...
    private fun listener(vararg players: PlayerFixture): NicknameMessageListener {
        ...
        for (player in players) {
            ...
            byUuid[player.uuid] = resolved
            byName[player.accountName] = resolved
        }
        return NicknameMessageListener(
            object : NicknameMessageListener.NicknameResolver {
                override fun byUuid(uuid: UUID) = byUuid[uuid]

                override fun byAccountName(accountName: String) = byName[accountName]
            }
        )
    }

    private fun argument(component: Component, index: Int): Component {
        if (component !is TranslatableComponent) throw AssertionError("expected a translatable component")
        val value = component.arguments()[index].value()
        if (value !is ComponentLike) throw AssertionError("expected a component translation argument")
    ...
    private fun hasStyledText(component: Component, text: String, color: NamedTextColor): Boolean =
        component.iterable(ComponentIteratorType.DEPTH_FIRST).any {
            it is TextComponent && it.content() == text && it.color() == color
        }

    private fun checkOnce(value: String, nickname: String, label: String) {
        check(
            value.indexOf(nickname) >= 0 && value.indexOf(nickname) == value.lastIndexOf(nickname),
    ...
    private data class PlayerFixture(val uuid: UUID, val accountName: String, val nickname: Component)
}
