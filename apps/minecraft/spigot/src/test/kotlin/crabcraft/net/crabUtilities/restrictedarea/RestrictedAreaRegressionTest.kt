...
object RestrictedAreaRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        Items.COOKED_BEEF.builtInRegistryHolder()
            .bindComponents(
    ...
    private fun verifyConfiguration() {
        RestrictedAreaRegressionTest::class.java.classLoader.getResourceAsStream("modules/gameplay.yml").use { input ->
            ...
            config.loadFromString(String(input!!.readAllBytes(), Charsets.UTF_8))
            check(!config.getBoolean("restricted-area.enabled", true), "protection is not opt-in")
            check(
                config.getString("restricted-area.bypass-permission", "") == RestrictedAreaSettings.DEFAULT_PERMISSION,
                "default permission changed",
            )
            check(
                !config.contains("restricted-area.bounds") &&
                    !config.contains("restricted-area.world") &&
                    !config.contains("restricted-area.return-location"),
                "obsolete settings remain in defaults",
            )
        }
    ...
    private fun verifyPermissionChangesAreLive() {
        ...
        check(
            !RestrictedAreaListener.isRestricted(player, RestrictedAreaSettings.disabled()),
            "disabled feature still restricts players",
        )
    }

    private fun verifyGriefProtectionInEveryWorld() {
        for (name in listOf("world", "world_nether", "world_the_end", "custom_world")) {
            val listener = listener()
            val verified = AtomicBoolean()
            val player =
                player(verified, AtomicBoolean(), AtomicInteger(), proxy(World::class.java, mapOf("getName" to name)))
            ...
            // Preserve the deliberately absent item used by the original Java event mock.
            val placing =
                BlockPlaceEvent::class
                    ...
                    .newInstance(
                        block,
                        proxy(BlockState::class.java, emptyMap()),
                        block,
                        null,
                        player,
                        true,
            ...
            for (target in
                listOf(
                    proxy(Entity::class.java, emptyMap()),
                    player(AtomicBoolean(true), AtomicBoolean(), AtomicInteger(), player.world),
                )) {
        ...
        for (type in
            ...
            val view =
                proxy(
                    ...
                    mapOf("getType" to type, "getPlayer" to player, "getTopInventory" to inventory),
                )
            ...
            val personal =
                type == InventoryType.CRAFTING || type == InventoryType.CREATIVE || type == InventoryType.PLAYER
    ...
    private fun verifyNormalMovementAndCommands() {
        ...
        val events =
            listOf(
                ...
                PlayerToggleFlightEvent(player, true),
                FoodLevelChangeEvent(player, 10, null),
                PlayerCommandPreprocessEvent(player, "/tp 0 90 0", emptySet()),
                PlayerCommandPreprocessEvent(player, "/msg friend hello", emptySet()),
            )
        for (event in events) {
            dispatch(listener, event)
            check(!(event as Cancellable).isCancelled, "unrelated behaviour was restricted: ${event.eventName}")
        }
    ...
    private fun verifyPassiveInteractionsStaySilent() {
        ...
        val physical =
            PlayerInteractEvent(player, Action.PHYSICAL, null, proxy(Block::class.java, emptyMap()), BlockFace.UP)
        ...
        val breaking =
            PlayerInteractEvent(
                ...
                null,
                proxy(Block::class.java, emptyMap()),
                BlockFace.UP,
            )
        ...
        val chest =
            PlayerInteractEvent(
                ...
                null,
                proxy(Block::class.java, mapOf("getType" to Material.CHEST)),
                BlockFace.UP,
            )
        ...
        val food =
            object :
                PlayerInteractEvent(
                    ...
                    Action.RIGHT_CLICK_BLOCK,
                    null,
                    proxy(Block::class.java, mapOf("getType" to Material.STONE)),
                    BlockFace.UP,
                ) {
    ...
    private fun dispatch(listener: RestrictedAreaListener, event: Event) {
        for (method in RestrictedAreaListener::class.java.declaredMethods) {
            if (method.isAnnotationPresent(EventHandler::class.java) && method.parameterTypes[0].isInstance(event)) {
                method.invoke(listener, event)
            }
        }
    }

    private fun listener() = RestrictedAreaListener(null, RestrictedAreaSettings.load(enabledConfig()))
    ...
    private fun player(
        verified: AtomicBoolean,
        operator: AtomicBoolean,
        messages: AtomicInteger,
        world: World,
    ): Player {
