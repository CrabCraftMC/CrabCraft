...
object NicknameMessagesRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        check(
            plain(NicknameComponentResolver.fromRawNick("&aCrabby")) == "Crabby",
        ...
        val standardJson = GsonComponentSerializer.gson().serialize(standard!!).lowercase()
        check(standardJson.contains("red") && standardJson.contains("bold"), "standard nickname styling was discarded")
        check(
            plain(NicknameComponentResolver.fromRawNick("<color:red>Crabby</color>")) == "Crabby",
            "explicit color tag was not parsed",
        )
        check(
            plain(NicknameComponentResolver.fromRawNick("<gradient:#ff0000:#0000ff>Crabby</gradient>")) == "Crabby",
            "gradient nickname was not parsed",
        )
        check(
            plain(NicknameComponentResolver.fromRawNick("<rainbow>Crabby</rainbow>")) == "Crabby",
            "rainbow nickname was not parsed",
        )
        check(
            plain(NicknameComponentResolver.fromRawNick("<red>Crab<reset>by")) == "Crabby",
        ...
        check(
            plain(NicknameComponentResolver.fromRawNick(malformed)) == malformed,
        ...
        check(
            GsonComponentSerializer.gson().serialize(single).lowercase().contains("#12ab34"),
            "single sleeper lost nickname colour",
        )
        val two =
            SleepBroadcastListener.formatMessage("<players> slept (<count>).", listOf(crabby, Component.text("Shelly")))
        check(plain(two) == "Crabby and Shelly slept (2).", "two-player nickname list was not joined naturally")
        check(
            SleepBroadcastListener.joinNames(listOf(crabby, Component.text("Shelly"))).color() == null,
            "first nickname colour leaked into the rest of the sleeper list",
        )
        val three =
            SleepBroadcastListener.formatMessage(
                "<players>",
                listOf(crabby, Component.text("Shelly"), Component.text("Claws")),
            )
        check(plain(three) == "Crabby, Shelly, and Claws", "three-player nickname list was not joined naturally")
        val literalTags =
            SleepBroadcastListener.formatMessage("<player> slept.", listOf(Component.text("<red>Crab</red>")))
        check(plain(literalTags) == "<red>Crab</red> slept.", "nickname text was parsed as MiniMessage")
        check(
            NicknameComponentResolver.fromRawNick(null) == null,
            "missing nickname did not preserve the account-name fallback",
        )
    }

    private fun plain(component: Component?) = PLAIN.serialize(component!!)
    ...
    private fun checkNoInteractiveEvents(component: Component, raw: String) {
        for (child in component.iterable(ComponentIteratorType.DEPTH_FIRST)) {
            check(
                child.clickEvent() == null && child.hoverEvent() == null && child.insertion() == null,
