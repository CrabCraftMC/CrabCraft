...
object EndPortalBlockerListenerRegressionTest {
    ...
    fun main(args: Array<String>) {
        ...
        val listener = EndPortalBlockerListener(preventEntry::get)
        assertNotCancelled(
            listener,
            portalEvent(Player::class.java, PortalType.ENDER),
        ...
        assertCancelled(
            listener,
            portalEvent(Entity::class.java, PortalType.ENDER),
            "non-player End portal entry was not blocked",
        )
        assertNotCancelled(
            listener,
            portalEvent(Player::class.java, PortalType.NETHER),
            "Nether portal entry was blocked",
        )
        assertNotCancelled(
            listener,
            portalEvent(Player::class.java, PortalType.END_GATEWAY),
        ...
        javaClass.classLoader.getResourceAsStream("modules/tweaks.yml").use { input ->
        ...
        check(
            !config.getBoolean(EndPortalBlockerListener.CONFIG_PATH, true),
            "End portal blocker is not disabled by default",
        )
    }

    private fun <T : Entity> portalEvent(type: Class<T>, portalType: PortalType) =
        EntityPortalEnterEvent(proxy(type), Location(null, 0.0, 0.0, 0.0), portalType)
