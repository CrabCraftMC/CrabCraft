...
object BingoCardFiveMobListenerRegressionTest {
    ...
    private fun requiresAnExactDroppedPandaCake() {
        check(
    ...
    private fun checksIronGolemRepairPreconditions() {
        check(
    ...
    private fun requiresConfirmedHealthGrowth() {
        check(
    ...
    private fun keepsAttributionOnlyForTheSameCard() {
        check(
        ...
        check(
                !BingoCardFiveMobListener.markerMatchesCard(5, Int.MIN_VALUE),
                "Attribution must be rejected while no card is active")
    }

    private fun expiresAttributionAcrossTimeAndPlayerResetBoundaries() {
        val now = 10_000L
        check(
