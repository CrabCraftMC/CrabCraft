...
object ModuleConfigManagerRegressionTest {
    ...
    private fun verifyValueStagingDoesNotMutateCachedConfiguration() {
        ...
        check(
            cached.getDouble("media.discs.volume") == 1.0,
            "staging a module save mutated the cached configuration",
        )
        check(
            staged.getDouble("media.discs.volume") == 0.35,
    ...
    private fun verifyBundledLayoutAndMergedView() {
        ...
        check(
            merged.getString("redis.host", "") == "localhost",
            "core configuration disappeared from the merged view",
        )
        check(
            merged.getBoolean("mod-protocols.jade.enabled"),
            "integration configuration disappeared from the merged view",
        )
        check(
            !merged.getBoolean("mod-protocols.accurate-block-placement.enabled"),
            "accurate block placement is not opt-in in the merged view",
        )
        check(
            merged.getString("media.providers.yt-dlp-path", "") == "auto",
            "media configuration disappeared from the merged view",
        )
        check(
            !merged.getBoolean("tweaks.view-distance.enabled"),
    ...
    private fun verifyDefaultMergePreservesCustomValues() {
        ...
        check(
            custom.getDouble("media.discs.volume") == 0.35,
            "custom media value was overwritten by a default",
        )
        check(
            custom.getString("media.providers.yt-dlp-path", "") == "auto",
            "missing media provider default was not added",
        )
    }

    private fun verifyPathOwnership() {
        check(moduleName("media.providers.ffmpeg-path") == "media", "media path was routed to the wrong module")
        check(moduleName("mod-protocols.jade.enabled") == "integrations", "Jade path was routed to the wrong module")
        check(
            moduleName("bluemap.sign-markers.enabled") == "integrations",
            "BlueMap path was routed to the wrong module",
        )
        check(moduleName("public-chat.enabled") == "chat", "public chat path was routed to the wrong module")
        check(
            moduleName("restricted-area.enabled") == "gameplay",
            "restricted area path was routed to the wrong module",
        )
        check(moduleName("tweaks.view-distance.enabled") == "tweaks", "tweak path was routed to the wrong module")
        check(ModuleConfigManager.moduleForPathStatic("redis.host") == null, "core Redis path was routed to a module")
        val names = ModuleConfigManager.MODULES.map { it.name() }
        check(
            names == listOf("integrations", "chat", "voicechat", "media", "gameplay", "tweaks"),
    ...
    private fun load(resource: String): YamlConfiguration {
        ModuleConfigManagerRegressionTest::class.java.classLoader.getResourceAsStream(resource).use { input ->
            ...
            return YamlConfiguration().apply {
                loadFromString(String(input!!.readAllBytes(), StandardCharsets.UTF_8))
            }
        }
    }
