...
export async function runTicketMemberCommand(
  ...
  const invokingMember = await interaction.guild.members
    ...
    .catch(() => null);
  const isStaff = invokingMember?.roles.cache.has(config.MOD_ROLE_ID)
    || invokingMember?.roles.cache.has(config.COUNCIL_ROLE_ID);
  if (interaction.user.id !== ticket.opener_discord_id && !isStaff) {
    await reply(
      interaction,
      errorContainer("**Error!** Only the ticket opener or staff can manage ticket members."),
    );
    return;
  }
