...
export async function createApplicationChannelFor(
  ...
  // Legacy fallback: match by name for channels created before the DB record.
  if (!channel) {
    channel =
      (member.guild.channels.cache.find(
        (c) =>
          c.parentId === category.id &&
          c.name === `app-${member.user.username}`,
 ...
 * channel, and drop the DB row. Pass `transcriptReason = null` when a
 * transcript was already saved (e.g. at accept/deny time). Never throws.
 */
export async function finalizeApplicationChannel(
  ...
  if (channel && transcriptReason) {
    const logChannel = (await guild.channels
      .fetch(config.LOG_CHANNEL_ID)
      .catch(() => null)) as TextChannel | null;
    if (logChannel) {
      await saveTranscriptToLog(channel, logChannel, transcriptReason).catch(
        () => null,
      );
    }
  }
