...
import {
  getStreamChannelsByPlatform,
  getVerifiedStreamChannelsByPlatform,
} from "./appDb.js";
import {
  YOUTUBE_RSS_POLL_MS,
...
import { singleFlight, withDeadline } from "./streamPoll.js";
import {
  MAX_TWITCH_POLL_TARGETS,
  selectTwitchPollTargets,
  twitchLoginBatches,
  TWITCH_REQUEST_TIMEOUT_MS,
} from "./twitchPoll.js";
...
async function pollTwitch(guild: Guild): Promise<void> {
  const candidates = await getVerifiedStreamChannelsByPlatform("twitch");
  const channels = selectTwitchPollTargets(
    candidates,
    new Set(guild.members.cache.keys()),
  );
  if (channels.length === 0) return;

  if (candidates.length > MAX_TWITCH_POLL_TARGETS) {
    logger.warn(
      `Twitch poll capped at ${MAX_TWITCH_POLL_TARGETS} eligible channels`,
    );
  }
  ...
  for (const batch of twitchLoginBatches(channels)) {
    const params = new URLSearchParams();
...
async function checkTikTokLive(username: string): Promise<boolean> {
  try {
    ...
    const connection = new TikTokLiveConnection(username, {
      webClientOptions: {
        timeout: { request: TIKTOK_REQUEST_TIMEOUT_MS },
      },
    } as ConstructorParameters<typeof TikTokLiveConnection>[1]);
...
async function pollTikTok(guild: Guild): Promise<void> {
  const candidates = await getVerifiedStreamChannelsByPlatform("tiktok");
  const channels = selectTikTokPollTargets(
    candidates,
    new Set(guild.members.cache.keys()),
  );
  if (channels.length === 0) return;

  if (candidates.length > MAX_TIKTOK_POLL_TARGETS) {
    logger.warn(
      `TikTok poll capped at ${MAX_TIKTOK_POLL_TARGETS} eligible channels`,
    );
  }

  const checks = await checkTikTokPollTargets(channels, checkTikTokLive);
  for (const { channel, isLive } of checks) {
    try {
...
export async function initStreamMonitor(client: Client): Promise<void> {
  ...
  if (!guild) {
    logger.error("Stream monitor: configured guild not found");
    return;
  }
  ...
  if (config.TWITCH_CLIENT_ID && config.TWITCH_CLIENT_SECRET) {
    const pollTwitchOnce = singleFlight(() => pollTwitch(guild));
    setInterval(async () => {
      try {
        const started = await pollTwitchOnce();
        if (!started) logger.warn("Skipped overlapping Twitch poll");
      }
  ...
  const pollTikTokOnce = singleFlight(() => pollTikTok(guild));
  setInterval(async () => {
    try {
      const started = await pollTikTokOnce();
      if (!started) logger.warn("Skipped overlapping TikTok poll");
    }
