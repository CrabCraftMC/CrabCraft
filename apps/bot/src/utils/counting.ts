...
const MEDIA_PARSE_COOLDOWN_MS = 10_000;
const lastMediaParseByUser = new Map<string, number>();
...
function findImageUrl(message: Message | PartialMessage): string | null {
  const att = message.attachments?.find((a) => {
    if (a.size > MAX_IMAGE_BYTES) return false;
  ...
  for (const embed of message.embeds ?? []) {
    ...
    if (url && isDiscordMediaUrl(url)) return url;
  }
  return null;
}
...
function findVoiceUrl(message: Message | PartialMessage): string | null {
  const att = message.attachments?.find((a) => {
    if (a.size > MAX_VOICE_BYTES) return false;
...
function isDiscordMediaUrl(value: string): boolean {
  try {
...
function claimMediaParse(userId: string | undefined): boolean {
  if (!userId) return false;
  const now = Date.now();
  const last = lastMediaParseByUser.get(userId) ?? 0;
  if (now - last < MEDIA_PARSE_COOLDOWN_MS) return false;
  lastMediaParseByUser.set(userId, now);
  if (lastMediaParseByUser.size > 10_000) {
    for (const [id, timestamp] of lastMediaParseByUser) {
      if (now - timestamp >= MEDIA_PARSE_COOLDOWN_MS) {
        lastMediaParseByUser.delete(id);
      }
    }
  }
  return true;
...
export async function parseNumberFromMessage(
  ...
  if (!findVoiceUrl(message) && !findImageUrl(message)) return null;
  if (!claimMediaParse(message.author?.id)) return null;
