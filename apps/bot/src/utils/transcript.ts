...
async function fetchAllMessages(channel: TextChannel, maxMessages?: number): Promise<Message[]> {
  ...
  const cap = Math.max(0, Math.min(maxMessages ?? DEFAULT_MAX_MESSAGES, MAX_MESSAGES));
...
function renderMessage(msg: Message): string {
  ...
  const avatar = safeExternalUrl(author.displayAvatarURL({ size: 64 }));
  ...
  for (const [, att] of msg.attachments) {
    const url = safeExternalUrl(att.url);
    if (url && att.contentType?.startsWith("image/")) {
      inner += `<div class="msg-attachment"><img src="${esc(url)}" alt="${esc(att.name ?? "image")}" style="max-width:400px;border-radius:8px" /></div>`;
    } else if (url) {
      inner += `<div class="v2-file"><a href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(att.name ?? "file")}</a> (${formatBytes(att.size)})</div>`;
    }
  }
...
function renderThumbnail(comp: any): string {
  const url = safeExternalUrl(comp.media?.url ?? comp.data?.media?.url ?? "");
...
function renderMediaGallery(comp: any): string {
  const items = (comp.items ?? comp.data?.items ?? [])
    .map((item: any) => {
      const url = safeExternalUrl(item.media?.url ?? "");
...
function renderButton(comp: any): string {
  ...
  if (style === ButtonStyle.Link && data.url) {
    ...
    if (href) {
      return `<a class="v2-button ${styleClass}${disabled}" href="${esc(href)}" target="_blank" rel="noopener noreferrer">${emoji}${label} ↗</a>`;
    }
  }
...
function renderEmbed(embed: APIEmbed): string {
  ...
  if (embed.title) {
    const href = embed.url ? safeExternalUrl(embed.url) : null;
    const titleHtml = href
      ? `<a href="${esc(href)}" target="_blank" rel="noopener noreferrer">${esc(embed.title)}</a>`
      : esc(embed.title);
  ...
  const imageUrl = embed.image?.url ? safeExternalUrl(embed.image.url) : null;
  if (imageUrl) {
    inner += `<img src="${esc(imageUrl)}" style="max-width:400px;border-radius:4px;margin-top:8px" />`;
  }
  const thumbnailUrl = embed.thumbnail?.url ? safeExternalUrl(embed.thumbnail.url) : null;
  if (thumbnailUrl) {
...
export function renderMarkdown(text: string): string {
  ...
  html = html.replace(
    /\[([^\]]+)\]\((&lt;)?([^)>]+)(&gt;)?\)/g,
    (_, label, _openingBracket, rawUrl) => {
      const href = safeExternalUrl(decodeEscapedUrl(rawUrl));
      return href
        ? `<a href="${esc(href)}" target="_blank" rel="noopener noreferrer">${label}</a>`
        : `${label} <span class="unsafe-link">[unsafe link removed]</span>`;
    },
  );
  ...
  html = html.replace(
    ...
    (_, rawUrl) => {
      const href = safeExternalUrl(decodeEscapedUrl(rawUrl));
      return href
        ? `<a href="${esc(href)}" target="_blank" rel="noopener noreferrer">${rawUrl}</a>`
        : rawUrl;
    },
  );
...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src https: data:; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'; object-src 'none'; script-src 'none'">
