...
// Match the website's public, redistributable Unbounded typeface.
...
function medal(label: string, value: number, from: string, to: string) {
  return container({
    ...
    children: [
      ...
      text(String(value), { fontFamily: SANS, fontSize: 34, color: "#ffffff" }),
    ],
  });
}
...
function statTile([label, value]: [string, string]) {
  return container({
    ...
    children: [
      text(value, { fontFamily: SANS, fontSize: 26, color: FG }),
...
export async function generatePlayerCard(data: PlayerCardData): Promise<Buffer> {
  ...
  const headerLeft: any[] = [text(data.username, {
    fontFamily: SANS,
    fontSize: data.username.length > 12 ? 26 : 38,
    fontWeight: 700,
    color: "#ffffff",
  })];
  ...
  gradientChildren.push(container({
    ...
    children: [
      ...
      container({
        ...
        children: [
          text(String(data.points), { fontFamily: SANS, fontSize: 48, color: "#ffffff" }),
    ...
    children: [
      ...
      skin
        ? image({
            ...
            width: skinW,
            height: SKIN_H,
            // Takumi v2 defaults object-position to center; preserve v1's top-left anchor.
            style: { position: "absolute", bottom: 0, left: 24, objectPosition: "left top" } as any,
          })
