...
export default async function HomePage() {
  ...
  const homepageStatsPromise = getHomepageStats().catch(() => ({
    seasonCount: 0,
    acceptedCurrentSeasonPlayerCount: 0,
  }));
  const whitelistedPlayerCountPromise = process.env.DISCORD_BOT_TOKEN
    ? getCurrentSeasonWhitelistedPlayerCount().catch((error) => {
        console.error("Failed to count current-season Discord members:", error);
        return null;
      })
  ...
  whitelistedPlayers =
    currentSeasonPlayerCount ?? homepageStats.acceptedCurrentSeasonPlayerCount;
              ...
              >
                ...
                <div className="relative z-10 flex flex-col justify-between h-full">
                  ...
                  <div className="mt-4">
                    ...
                    <p className="text-white/70 text-sm mt-1">this season</p>
                  </div>
                </div>
              </Squircle>
            </div>
