...
const TEXTURE_BASE =
  "https://raw.githubusercontent.com/Mojang/bedrock-samples/921fafb05c93abeae56c2f2868cd8f942bdbcc0f/resource_pack/textures/blocks";

const MC_TEXTURE_BASE =
  "https://raw.githubusercontent.com/InventivetalentDev/minecraft-assets/81a98400f6909b707cf0ebfde1b2b73e8d70256b/assets/minecraft/textures";
...
async function downloadPng(url: string): Promise<Buffer> {
  ...
  try {
    ...
    if (Number.isFinite(declared) && declared > MAX_TEXTURE_BYTES) {
      throw new Error("texture exceeds 2 MiB");
    }
    if (!response.body) throw new Error("texture response has no body");
    ...
    if (
      ...
      || !metadata.width
      || !metadata.height
      || metadata.width > MAX_TEXTURE_DIMENSION
      || metadata.height > MAX_TEXTURE_DIMENSION
    ) {
      throw new Error("texture is not a bounded PNG");
    }
    return buffer;
  } finally {
    ...
    await Promise.all(
      batch.map(async (texture) => {
        ...
        try {
          ...
          let buffer: Buffer<ArrayBufferLike> = await downloadPng(url);
 ...
 * Fetches each image from an immutable upstream commit.
 */
async function downloadWrappedTextures() {
  ...
  for (const path of paths) {
    ...
    // Fetch from the pinned Java-edition asset revision. Animated items like clock /
  ...
  console.log(
    `  Story textures: ${downloaded} downloaded, ${skipped} already existed, ${failed} failed.`
  );
}
